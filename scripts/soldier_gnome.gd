...
## ⭐ ВКЛЮЧИТЬ/ВЫКЛЮЧИТЬ РЕЖИМ ЭКИПАЖА (2026-08-11). Мало поднять флаг: гном мог
## УЖЕ быть в разгоне к дереву или горшку, а charge ведёт своя state-machine и
## squad-логика в неё не вмешивается — тело так и ехало бы рабочим шагом, игнорируя
## команды строя. Поэтому включение ещё и сбрасывает текущий заход в READY.
func set_crew_mode(on: bool) -> void:
	crew_mode = on
	if not on:
		return
...
## ⭐ РЕЖИМ ЭКИПАЖА (2026-08-11): гнома ведёт игрок как отряд, поэтому своих дел
## он не ищет — ни врага, ни дерева, ни горшка, ни стройки. Только держит строй.
## false = штатное поведение, так что основная игра и данж без этого флага
## работают ровно как раньше. Ставит CrewController, пока экипаж снаружи Ладьи.
var crew_mode: bool = false

## СВОБОДНЫЙ БИНД (данж, 2026-08-07): на какой кнопке работает этот гном.
## 0 = ЛКМ, 1 = ПРОБЕЛ, 2 = ПКМ. Кнопка больше НЕ называет способность — она
## называет «кто по ней работает», и каждый гном делает СВОЁ: лучник льёт,
## копейщик бьёт, артельщик катит вал, огневик пускает залп. Типы на одной
## кнопке можно мешать. Дефолт ставит сцена данжа по классу, поэтому без
## перенастройки раскладка ровно прежняя; основной игре поле безразлично.
var bind_slot: int = 0
...
func _active_tick(delta: float) -> void:
	...
	# ⭐ РЕЖИМ ЭКИПАЖА ОБХОДИТ ВСЮ РАБОЧУЮ ВЕТКУ (2026-08-11). Ниже идёт логика
	# «рабочий живёт своей жизнью»: смена в здании, тревога, прятки в башне,
	# наряд по area-клику — и заканчивается она безусловным return, то есть до
	# ветки строя (step_to_slot) рабочий НЕ ДОХОДИТ ВООБЩЕ.
	#
	# Именно здесь застревал отряд в большом мире: все стартовые гномы там —
	# рабочие, они уходили сюда и топали своим шагом 1.5 м/с, полностью
	# игнорируя команду хода (замер: body_velocity=7.5, фактическая 1.5).
	# Ни crew_mode на поиске целей, ни сброс charge, ни move_speed это не
	# лечили — они правили ветки, до которых исполнение не доживало.
	#
	# Гном, которого ведёт игрок, не «рабочий на смене»: он экипаж и держит
	# строй. Дефолт crew_mode = false, поэтому основная игра и данж идут прежним
	# путём буква в букву.
	if is_worker() and not crew_mode:
	...
	# ⭐ РЕЖИМ ЭКИПАЖА (2026-08-11, дефолт false → основная игра не затронута).
	# Гном, которого ведёт игрок как отряд, НЕ уходит по своим делам: иначе в
	# большом мире он мгновенно цеплял ближайшее дерево/горшок/стройку, валился
	# в charge и до строя (step_to_slot) управление не доходило вовсе — строй
	# плёлся рабочим шагом 1.5 м/с при команде 7.5. Ставится покадрово из
	# CrewController, пока экипаж снаружи; сел в башню — снимается.
	if _attack_cd <= 0.0 and not crew_mode:
