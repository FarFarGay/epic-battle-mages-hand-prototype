...
const SPELL_CATALOG: Dictionary = {
	...
	&"firestorm": {
		...
		# ⚖ mana_cost 75 → 10 (2026-08-08): тот же потолок «дэш не дороже 10».
		# ⚠ ОТКРЫТОЕ: damage 200 НИЖЕ, чем у фаербола (250), хотя фигура здесь
		# впятеро сложнее — пентаграмма из 5 точек против росчерка из 2. Сложный
		# заряд обязан бить сильнее простого; кандидат — 280. Не меняю без
		# решения: это баланс заклинания, а не потолок цены дэша.
		"dash_form": {
