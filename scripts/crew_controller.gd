...
##
## ⚠ ПОТОЛОК КРИТИЧЕН, И ОН НЕ «ПРОСТО ЧИСЛО». Поправка складывается с ходом
## тела: desired = body_velocity + corr. При ходе 7.5 и потолке 6 поправка,
## смотрящая назад (гном обогнал свой слот), почти ГАСИЛА движение — 7.5 − 6 =
## 1.5. Ровно это и наблюдалось: тело дёргалось между 1 и 11.6 м/с, центр отряда
## полз 2.2 при команде 7.5, а строй растягивало на 5 метров.
## Потолок 2 → крейсер ровно 7.5 м/с и разброс 1.0 м. Поднимать выше ~1/3 хода
## нельзя: вернётся то же самое.
@export var body_pull: float = 10.0
@export var body_pull_max: float = 2.0
...
func _physics_process(delta: float) -> void:
	...
	# же точки, и группа топчется вместо того чтобы ехать. В замере это выглядело
	# как асимметрия по осям: вбок 5.1 м/с, вперёд 2.6 при одной и той же команде.
	# Слерпим плавно: резкий флип телепортирует слоты и рвёт строй.
	if _vel.length_squared() > 0.25:
		var dir: Vector3 = _vel.normalized()
		_facing = dir if _facing.length_squared() < 0.01 \
				else _facing.slerp(dir, minf(1.0, delta * 3.5)).normalized()
	...
	# Присваиваем ПОЛЕ, а не зовём command_hold: во-первых, без сигнала на каждый
	# кадр, во-вторых — надёжно. Отряд в этом уровне переводят в оборону чужие
	# системы (лагерь/директор волн реагируют на скелетов), и тогда гном уходит
	# в _tick_defend_patrol с его прогулочным шагом 1.2 м/с. В замере это и был
	# тот самый потолок: команда (0,0,−7), а тело едет (0,0,−1).
