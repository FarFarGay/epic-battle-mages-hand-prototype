...
## Клик по Машинному цеху → HUD открывает окно ковки машин (экс-магазин заклинаний,
## пересборка 2026-07-21).
signal spell_shop_requested
## Клик по Плавильне-ядру → HUD открывает её панель (узлы ветки + статус трубы).
signal smelter_panel_requested(smelter: Node3D)
...
## Пад-здание УНИЧТОЖЕНО боем ([PadBuilding._die]; осознанный ПКМ-снос идёт другим
## путём — HandPlaceAim.demolished). Колода в HUD проверяет ПРАВИЛО ключевых зданий:
## последняя шахта/институт/верфь без карты в руке/колоде → карта наверх колоды.
signal pad_building_destroyed(building_id: StringName)
...
## Супер-дэш башни доехал до цели (рывок, заряженный обводкой фигуры пируэта).
## position — наземная точка импакта, spell_id — заклинание, чья дэш-форма была
## заряжена. Слушает HandSpell: диспатчит импакт-эффект на модуль-владелец
## (fireball → взрыв+горение, firestorm → залп по области). Прерванный рывок
## (отброс тараном меха) сигнал НЕ эмитит.
signal super_dash_impact(position: Vector3, spell_id: StringName)
