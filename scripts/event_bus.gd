...
## Башня ЗАЕХАЛА на платформу-верфь → HUD открывает окно срезов башни (TowerUpgrades,
## покупка за монеты). Эмитится на фронте заезда (не каждый тик стоянки).
signal tower_dock_requested
## Башня ВЫЕХАЛА с платформы-верфи → HUD закрывает окно срезов, если оно открыто.
signal tower_dock_left
