...
##   Свиток огня (пещера гномов) → САМА БАШНЯ (переопределён _nearest_receiver):
##                                 фаербол выучен навсегда (профиль).
##   Звёздный кристалл (застава) → верфь («dock»): клад — монеты в казну.
## (Чертёж Кафедры огня из храма — НЕ ArtifactElement: он FireLabBlueprint
## на языке чертежей CastleBlueprint, кладётся на верх башни.)
