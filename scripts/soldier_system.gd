...
const SOLDIER_CATALOG: Dictionary = {
	&"archer_squad": {
		...
		"squad_size": 3,
		# Потолок отряда = размеру: один найм заполняет посты гарнизона казармы,
		# повторный найм лишь доливает павших (стол торга гасит «Купить» на full).
		"squad_cap": 3,
	...
	&"pikeman": {
		...
		"squad_size": 3,
		# Потолок отряда = ЧИСЛУ КЛЕТОК казармы-стены (3): по 1 копейщику на клетку-пост.
		# Повторный найм доливает павших до 3 (стол торга гасит «Купить» на full).
		"squad_cap": 3,
		...
		"stats": {
			...
			"attack_range": 2.8,  # длинное копьё: гарнизонный достаёт скелетов у основания стены с боевого хода
