...
func _ready() -> void:
	...
	# **НЕ** в [navmesh_source]: NavMesh видит ворота как открытый проём
	# в стене. Гномы (NavAgent3D-driven, mask=TERRAIN) строят путь через
	# ворота — короткий шорткат. Скелеты (NavAgent3D + mask содержит
	# WALL_GATE_BLOCK) тоже пытаются пройти через ворота как через проём,
	# но физически упираются → стучат по ним как по cели (ворота в
	# skeleton_target). Дизайнерски это и есть «защитное узкое горло».
