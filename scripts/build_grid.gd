...
func _spawn_vein_marker(cell: Dictionary) -> void:
	...
	# Сборка залежи — общий язык в VeinVisuals (перенесено 2026-07-21, делят
	# с полевой стоячей жилой VeinBoulder).
	if t == ResourcePile.ResourceType.WOOD:
		VeinVisuals.build_grove(root)
	else:
		var cols: Array = VeinVisuals.colors_for_type(t)
		VeinVisuals.build_ore_pile(root, cols[0], cols[1])
	cell["marker"] = root
