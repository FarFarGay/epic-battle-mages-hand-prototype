...
## Активная категория ввода. Захват предметов работает в ЛЮБОЙ категории (ЛКМ),
## различается только ПКМ: в PHYSICAL это шлепок/щелчок (базовый глагол добычи),
## в MAGIC — выстрел установленной пушки.
## ⭐ СТАРТ — ФИЗИКА, НЕ МАГИЯ (2026-08-14). Пока заклинания выбирались карточкой
## в трее, дефолт MAGIC был безобиден. С переходом на пушки колдовать можно
## только тем аппаратом, что стоит на крыше: на пустой крыше рука в MAGIC — это
## ПКМ, заряженный запертым огнешаром, то есть молчащая кнопка на старте партии.
## Категорию поднимает сама пушка при монтаже (WarMachine.equip_to_hand).
