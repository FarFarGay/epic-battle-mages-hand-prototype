...
## Остальные — собиратели (Gnome). С 2026-05-17 дефолт = 0: игрок стартует
## БЕЗ защитников, производит их через рекрут «3 гнома + 6 wood + 4 iron»
## в журнале (вкладка «Армия»). Поле оставлено @export для случая дизайнерских
## исключений (например, «fortress tent» с встроенным гарнизоном — выставит 1+
## per-instance в инспекторе). Раньше было 1: 4 жителя → 1 защитник + 3 собирателя
## в каждой палатке (4 палатки = 4 защитника на старте).
@export var defenders_per_tent: int = 0
