...
const CATALOG: Dictionary = {
	...
	# Плавильня: переплавляет металл. Группа &"smelter" — цель доставки гномов. 2 клетки:
	# печь+труба. Монеты сами НЕ чеканит — металл течёт дальше линией в чеканный двор.
	PAD_SMELTER: {
		"name": "Плавильня",
		"menu_label": "🔥 Плавильня (плавит металл)",
		"role": &"smelter",
	...
	# Чеканный двор: чеканит монеты из металла. Клик по двору — ручная перечеканка 100→1.
	# Сами монеты в казну идут только если двор подключён к ГНОМЬЕМУ БАНКУ. 2 клетки.
	PAD_MINT: {
		"name": "Чеканный двор",
		"menu_label": "🪙 Чеканный двор (чеканит)",
		"role": &"mint",
		"cells": [Vector2i(0, 0), Vector2i(1, 0)],
		"instant": true,
		"cost": {ResourcePile.ResourceType.BRONZE: 25},
	...
	# Гномий банк: помпезная крепость, конец всей цепочки. Монеты «отвозят» сюда → они
	# появляются у игрока (казна GoldBank). Ставится рядом с чеканным двором. 2 клетки.
	PAD_BANK: {
		"name": "Гномий банк",
