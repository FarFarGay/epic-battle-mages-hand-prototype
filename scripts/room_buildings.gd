...
const PAD_BARRACK := &"pad_barrack"    # барак: сапорт казармы (+кап найма), домино 2 клетки (роль barrack)
...
const CATALOG: Dictionary = {
	...
	# Барак — САПОРТ казармы (оборонный квартал): 1 клетка (под зону-соседство казармы), ставится в
	# её зону-силуэт (с внутренней стороны) → +кап найма этой казармы (HIRE_CAP_PER_BARRACK, см.
	# PadBuilding.hire_cap_bonus). Категория DEFENSE. Несколько бараков складываются.
	PAD_BARRACK: {
		"name": "Барак",
		"menu_label": "⛺ Барак (+3 найм)",
		"hint": "Сапорт казармы: +3 к капу найма. Ставь в зону казармы.",
		"role": &"barrack",
		"cells": [Vector2i(0, 0)],
		"instant": true,
		"cost": {ResourcePile.ResourceType.BRONZE: 40, ResourcePile.ResourceType.SILVER: 2},
