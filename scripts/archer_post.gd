...
## Скорость стрелы. Выше → меньше время полёта → меньше «эффективный промах» по
## движущимся скелетам (lead'а нет, целимся в текущую позицию). 36 ≈ 1.3м/тик.
...
## 0.07 — меткий пост: кучно кладёт в цель (был 0.15, мазал мимо группы).
@export var inaccuracy_radius: float = 0.07
...
## Перезарядка между выстрелами (с). 0.5-0.8 ≈ 1.5 выстр/с — пост реально
## огрызается по группе (был 1.2-2.0 ≈ 0.6/с — почти безобидный для волны).
