...
## Стоимость каста в мане Tower'а. При max_mana=100 — ~3 каста подряд, потом
## мана дорогая (реген 1.5/с ≈ 20с за каст; топливо — мана-орбы со скелетов,
## ~6 орбов на каст). Если маны не хватает — каст отменяется (cooldown не
## запускается, скорострел не «съедает» попытку).
