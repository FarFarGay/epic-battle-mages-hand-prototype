...
## Курсы ДЕСЯТИЧНЫЕ (пивот 2026-07-07, «доллары и центы»): 1🥈 = 10🥉, 1🥇 = 10🥈 →
## 🥇 = 100🥉 — разряды казны и цен сравниваются поразрядно. Бронза с добычи сама
## «копится» в серебро/золото (визуально), ручная чеканка не нужна. Один на сцену.
## См. [[project_ebm_coin_economy]].
##
...
const BRONZE_PER_GOLD := BRONZE_PER_SILVER * SILVER_PER_GOLD  # 100
...
## Сколько монет номинала ПОКАЗАТЬ (одометр): золото = value/100, остаток → серебро, остаток → бронза.
