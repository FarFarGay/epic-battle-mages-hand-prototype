...
## казарма — соседство её футпринта (число клеток переменное → плитки добираются по требованию).
## Не продюсер в руке → прячем. Клетка непригодна (за падом/на замке/занята) → красная, иначе голубая.
