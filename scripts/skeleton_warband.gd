...
## Плотность блоба: радиус = √count × это. 0.85 → 100 скелетов в блобе ~8.5м
## радиуса (горда). Больше — реже строй, меньше — плотнее.
@export var formation_spacing: float = 0.85
...
func setup(scene: PackedScene, origin: Vector3, count: int = -1) -> void:
...
## Переключить режим. ASSAULT: снимаем роум, ставим forced_target (ядро/башня) →
## штурм напрямую. ROAM: возвращаем личные якоря.
