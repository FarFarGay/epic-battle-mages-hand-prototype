...
##
## Баланс 2026-05-15: 1.5 → 0.4м. На skeleton.capsule_radius≈0.4м прежний
## новичок мазал 90%+ даже в упор (3.75м разброс в патруле). Сейчас:
##  - 0 выстрелов формация: 0.4м (≈64% попадание в упор)
##  - 0 выстрелов патруль: 0.72м (≈31%, заметно хуже но не «слепой»)
##  - 100 выстрелов формация: 0.2м (почти 100%)
...
##  - 0 выстрелов: 0.4м (новичок)
##  - 100: 0.2м (середина)
##  - 500: 0.067м (ветеран — почти всегда в цель)
...
## 1.6× — заметная просадка точности, стимул выстраивать формации, но
## защитник не «слепой» в патруле. На base 0.4м это даёт 0.64м wandering
## vs 0.4м formation (~39% попадания в упор vs 64%).
## История: 2.5 → 1.8 (2026-05-15, первый проход) → 1.6 (тюнинг тем же днём,
## дозум по запросу «добавим к свободному процентов 10»).
const WANDERING_INACCURACY_MULT: float = 1.6
...
## 2026-05-15: 0.6 → 0.85 — прежний 60%-срез делал патруль почти бесполезным
## против дальних целей; теперь срез только 15%, патруль всё ещё хуже
## формации (22.5×0.85=19м vs 22.5м), но реагирует адекватно.
const WANDERING_RANGE_MULT: float = 0.85


## True если защитник сейчас в режиме «свободного патруля»: не назначен на
## DefenseMarker, не отвечает на bell, И не имеет персональной тревоги
## (`_alarm_target` от EventBus.skeleton_attacked_camp). В таком состоянии
## получает дебаф точности и дальности.
##
## Тревога снимает дебаф (баланс 2026-05-15): когда на лагерь напали, защитник
## «собирается» — стреляет с baseline-точностью, как из формации. Дизайн: в
## мирном патруле он расслаблен, но при атаке должен быть нормальной защитой,
## не «слепцом с короткой рукой».
func is_wandering() -> bool:
	if is_in_formation_slot():
		return false
	if _is_responding_to_bell():
		return false
	# Alarm-цель есть и валидна — защитник в боевом режиме, дебаф снят.
	# Проверка через _resolve_alarm_target() здесь не нужна: даже «протухший»
	# alarm означает что недавно был инцидент, защитник всё ещё насторожен.
	# Сама очистка _alarm_target по таймеру делается в _resolve_alarm_target()
	# на следующем боевом тике.
	if _alarm_target != null and is_instance_valid(_alarm_target) \
			and Time.get_ticks_msec() <= _alarm_until_msec:
		return false
	return true
