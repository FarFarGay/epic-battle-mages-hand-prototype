...
func _physics_process(delta: float) -> void:
	if not is_on_floor():
		...
		# Пол гасит ПАДЕНИЕ, но не подброс (2026-08-08). Раньше здесь стояло
		# безусловное velocity.y = 0, и любой импульс вверх умирал в том же
		# кадре, в котором его выдали: строка выполняется ДО move_and_slide,
		# пока is_on_floor ещё true. Из-за этого молча не работали ВСЕ вертикали
		# в коде — гигант трижды швыряет скелетов `+ Vector3.UP * 2..3`
		# (skeleton_giant.gd), пинок аппарата — `+ Vector3.UP * 0.5`. Выглядело
		# как плоский толчок, хотя автор писал подброс.
		#
		# Взрывы этим НЕ затронуты: AoeDamage сам зануляет dir.y (радиальный
		# push горизонтален по замыслу) — врагов от гранат в воздух не унесёт.
		velocity.y = 0.0
