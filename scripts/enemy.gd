...
func _physics_process(delta: float) -> void:
	if not is_on_floor():
		...
		# ⚠ ИЗВЕСТНЫЙ БАГ, НЕ ТРОГАТЬ БЕЗ РЕШЕНИЯ (замерено 2026-08-08). Обнуление
		# безусловное, а строка выполняется ДО move_and_slide, пока is_on_floor
		# ещё true — поэтому ЛЮБОЙ импульс вверх умирает в том же кадре, в котором
		# его выдали. Молча не работают все вертикали в коде: гигант трижды швыряет
		# скелетов `+ Vector3.UP * 2..3` (skeleton_giant.gd), пинок аппарата —
		# `+ Vector3.UP * 0.5`. Автор писал подброс, на экране плоский толчок.
		#
		# Лечится одной строкой — `elif velocity.y <= 0.0` (пол гасит только
		# падение); взрывы не затронет, AoeDamage сам зануляет dir.y. НО это
		# меняет чувство уже оттюненного гиганта, поэтому ждёт отдельного решения
		# и плейтеста, а не едет прицепом к чужой задаче.
		velocity.y = 0.0
