...
@export_group("Mount")
## Доп. вертикальный подъём модуля над точкой слота при монтаже — обычно
## полувысота меша, чтобы низ модуля стоял на земле. Слот добавляет это к
## своему module_offset. Свободный модуль садится физикой сам; это значение
## используется ТОЛЬКО для mounted-позиционирования. 0 = старое поведение
## (вертикаль задаёт целиком module_offset слота, как у турели на центр-слоте).
@export var mount_lift: float = 0.0
