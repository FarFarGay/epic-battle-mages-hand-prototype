...
## HP гиганта в финале.
##
## ⚖ 450 → 250 (2026-08-13, фидбек «гиганты в данже всё ещё колупаются»). Ручка
## перекрывает каталог, и её значение протухло дважды: 450 ставили как СМЯГЧЕНИЕ
## прежних каталожных 750, а каталог с пивотом роли уехал на 250 — смягчение
## превратилось в утяжеление, гигант интро стал крепче, чем в большом мире, где
## башня прокачана.
##
## ⚠ В ИНТРО НЕТ ЗАРЯЖЕННОГО РЫВКА: по умолчанию открыта одна Искра
## (SPELL_CATALOG.unlocked_by_default), а у неё нет dash_form. Игрок бьёт голым
## супер-рывком на 130 — при 450 это ЧЕТЫРЕ рывка на каждого, восемь на двоих.
## На 250 выходит два на каждого: та же вилка «1-2», что в большом мире.
## Хочешь убийство с одного — ставь ≤ 130, ручка для того и оставлена.
@export var finale_giant_hp: float = 250.0
...
## ⚠ МЁРТВАЯ РУЧКА (проверено грепом 2026-08-13): её читает только
## _intro_dash_hit, а того НИКТО не зовёт с тех пор, как посадка отдала Ладью
## штатному tower.gd целиком (2026-08-07) — таран считает башня своими
## dash_damage / super_dash_damage. Крутить это число бесполезно: оно ни на что
## не влияет. Оставлено как есть, чтобы не резать заодно живой код рядом;
## удалять — вместе с _intro_dash_hit.
...
## ⛔ Кап отряда числом ВЫПИЛЕН (2026-08-06): состав = три слота-кнопки
## (по группе на кнопку), найм гейтится занятостью слота ПКМ.
...
## Слот-кнопка-сбор: кучки-стойки групп классов, три пятака-слота и группа
## в руке. _roster_stacks: cls → {pos, queue: Array[int] (alive снятых/свежих
## групп), bodies}. _slots: [{id, pos, cls, alive, bodies, ring}] — слот =
## кнопка, состав считается по слотам.
...
## Группа в руке: cls (&"" = руки пусты), живых в группе, тела кучкой.
...
## СБОР = СЛОТ-КНОПКА (финал 2026-08-06 после двух отвергнутых итераций —
## ряды-зоны и полимино: «расстановка нужна, но порядок не важен»). В строю
## ТРИ больших пятака-слота, слот = КНОПКА отряда: [ЛКМ] лучники / [ПРОБЕЛ]
## копейщики / [ПКМ] спецы (артель ИЛИ огневики). Группу кидаешь «в кучу» —
...
func _build_slots(c: Vector3) -> void:
	_slots.clear()
	for sd in ROSTER_SLOTS:
		var pos: Vector3 = c + (sd["off"] as Vector3)
		var ring := AoeVisual.spawn_ground_ring(self, pos, 2.4, 0.0, SLOT_FREE)
		if ring != null:
			_roster_props.append(ring)
		var lbl := Label3D.new()
		lbl.text = sd["label"]
	...
	_roster_room.add_child(title)
	title.global_position = Vector3(c.x, 0.05, c.z - 6.4)


func _slot_by_id(id: StringName) -> Dictionary:
	for s in _slots:
		if s["id"] == id:
			return s
	return {}
...
## Кучка = гномы КУЧЕЙ (кружком, сумма живых по очереди групп, кап 8) —
## порядок не важен, никакой формы. Дёшево: статуи без теней и коллизий.
...
## Курсор в зоне строя (рядом с любым слотом)?
func _near_slots(p: Vector3) -> bool:
	for s in _slots:
		var sp: Vector3 = s["pos"]
		if Vector2(p.x - sp.x, p.z - sp.z).length() <= 4.5:
			return true
	return false


## Тик сбора ГРУППАМИ, порядок не важен. Клик по кучке — группа в руку (толпой
## над курсором), клик по строю — группа САМА ложится в слот своей кнопки
## (чужой класс в слоте выпрыгивает в кучку), клик мимо — назад в кучку.
## ПКМ по слоту — снять группу.
func _tick_roster(_delta: float) -> void:
	...
	if _drag_class != &"":
		for i in range(_drag_bodies.size()):
			...
			if is_instance_valid(b):
				var ang: float = TAU * float(i) / float(maxi(_drag_bodies.size(), 1))
				b.global_position = Vector3(cursor.x + cos(ang) * 0.8, 1.1,
						cursor.z + sin(ang) * 0.8)
...
## 1/2/3/4 — поставить группу класса в её слот-кнопку, Shift+цифра — снять.
## Запасной путь к тому же, что делает рука.
...
## уже стоят в слотах своих кнопок. fire_stock — покупных ГРУПП огневиков в
## кучке (покупка при занятом ПКМ); 0 = «Перенастроить отряд». Жест как при
## сборе: ЛКМ кучка→строй (слот сам), ПКМ по слоту — снять, ENTER — применить.
func _open_swap_roster(fire_stock: int = 0) -> void:
	...
	if fire_stock > 0:
		EventBus.tutorial_hint.emit(
				"Кнопка ПКМ занята! Кинь огневиков в строй — артель уступит место. ENTER — готово", 7.0)
	else:
		EventBus.tutorial_hint.emit(
				"Перестройка: ЛКМ кучка → строй (кнопка сама), ПКМ по слоту — снять, ENTER — готово", 6.0)
...
func _apply_kind(sk: Node3D, kind: StringName) -> void:
	match kind:
		...
		&"bomber":
			# ⭐ ВЗРЫВАЮЩИЙСЯ (2026-08-08): он не «рвётся от смерти», он БОМБА С
			# ФИТИЛЁМ. Дошёл до гнома → запустил обратный отсчёт (заряд наливается
			# красным и мигает всё чаще) → рванул. Застрелил ВО ВРЕМЯ отсчёта —
			# рвётся на месте: фитиль уже горит, отменить нечем.
			#
			# А вот снятый ДО отсчёта не взрывается вовсе — в этом вся тактика:
			# успел заметить и снять на подходе → чисто; проморгал → выбирай, где
			# он рванёт. Логика тика — _tick_bombers.
			sk.set(&"hp", 26.0)
			sk.set(&"move_speed", 2.8)
			sk.set(&"attack_damage_min", 4.0)
			sk.set(&"attack_damage_max", 6.0)
			sk.set_meta(&"bomber", true)
			# Заряд инертно-тёмный: КРАСНЫМ он станет только когда фитиль зажжён
			# (один визуальный язык — красное мигание значит ровно «сейчас рванёт»,
			# и ничего больше). Оранжевый гребень остаётся приметой бегуна.
			var charge := _add_kind_decor(sk, Vector3(0.62, 0.62, 0.62),
					Vector3(0.0, 1.05, 0.42), BOMB_IDLE_COLOR)
...
const ROOM_PATTERN := {
	...
	2: [&"grunt", &"shieldman", &"grunt", &"runner"],
	3: [&"grunt", &"runner", &"grunt", &"bomber"],
}
var _room_cursor: Dictionary = {}

## ⭐ БОМБИСТ — БОМБА С ФИТИЛЁМ (2026-08-08). Подошёл к гному ближе BOMB_ARM_DIST →
## зажёгся фитиль: заряд наливается красным и мигает всё чаще, через BOMB_FUSE
## секунд — взрыв. Убежать от собственного фитиля он не может: зажёгся — горит.
##
## Убит ВО ВРЕМЯ отсчёта — рвётся на месте (фитиль уже горит). Убит ДО отсчёта —
## не взрывается вовсе. В этом вся тактика: снял на подходе — чисто, проморгал —
## выбираешь только место, где рванёт.
const BOMB_ARM_DIST := 3.2
const BOMB_FUSE := 1.6
## Мигание: от медленного к частому по мере догорания (Гц в начале и в конце).
const BOMB_BLINK_START := 3.0
const BOMB_BLINK_END := 14.0
const BOMB_IDLE_COLOR := Color(0.24, 0.2, 0.18)
const BOMB_LIT_COLOR := Color(1.0, 0.12, 0.06)
## Взрыв: бьёт ВСЕХ в радиусе — и гномов, и своих (симметрия: сила, которая бьёт
## одних, бьёт и других; толпа скелетов вокруг бомбиста — твой шанс).
...
## ФИТИЛЬ БОМБИСТОВ: подошёл к гному — зажёгся, дальше только тикает. Своей
## ИИ-ветки у бомбиста нет: параметризованный скелет плюс этот тик.
func _tick_bombers(delta: float) -> void:
	for n in get_tree().get_nodes_in_group(Skeleton.SKELETON_GROUP):
		if not is_instance_valid(n) or (n as Node).is_queued_for_deletion():
			continue
		var sk := n as Node3D
		if not sk.has_meta(&"bomber"):
			continue
		if not sk.has_meta(&"fuse"):
			# Ещё не зажёгся: ищем повод. Дистанция до БЛИЖАЙШЕГО гнома — тот же
			# критерий, что игрок видит глазами («подошёл вплотную»).
			var g: Node3D = _nearest_alive_gnome(sk.global_position)
			if g == null:
				continue
			var to := g.global_position - sk.global_position
			to.y = 0.0
			if to.length() > BOMB_ARM_DIST:
				continue
			sk.set_meta(&"fuse", BOMB_FUSE)
			EventBus.camera_shake.emit(0.06, sk.global_position)
			continue
		var t: float = float(sk.get_meta(&"fuse")) - delta
		if t <= 0.0:
			# Догорел: убиваем ШТАТНЫМ уроном, а не своим взрывом напрямую —
			# тогда и «догорел», и «застрелили с горящим фитилём» приходят
			# в _on_enemy_destroyed_bomb одной дорогой, без второй ветки.
			sk.set_meta(&"fuse", 0.0)
			if sk.has_method(&"take_damage"):
				sk.call(&"take_damage", 99999.0)
			continue
		sk.set_meta(&"fuse", t)
		_bomb_blink(sk, t)


## Мигание заряда: чем меньше осталось, тем чаще. Красим ТОЛЬКО заряд —
## hit-flash и shatter самого скелета остаются штатными.
func _bomb_blink(sk: Node3D, left: float) -> void:
	var charge := sk.get_node_or_null(^"BombCharge") as MeshInstance3D
	if charge == null:
		return
	var mat := charge.material_override as StandardMaterial3D
	if mat == null:
		return
	var k: float = clampf(1.0 - left / BOMB_FUSE, 0.0, 1.0)
	var hz: float = lerpf(BOMB_BLINK_START, BOMB_BLINK_END, k)
	# Фаза копится в мете: считать от глобального времени нельзя — все бомбисты
	# мигали бы в такт, как гирлянда, вместо личных фитилей.
	var phase: float = float(sk.get_meta(&"fuse_phase", 0.0)) \
			+ hz * get_physics_process_delta_time()
	sk.set_meta(&"fuse_phase", phase)
	var on: bool = fmod(phase, 1.0) < 0.5
	mat.albedo_color = BOMB_LIT_COLOR if on else BOMB_IDLE_COLOR
	# Раздувается к концу — читается и боковым зрением, не только по цвету.
	charge.scale = Vector3.ONE * (1.0 + 0.5 * k + (0.18 if on else 0.0))


## Ближайший живой гном к точке (бомбист поджигает фитиль от того, кто реально
## рядом, а не от «случайного из отряда»).
...
func _on_enemy_destroyed_bomb(enemy: Node3D) -> void:
	if enemy == null or not is_instance_valid(enemy) or not enemy.has_meta(&"bomber"):
		return
	# ⭐ РВЁТСЯ ТОЛЬКО С ЗАЖЖЁННЫМ ФИТИЛЁМ. Снятый на подходе — просто труп: иначе
	# «убей его заранее» перестало бы быть решением, и разницы между расстрелять
	# издали и пропустить вплотную не было бы никакой.
	if not enemy.has_meta(&"fuse"):
		return
...
func _intro_hire_fire() -> void:
	if _alive_fire_mages() >= 2:
		...
		return
	# Гейт прямого найма: слот ПКМ занят артелью → экран замены (там огневики
	# в кучке, кинул в строй — артель выпрыгнула). Слот свободен — пара сразу.
	if _alive_artel() > 0:
		_open_swap_roster(1 if _alive_fire_mages() <= 0 else 0)
		return
