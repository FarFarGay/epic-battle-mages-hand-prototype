...
static func play(b: Node3D, radius: float) -> void:
	if b == null or not is_instance_valid(b):
		return
	var ground_y: float = b.global_position.y
	b.global_position.y += DROP_HEIGHT
	var tw := b.create_tween()
	tw.tween_property(b, "global_position:y", ground_y, DROP_TIME) \
		.set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
	tw.tween_callback(_land.bind(b, b.scale, radius))


static func _land(b: Node3D, base_scale: Vector3, radius: float) -> void:
	if b == null or not is_instance_valid(b) or not b.is_inside_tree():
		return
	# Сквош вокруг подошвы (origin зданий на земле) и упругое восстановление.
	b.scale = base_scale * SQUASH
	var tw := b.create_tween()
	tw.tween_property(b, "scale", base_scale, SQUASH_TIME) \
