...
## НАСЕЛЕНИЕ = АРТЕЛЬ (автолоад Population, пивот 2026-07-12): здание с pop_demand > 0
## забирает РЕАЛЬНОГО гнома-рабочего — тот приходит и живёт внутри (смена); без гнома
## здание простаивает. Дом слоты больше НЕ даёт (пула нет) — он ось «Объём» у шахты.
const MINE_POP_DEMAND := 1  # работающей шахте на жиле нужно столько гномов (иначе простой)
...
func _open_hire() -> void:
	...
	# СВОБОДНЫЕ ГНОМЫ АРТЕЛИ), иначе торг гейтил бы «Артель полна» без учёта наличия гномов.
...
## • АРТЕЛЬ (глобально): солдат = бывший рабочий, свободные гномы — есть ли кого «переодеть».
## cap = min(гарнизон, живые + свободные гномы). Уперлись → торг гасит найм (строй барак ИЛИ найми рабочих).
...
## Сколько гномов-рабочих требует это PRODUCTION-здание (смена). Шахта на жиле —
## MINE_POP_DEMAND; плавильня/двор — 1 (рабочее место). Прочее — 0. Population назначает
## реальных свободных гномов артели; без пришедшего гнома шахта простаивает, а
## плавильня/двор не дают свою ось (см. _quarter_status).
func pop_demand() -> int:
	match _role:
		&"mine":
...
func _refresh_magic_indicator() -> void:
	...
	if not staffed:
		last = "🚨 ТРЕВОГА — простой" if (Population != null and Population.alarm_active) else "⏸ Нет гнома на смене — простой"
...
func _refresh_mine_indicator() -> void:
	...
	if not staffed:
		last_row = "🚨 ТРЕВОГА — простой" if (Population != null and Population.alarm_active) else "⏸ Нет гнома на смене — простой"
	_apply_indicator_rows([
...
## Номинальный эффект роли на АРТЕЛЬ для подписи в палитре (иконка 👥±N): PRODUCTION БЕРЁТ
## реального гнома на смену (−). Дом слоты больше не даёт (население = артель, 2026-07-12) —
## 0, как барак (тот — ёмкость казармы, см. garrison_for_role).
