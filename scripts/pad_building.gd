...
## Оборонный квартал: КАЗАРМА (продюсер) + БАРАКИ (сапорт в её зоне-соседстве) → +кап найма.
const HIRE_CAP_PER_BARRACK := 3  # каждый барак (1 клетка) в зоне казармы поднимает её кап найма на столько
...
func _build_barrack() -> void:
	var s: float = CityGrid.CELL
	var stone := _solid(_WALL_STONE, 0.0, 0.85)
	var dark := _solid(_STONE_DARK, 0.0, 0.9)
	var wood := _solid(_WOOD, 0.0, 0.8)
	_solid_shape(_BASE_H * 0.5, _BASE_H, dark, _STREET)              # цоколь-фундамент
	var bh := 1.05
	_solid_shape(_BASE_H + bh * 0.5, bh, stone, _STREET + 0.06)      # каменное тело (ниже стены)
	# Дверь (тёмное дерево) на фронте (+Z)
	_box(Vector3(0.42, 0.66, 0.08), Vector3(0.0, _BASE_H + 0.33, s * 0.5 - _STREET - 0.06),
		_solid(_WOOD_DARK, 0.0, 0.8), true)
	# Лин-ту крыша — наклонный деревянный скат поверх тела (пристройка прислонена к стене)
	var roof := MeshInstance3D.new()
	var bm := BoxMesh.new()
	bm.size = Vector3(s - 2.0 * _STREET + 0.18, 0.1, s - 2.0 * _STREET + 0.34)
	roof.mesh = bm
	roof.material_override = wood
	roof.position = Vector3(0.0, _BASE_H + bh + 0.14, 0.0)
	roof.rotation.x = deg_to_rad(-18.0)
	add_child(roof)
...
func _open_hire() -> void:
	...
	# count_fn — счёт живых ИМЕННО этой казармы → гейт per-barracks. cap_fn — эффективный кап (база
	# типа + бараки квартала), иначе торг гейтил бы «Артель полна» по базе, игнорируя бараки.
...
## Эффективный кап найма ЭТОЙ казармы для гейта торга = база типа + бараки квартала (hire_cap_bonus).
func _my_hire_cap() -> int:
	var stype: StringName = RoomBuildings.get_data(building_id).get("squad_type", &"archer_squad")
	var base: int = int(SoldierSystem.get_squad_cap(stype)) if SoldierSystem != null else 0
	return base + hire_cap_bonus()


## Оплата прошла: заказываем у спавнера want юнитов типа у казармы. Лучники (corner_tower)
## Прибавка к капу найма ЭТОЙ казармы от бараков в её зоне-соседстве (оборонный квартал): каждый
## барак = +HIRE_CAP_PER_BARRACK. Зовётся спавнером (GnomeSquadSpawner) для эффективного капа.
...
func _quarter_status() -> Dictionary:
	...
	if capacity == 0 or support_roles.is_empty():
		return {"roles": {}, "fill": 0.0, "covered": {}, "support_count": 0}
	...
	for b in get_tree().get_nodes_in_group(GROUP):
		...
		if in_plot:
			roles[r] = true
			count += 1
	return {"roles": roles, "fill": float(covered.size()) / float(capacity), "covered": covered, "support_count": count}
...
func _refresh_mine_indicator() -> void:
	...
	_apply_indicator_rows([
		"🔥 Скорость   %s" % ("×%s" % MINE_SPEED_MULT if speed_on else "—  (плавильня)"),
		"🪙 Номинал    %s" % mint_val,
		"🏠 Объём       %s" % ("×%d" % MINE_VOLUME_MULT if vol_on else "—  (дом гномов)"),
		"≈ %s %s/сек" % [String.num(rate, 1), _coin_emoji(coin)],
	])
	if _ind_rows.size() >= 4 and is_instance_valid(_ind_rows[3]):
		(_ind_rows[3] as Label).add_theme_color_override(&"font_color", Color(1.0, 0.9, 0.4))


## Строки плашки КАЗАРМЫ: «Гарнизон живых/кап» + разбивка капа (база + бараки квартала).
func _refresh_barracks_indicator() -> void:
	...
	var base: int = cap - bonus
	_apply_indicator_rows([
		"🛡 Гарнизон   %d / %d" % [living, cap],
		"⛺ Кап: %d база + %d бараки" % [base, bonus],
	])
