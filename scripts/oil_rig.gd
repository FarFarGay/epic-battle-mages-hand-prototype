...
## её по ТРУБОПРОВОДУ в [Castle]: коллектор сам считает связность по портам и
...
## при установке (MMB) — патрубок целишь в сторону трассы. ДЛИНА КРАТНА КЛЕТКЕ (3.0 =
## 1.5 клетки): при центре бура в клетке порт ложится на УЗЕЛ сетки → труба стыкуется
## встык (как у коллектора, inlet_dist=3.0 — единообразно). Менять только кратно 1м.
const OUTLET_LOCAL := Vector3(-3.0, 0.0, 0.0)
...
## Подключить/отключить коллектор-приёмник (зовёт Castle при пересчёте сети,
