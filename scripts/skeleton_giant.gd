...
## ⭐ РЁВ-ПОДАЧА (баннер «босс» + шейк + аггр комнаты) — ТОЛЬКО У ПЕРВОГО гиганта
## группы. С 2026-08-13 гигант перестал быть боссом волны и приходит ТРОЙКОЙ: это
## «простой враг для башни», которого весело щёлкать, а не событие. Три баннера
## подряд превратили бы подачу в шум и обесценили её к тому моменту, когда
## объявляться будет мех. Ставит спавнер (WaveDirector): первому true, остальным
## false. Одиночный гигант, поставленный в сцене руками, ревёт как раньше.
@export var boss_intro: bool = true
## Множитель входящего урона, пока гигант НЕ оглушён. 1.0 = гигант уязвим ВСЕГДА
## (фидбек 2026-07-06: стан — удобное окно, не единственное).
##
## ⚖ HP 750 → 250 (2026-08-13, фидбек «много ковырять — душно»). Гигант стоил
## 6 обычных супер-рывков и был ВДВОЕ крепче меха-стража (400) — элитный моб
## прочнее финального босса, отсюда и ощущение. Теперь ровно то, что просили:
## заряженный супер-рывок (250) кладёт с ОДНОГО, незаряженный (130) — с ДВУХ.
## Числа самих рывков не трогали: их лестницу (100 → 130 → 200-250) настраивали
## отдельным фидбеком, и она про «смысл апгрейдиться», а не про этого моба.
## Ручка на будущее: <1.0 вернёт «броню активного».
...
func _ai_step(delta: float) -> void:
	...
	if not _roared and _cached_target != null and is_instance_valid(_cached_target) \
		...
		if boss_intro:
			...
			_roared = true  # молча: подача занята первым в группе
