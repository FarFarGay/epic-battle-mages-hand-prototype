...
## ⭐ ПРОИЗВОДСТВО ОТВЯЗАНО ОТ ГНОМОВ (2026-08-11, решение юзера: «никто не
## работает на шахте и в плавильне, переделываем дизайн немного»).
##
## Гном больше не рабочая единица, которую куда-то ОТПРАВЛЯЮТ, — он экипаж
## Ладьи. Поэтому смен нет: построенное здание работает само, по факту
## постройки. Возвращаем true всегда.
##
## Правку держим ЗДЕСЬ, в одной точке, а не размазываем по девяти обращениям
## в pad_building: так «простой без гнома» исчезает разом во всех потребителях
## (тик шахты, тик плавильни, ось сапорта, обе панели статуса), и вернуть
## смены обратно можно одной строкой.
##
## ⚠ Учёт живых гномов (cap/free_slots) НЕ трогаем: его читает найм солдат в
## казарме, а военная экономика — отдельный вопрос, он висит в DESIGN §9.3.7.
func is_staffed(_producer: Node) -> bool:
	return true
