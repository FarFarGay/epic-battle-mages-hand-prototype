...
- **ПЛАШКА-МОСТ** (`bridge_plank.gd`, узел `Room5/BridgePlank` в `level_rooms.tscn`; пивот 2026-07-03 — «больше ручного взаимодействия»). RigidBody3D-доска (11×0.35×4.6, две поперечины снизу) лежит у пропасти. **Grabbable** (подсветка-кандидат `set_highlighted` по [[feedback_grabbable_highlight]]) + `hand_soft_release` («ставится, не бросается»). Рука подняла и отпустила **над полосой пропасти** (X в пределах стены±полдоски, Z в полосе барьера ± `snap_margin=3`) → **защёлк**: tween 0.16с на место поперёк пропасти (X = центр стены, Z clamp в полосу), пыль по обоим концам + `camera_shake 0.3`.
