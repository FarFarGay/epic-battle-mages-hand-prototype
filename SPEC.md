...
- Биты: движение (тоннель) → дэш (одиночки) → мана (орбы) → супер-дэш при ≥50 маны → красная дверь = выход из пещеры.
...
**Актуальные (акт II «Долина», 2026-07-07):**

- **Замок потерян = мягкий софтлок.** Замок уязвим и взрывается (§5.27.5), но восстановить его нельзя (чертёж и фундамент one-shot), поражения-оверлея нет — штурмы просто прекращаются. Дизайн-развилка за геймдизайнером: смерть замка = поражение уровня ИЛИ руина с перезакладкой.
- **Усиление заставы — одобрено, не сделано:** волны на шум работы у станка (паттерн GroveGate), «печать под осадой» (~15с обороны станка после пускача; станку нужен HP — контракт зданий готов), электро-дверь-тизер глубины (вход к «Звёздному кристаллу», фаза Врат).
- **Хаб (§5.27):** сделаны фазы ①② + Подземелье гномов v1 (§5.27.7, пивот: чертёж-гейт выпилен, завалы разбираются рукой). Осталось: комната А «Разлом» (СЗ, экзамен руки: элемент «Ключ-плита»), глубина заставы («Звёздный кристалл» за электро-дверью), пробуждение меха на `GateRuin._on_awakened` + открытие створ. Замок ур.2 (квартал-соседство) — под вопросом после пивота: больше ничего не гейтит.
