...
- **ПЛАШКА-МОСТ** (`bridge_plank.gd`, узел `Room5/BridgePlank` в `level_rooms.tscn`; пивот 2026-07-03 — «больше ручного взаимодействия»). RigidBody3D-доска (11×0.35×4.6, две поперечины снизу) лежит у пропасти. **Grabbable** (подсветка-кандидат `set_highlighted` по [[feedback_grabbable_highlight]]) + **`hand_haul` — ВОЛОЧЕНИЕ** (2026-07-03): рука НЕ морозит доску, а тянет пружиной-демпфером за **точку хвата** (проекция позиции руки на короб коллайдера при грабе — взялся за конец → тянешь за конец, доска разворачивается рычагом, дальний конец скребёт землю). Реализация `HandPhysicalActions._apply_haul_force` (в `_physics_process`; экспорты `haul_stiffness=40 / haul_damping=7 / haul_max_accel=60 / haul_angular_damp=2`); обычный grab (freeze+телепорт) не тронут — haul только для `Layers.HAND_HAUL_GROUP`. Release без импульса броска — доска отцепляется с естественным моментом. Рука подняла и отпустила **над полосой пропасти** (X в пределах стены±полдоски, Z в полосе барьера ± `snap_margin=3`) → **защёлк**: tween 0.16с на место поперёк пропасти (X = центр стены, Z clamp в полосу), пыль по обоим концам + `camera_shake 0.3`.
