...
**Пивот экономики (дизайнер):** дерево/камень-ресурс под стройку заменяются на **монеты трёх номиналов** — 🥉 бронза / 🥈 серебро / 🥇 золото. Цена постройки СОСТАВНАЯ (напр. замок = 2🥇 4🥈 20🥉), а не единое число. Чеканка вверх (100🥉→1🥈, 100🥈→1🥇) — **вручную у плавильни** (ШАГ 4, ещё нет). Добыча: бронза сыпется со всего + плавильня руда→слиток (ШАГИ 2-3, ещё нет). См. [[project_ebm_coin_economy]].
...
**PENDING:** ШАГ 1b (build-site постройки на монеты, выпил wood-стройки); ШАГ 2 (бронза-дроп со всего + сбор); ШАГ 3 (плавильня: руда→слиток→монеты); ШАГ 4 (ручная чеканка 100→1 у плавильни). Баланс цен — плейсхолдер. Найм отрядов пока на GOLD (`trade_ui`) — возможно перевести на составную цену позже.
