...
Под новые постройки — добавлять пункты сюда. Сейчас тут также **Бур** и **Трубы** (прямая/угол/крест) нефтесети (§5.22.8), все за гейтом `building_unlocked`.
...
**OilGrid** (`oil_grid.gd`, RefCounted, static) — ЕДИНАЯ grid-математика, один источник для снапа/построек. `CELL=2`, якорь = центр коллектора-замка (`anchor(tree)` = группа `oil_collector`). `world_to_cell`/`cell_to_world`/`snap`/`rotate_offset` (поворот offset кратно 90°, сверен с `Basis(Y,θ)`: +90°→(z,−x))/`building_cells`(центр+маска+поворот→мировые клетки). Площадка: `PAD_RADIUS=4` (пятно 9×9), ядро-качалка `PUMP_RADIUS=1` (центр 3×3 занят, строить нельзя); `in_pad`/`is_pump`.
...
#### 5.22.10 Казарма лучников: настоящий отряд + гарнизон стен (2026-06-24) (`pad_building.gd`, `archer_soldier.gd`, `gnome_squad_spawner.gd`)

**Казарма = НАСТОЯЩИЙ отряд, не статичные посты.** `PAD_BARRACKS` (L, `corner_tower`, синий стяг) при достройке заказывает у спавнера 3 `ArcherSoldier` (`_spawn_archers` → `spawner.request_squad("archer_squad", 3, ground)`). Это **полный паритет с наёмными лучниками даром из системы**: карточка отряда в HUD, hp=18/смерть/hit-flash/полоска, точность с накоплением опыта, конус зрения, та же стрела и статы, все команды («Идти сюда»/«За башней»/роспуск). Перекраска модели (box-визуал) — в `ArcherSoldier._apply_visual`. `PAD_SPEARMEN` (T, красный стяг) — без `corner_tower`, отряд не плодит (Фаза 2).
