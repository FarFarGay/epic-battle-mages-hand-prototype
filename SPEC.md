...
- **ЗОНА-СИЛУЭТ (плот)** = `MINE_PLOT_CELLS` (offset'ы от клетки шахты, дефолт 4×3 сбоку, 11 клеток), ПОВЁРНУТЫЕ на ориентацию шахты (`_plot_cells_ordered` → `CityGrid.rotate_offset` по `rotation.y`). Заполняешь зону сапортами — **обкладывать шахту со всех сторон НЕ нужно, нужно занять зону**. `_quarter_status`: `distinct` = РАЗНЫЕ роли сапортов, чьи клетки попали в зону; `fill` = доля клеток зоны, закрытых сапортами. Дубликат типа растит `fill`, но не `distinct`.
