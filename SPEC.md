...
- **ИНДИКАТОР ОСЕЙ при наведении** (`set_highlighted` ← `Hand._update_pickup_highlight`; шахта в `PICKUP_HIGHLIGHT_GROUP`): наводишь руку на шахту → плашка над ней (2D-панель StyleBox чёрный α + emoji-иконки 🔥/🪙/🏠 + монета номинала, в SubViewport → Sprite3D-билборд). Каждая ось: значение или `— (что построить)`; внизу итоговая добыча. SubViewport рендерит только пока наведено. `_build/_refresh/_set_quarter_indicator_visible`.
