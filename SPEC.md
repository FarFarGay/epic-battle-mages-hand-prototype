...
#### 5.26.6 Room5 = «первая работа под угрозой» (`grove_gate.gd`)
Урок командования артелью + первый росток корневого дайла «добыча притягивает угрозу»:
- Плашка моста **перенесена в рощу** (8, 0.5, 121) и заперта GroveGate'ом (`collision_layer 0` + `freeze` — рука её не видит и не подсвечивает).
- **`WoodSource` получил сигналы** `chopped(remaining)` / `depleted` (эмиты в `gnome_hit`/`_break`) — для сценариев комнат.
- **`GroveGate`** (узел Room5; дети-Marker3D `FlankNorth/South` = точки спавна): первый удар топора → хинт «шум разбудил скелетов» + волна `first_chop_wave=4`; каждое срубленное дерево → волна `tree_felled_wave=6` (спавн по флангам поочерёдно, forced-цель = башня, урон 3); вся роща срублена → плашка free (слой ITEMS + unfreeze + пыль) + хинт «мосток свободен». Все числа/тексты — экспорты.
- `SkeletonFiller6`: 26→10, только **дальний берег** (center (43,0,112)) — арьергард ждёт перехода по мосту.
- HintArtel на входе учит приказу: карточка артели → «Идти сюда» → клик по дереву = GATHER; зона добычи башни `gather_radius=16` — башня должна стоять возле рощи.
- **PENDING (обсуждено 2026-07-06, ждёт решения юзера):** динамика Room5 — «игрок = зритель»; предложены: камнеметатель на дальнем берегу (обстрел через пропасть, ответ = Искра/парирование), таран башни по дереву = несколько «ударов топора», волны целят РАБОЧИХ (игрок-телохранитель), `wood_remaining` 6→4.
