...
- **Секции-категории** (`BUILD_SECTIONS`, та же таксономия `PadBuilding.category`): ⛏ ДОБЫЧА (шахта/плавильня/двор) · 🛡 ОБОРОНА (стены/ворота/казармы) · 🏰 ЗАМОК·СОЦИУМ (качалка/дом) · 🌉 ИНЖЕНЕРИЯ (мост). Под заголовком — карточки в GridContainer.
...
- **ШАХТА** (`PAD_MINE`, role mine) на жиле (`OilDeposit`, 1 тип = бронза; гейт `_pad_valid`: на жилу только шахту, шахту только на жилу) сама капает деньги в казну: `rate = MINE_RATE(1.0) × (1 + N_РАЗНЫХ_типов_в_зоне × SUPPORT_BONUS(0.6) × доля_заполнения_зоны)`. Разнообразие И заполнение считаются ВМЕСТЕ (`_quarter_status`). Бронза → `GoldBank.add_coin` (одометр). +N всплывашка (реюз `SquadXpPopup`) и фейерверк на каждую новую золотую — над шахтой.
- **ЗОНА-СИЛУЭТ (плот)** = `MINE_PLOT_CELLS` (offset'ы от клетки шахты, дефолт 3×3 с шахтой в углу → 8 клеток зоны, ровно чеканный двор+плавильня+дом гномов), ПОВЁРНУТЫЕ на ориентацию шахты (`_plot_cells_ordered` → `CityGrid.rotate_offset` по `rotation.y`). Заполняешь зону сапортами — **обкладывать шахту со всех сторон НЕ нужно, нужно занять зону**. `_quarter_status`: `distinct` = РАЗНЫЕ роли сапортов, чьи клетки попали в зону; `fill` = доля клеток зоны, закрытых сапортами. Дубликат типа растит `fill`, но не `distinct`.
- **ЗОНА ПОД КВАРТАЛ РЕЗЕРВИРУЕТСЯ при установке шахты** (`hand_place_aim._mine_plot_clear` в `_pad_valid`): вся зона (повёрнутая на `_rot_y`) должна быть СВОБОДНА — каждая клетка в площадке, не на замке, не занята зданием. Иначе шахту не поставить (силуэт красный). В превью при установке непригодные клетки зоны — КРАСНЫЕ, свободные — голубые: крутишь средней кнопкой, пока вся зона голубая. Сначала столбишь шахту на чистом месте, потом застраиваешь зону.
- **ПРАВИЛО КАТЕГОРИЙ:** в зону шахты идут только PRODUCTION/SOCIAL (плавильня/двор/дом). Казарма/стена (DEFENSE) физически могут лежать в зоне, но НЕ считаются — это другой квартал. Силуэт = квартал в пределах своей категории (оборонный квартал и т.д. — pending).
