...
- **Гномы выходят свободными на установке.** `_start_deploy` ставит `CollectionMode.FREE` + `_assign_idle_life` → гномы высыпаются и бродят вокруг ядра (свободное состояние). «Работа» (C) → добыча, «Тревога» (V) → в башню. Свободные без работы тоже бродят у харвестера (это норма).
...
Выбор — **Журнал → вкладка «Лагерь»** → клик по зданию: `JournalPanel._on_build_pressed` (ветка `grid_building`) закрывает журнал и зовёт `Camp.spawn_building_into_hand(id)` → инстанс BuildBlock в руку (`Hand.hold_item`), сразу нужного размера. Дальше игрок прихлопывает в ячейку грида.
