...
**Назначение:** маркер обороной позиции. Игрок ставит точку на земле, защитники назначаются на слоты в линию перпендикулярно `facing_dir`, лицом в указанное направление. В формации защитник работает с baseline-статами; вне — wandering-дебаф (см. §5.8.1).
...
- `DEFAULT_SLOT_COUNT = 3` — дефолт когда HUD не передал значение.
- `@export slot_count: int = DEFAULT_SLOT_COUNT` — per-instance количество слотов. Игрок выставляет через HUD-счётчик `[−] Слотов: N / max [+]` в defender card. Clamp 1..defender_count.
- `@export slot_radius: float = 1.5` — расстояние между соседними слотами в линии.
- `@export slot_jitter_lateral: float = 0.3` / `slot_jitter_depth: float = 0.25` — «небрежность» расположения: детерминистичный per-slot offset, генерится в `setup()` после установки `facing_dir` (`_slot_offsets: Array[Vector3]`, world-space, кэш). Без кэша защитники бесконечно шли бы к слегка смещённой точке (RNG нестабильна между вызовами).
- `@export facing_dir: Vector3 = Vector3.FORWARD` — направление обстрела (XZ).

**API:**
- `setup(world_pos, dir_xz, count: int = DEFAULT_SLOT_COUNT)` — позиция + направление + желаемое N. Нормализует facing_dir, clamp count ≥ 1, генерит `_slot_offsets`, перевыставляет визуальный arrow.
- `get_slot_position(i: int) -> Vector3` — центрированное распределение для любого N: `offset = (i − (N−1)/2) × slot_radius` вдоль `right = facing × UP`. N=1 → [0]; N=2 → [−0.5r, +0.5r]; N=3 → [−r, 0, +r] (совпадает со старым layout'ом); N=5 → [−2r, −r, 0, r, 2r]. К базе прибавляется cached jitter.
...
**Multi-marker.** Camp хранит `_defense_markers: Array[DefenseMarker]`. Каждое нажатие «На защиту» создаёт новый маркер — игрок может строить несколько линий обороны (например, по сторонам света). Каждый забирает следующие N ближайших СВОБОДНЫХ защитников (N из счётчика в HUD). «Патруль» уничтожает все маркеры разом (`Camp.disband_all_defense_markers`).

**UX расстановки** (`HandBuildAim.start_defense_formation_aim(slot_count)` + `_commit_direction_aim`, гибрид click/drag, 2026-05-16):
- **Короткий клик** (drag-length < `DEFENSE_AUTO_FACING_DRAG_THRESHOLD = 0.5м`) → facing вычисляется автоматически наружу от центра лагеря (`facing = (origin − build_zone_center).normalized()`). Покрывает 90% случаев — круговая оборона.
- **Drag** (длиной ≥ 0.5м) → facing = направление drag'а. Edge case'ы: фланг, засада, ambush.
- Без UI-усложнения, оба режима в одном жесте. Опытный игрок открывает drag-override, новичок просто кликает. Threshold в 0.5м фильтрует случайные «дёрнул мышью при обычном клике».
...
#### 5.22.6 Вкладка «Стройка» в карточке отряда (`gameplay_hud.gd`)
Стройка не висит кнопкой на экране — она **вкладка карточки отряда рабочих**. Только у `soldier_type==&"worker"`: кнопка «🔨 Стройка» → `_open_build_menu(btn)` → PopupMenu (`_build_menu`, ленивое создание). Пункты:
- `BUILD_MENU_BRIDGE` «🌉 Мост через пропасть» → `hand.bridge_aim.start_aim()` (span-flow, §5.22.5). Не гейтится.
- `BUILD_MENU_WALL` «🧱 Стена» / `BUILD_MENU_WATCHTOWER` «🗼 Сторожевая башня» → свободный силуэт (`hand.place_aim.start_aim(RoomBuildings.WALL/WATCHTOWER)`, §5.22.7 каталог). **Гейт `_building_unlocked()`** — оба заблокированы (greyed «(знание гномов)») пока не запущен станок Room11 (PlayerProfile.building_unlocked, §5.22.7). Башня дополнительно требует отряд лучников (`_has_archer_squad`).

Под новые постройки — добавлять пункты сюда (модули нефтекачалки, §5.22.8 — тем же свободным силуэтом).
...
#### 5.22.8 Гномья нефтекачалка (Room8) — хребет геймлупа [якорь реализован, стадии 2-4 в дизайне] (2026-06-22) (`oil_rig.gd`, `oil_rig.tscn`)

**Дизайн-пивот.** Квест строителей перерос из разового пазла в **хребет геймлупа**. Центр — большая сломанная нефтекачалка (**OilRig**, мегабур) в Room8 у поселения. **Накопить N гномьей нефти = победа матча** (заменит/дополнит 1000 золота/ключ, §5.15). Станок Room11 (§5.22.7) — первое звено цепи (учёба строить), не вендинг.

**Цепочка стадий** (каждая учит свой глагол): 1) лицензия строителя (станок Room11 — РЕАЛИЗОВАНО) → 2) отстрой узлы/модули → 3) запуск насоса → 4) защити под штурмом.

- **Визуал:** компактный мегабур (~2.6м) — корпус + раскалённое ядро-кольцо (emission) + бур-бит, вгрызается в нефтяную жилу (диск-пятно). В `PUMPING` бит крутится (`drill_spin_speed`). Заменил первый громоздкий деррик (забракован геймдизайнером: «чёрный сарай»).
- **OilRig** (`StaticBody3D`, группа `oil_rig`, layer 544): FSM `BROKEN→RESTORING→PUMPING`; API `begin_restore()`/`restore_module()`/`ignite()`/`restore_unlocked()` (гейт = `building_unlocked`); в `PUMPING` `oil_per_sec` тикает к `oil_goal`. Победа/штурм пока НЕ подключены (стадии 3-4) — чтобы не ломать текущее условие матча. Вырезан из спавна `SkeletonFillerR8` (`exclude` 4.5×4.5).
- **Модель построения модулей — ПУТЬ A** (выбран после отказа от слотов/секторов): модули = обычные постройки комнатного режима (как стены — через `RoomBuildings` + `HandPlaceAim` + `RoomBuildSite`), регистрируются на бур в его зоне. НЕ возврат старого `build_grid`/колец. Модули «на два фронта» (добыча И оборона): **насос** (+добыча, шум→враги), **турель** (оборона), **охладитель** (анти-перегрев), **броня** (+HP/−добыча). Первый насос = момент запуска бура. Раскладка = баланс добыча/защита. *[НЕ реализовано — следующий инкремент]*
...
- **7 рабочих живут в башне с самого старта** (`_spawn_starting_workers` — спавн у башни + сразу прятка). Каталог `worker`: `squad_cap=7` (helper `get_squad_cap`); докупка у домика **доливает до капа** (`_clamp_to_cap`), выше — нельзя. На капе `gnome_house` даёт реплику `workers_full` (Остин-Пауэрс-флейвор), торг не открывает. TradeUI берёт размер из каталога (`get_squad_size`), `coin_value` стал `@export`.
