...
① база: закладка замка чертежом (СДЕЛАНО, см. 5.27.4); ② врата-руина + 3 гнезда + расширение ValleyQuests (СДЕЛАНО, см. 5.27.6); ③ замок ур.2 (квартал-соседство); ④ комнаты по одной с плейтестами (начать с Подземелья — самый рискованный дизайн); ⑤ пробуждение меха + финал-открытие врат. Проходы к комнатам Врат до своей фазы закрыты (завал/ворота в периметре); застава открыта сразу (база).
...
- **`RubblePile`** (`rubble_pile.gd` **extends Blocker**, `scenes/rubble_pile.tscn`) — завал: куча косых серых боксов (материал greybox) на StaticBody; физика/группы целиком от Blocker. `open()` — идемпотентный обвал: `ShatterEffect.building_explosion` в цвет камня + queue_free; зовётся фазой чертежа Врат (пока триггера нет — проходы стоят закрытыми). Группа `rubble_pile`. Два в сцене: **RubbleNorthWest (29, 0, 43)** — проём x 26..32 к комнате А «Разлом» (стена `WallNorthWest` разрезана: 18..26 + новая `WallNorthWestB` 32..44); **RubbleEast (92, 0, 56)** — на месте убранного `WallEastMid` (z 54..58), проход к комнате В «Подземелье гномов» (рядом гнездо камнеметателя (85,53) — дорога будет под обстрелом, осознанно).
...
- E2E headless: 3 фейк-посадки → count 3/3, awake=true, оба завала открылись без ошибок; тексты новых шагов рендерятся (dev-узел удалён) ✓.
...
- **Хаб-фазы 3-5 (§5.27):** фаза ② сделана (врата-руина + гнёзда + завалы, §5.27.6); осталось: замок ур.2 (квартал-соседство) + триггер «чертёж Врат» (он же откроет завалы `RubblePile.open()`), комнаты «Разлом»/«Подземелье гномов» с элементами, пробуждение меха на `GateRuin._on_awakened`.
