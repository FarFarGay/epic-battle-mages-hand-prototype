...
#### HandSpellMineScatter — `scripts/hand_spell_mine_scatter.gd` (2026-05-13, прямой залп 2026-05-14)

Магическое заклинание-ловушка. **Прямой залп из башни**: N мин-ракет вылетают серией со stagger'ом, каждая по своей баллистической дуге в свою точку приземления в круге `scatter_radius`. После приземления и arming-delay'я мины ждут жертв. Friendly-fire ON.

**Биндинг:** `equip_mine_scatter` (клавиша 5), `SpellType.MINE_SCATTER` в `HandSpell.SpellType` enum.

**Без промежуточного carrier'а** (дизайнерское решение 2026-05-14, удалён `MineCarrier`). Каждая мина сама является ракетой со своим trail'ом, своей баллистикой и landing-точкой. Multi-rocket-launcher feel + архитектурно проще (одна сущность вместо двух).

**Принципиально отличается от Super:** Super → carrier boost-up → homing-to-target → точечный удар сверху. Здесь — синхронный залп N независимых ракет в scatter-зону, каждая по индивидуальной дуге. Геометрия разлёта: точки приземления заранее в круге `scatter_radius`, скорость подгоняется обратной баллистикой под фиксированный launch_angle.
...
4. Telegraph: `AoeVisual.spawn_ground_ring` в зоне `_series_scatter_radius`, `warning_duration ≈ 0.9с`.
5. Заполняет `_landing_queue: Array[Vector3]` — `_series_mine_count` точек uniform-в-круге (`sqrt(rand)`) вокруг прицела. Каждая точка — `(target.x + cos(angle)·r, mine_ground_y, target.z + sin(angle)·r)`.
6. `_next_shot_in = 0.0` — первая ракета на ближайшем `tick()`.

**`tick(delta)`:**
- Декрементит `_cooldown_remaining`.
- Если `_landing_queue` не пуста и `_next_shot_in ≤ 0`: `pop_front()` точку → `_launch_one_mine(landing)` → `_next_shot_in = shot_interval (0.06с)`.
- `is_active()` возвращает true пока очередь не отстреляна. `can_trigger()` блокирует новые касты пока series идёт.

**`_launch_one_mine(landing)`:**
- `launch_pos` = `Tower.global_position + UP × launch_offset_y (3.0)`.
- `up_boost = random(0.0, 1.5) м/с` — мелкая добавка к vy для разнообразия арок.
- `velocity = _compute_arc_velocity(launch_pos, landing, up_boost)` — обратная баллистика.
- `_spawn_mine_at(launch_pos, velocity)` — Mine с заданной initial_velocity, начинает в FALLING phase с собственной аркой к landing'у.

**`_compute_arc_velocity(source, target, up_boost)`:**
- Фиксированный launch_angle (`launch_angle_deg = 55°`). Скорость v решается из квадратного уравнения относительно `u = 1/v`:

  `A·u² + B·u + C = 0`,
  `A = g·d²/(2·cos²(α))`,
  `B = -up_boost·d/cos(α)`,
  `C = -(source.y + d·tan(α) - target.y)`

  Положительный корень `u` → `v = 1/u`. Возврат: `dir_h · v · cos(α) + UP · (v · sin(α) + up_boost)`.

- При `up_boost = 0` уравнение упрощается до классического `v² = g·d² / (2·cos²(α)·(d·tan(α) - dy))`.
- Edge cases: `C ≤ 0` (target выше «потолка» арки), `disc < 0` (нет реальных корней), `d < 0.0001` (вырожденно) — фоллбэк на фиксированную скорость 20 м/с при том же угле. Серьёзных вырожденностей в обычном гейплее не должно быть.
