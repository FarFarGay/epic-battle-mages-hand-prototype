...
**`setup(source, target, launch_speed)`:**
- `_compute_high_arc(source, target, speed)`: вычисляет initial velocity для **high-arc** решения баллистики (`tan_high = (v² + √disc) / (g·d)`, противоположно low-arc'у у `Arrow.gd`). Угол >45° даёт читаемый «лоб», горизонтальная скорость замедленная, арка высокая.
- Если discriminant < 0 (цель вне досягаемости за `launch_speed`) — фоллбэк на прямой выстрел в направлении target'а.
...
- **Apex detection:** `_prev_vy > 0 and _velocity.y ≤ 0` — vy переходит из + в −. В этой точке `_do_burst()`. `min_flight_time = 0.2с` защищает от false-positive в первый кадр (если launch_speed мал и vy сразу near-zero).
- `lifetime = 3.0с` — failsafe: если апекс не сработал (короткая дистанция, странная физика), всё равно burst на 3-й секунде.
...
| Phase | Поведение |
|---|---|
| `FALLING` | Гравитация (14 м/с²) опускает мину, `Area3D.monitoring = false` — никаких ложных детонаций на пролёт. На y ≤ `ground_y (0.05)` → ARMING. |
| `ARMING` | `arming_delay` сек (по дефолту 0.5с). Защищает от само-цепной реакции с соседями и от уже стоящих врагов в точке падения. |
