...
- **ГАРПУН-ЯКОРЬ:** мех непротягиваем (канон), но верёвка теперь ДЕРЖИТ его
  `HarpoonBolt.TETHER_TIME=2.5с` (новый State.TETHER: болт сидит в корпусе,
  верёвка натянута, по таймеру рвётся как раньше). На привязи
  (`EnemyMech.set_tethered`): стрейф/евейд/спринт ВЫКЛ, ход ×0.35 — контрплей
  его главной бесилки «вечно уворачивается». Отмена ПКМ работает (can_cancel).
