...
- **Размеры и позиции:**
  ...
  | Hand | сфера r=0.5 | следует курсору | y=2.5 (фикс) |
  | Item (бокс) | 1×1×1 | произвольно | y=0.5 (на полу) |
...
**Назначение:** управляемая башня-герой. Передвигается по миру через WASD, прижимается гравитацией к полу.
...
**Экспорты:**
- `hand_height: float = 2.5` — фиксированная высота полёта руки.
...
1. **Следование за курсором (`_process`).**
   Луч из активной камеры через `mouse_pos` → пересечение с плоскостью `Plane(Vector3.UP, hand_height)`. Точка пересечения = новая `global_position` руки.
...
**Экспорты:**
- `item_color: Color` — цвет предмета. Применяется в `_ready` через создание `StandardMaterial3D` с этим `albedo_color`.
