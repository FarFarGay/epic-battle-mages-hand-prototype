...
① база: закладка замка чертежом (СДЕЛАНО, см. 5.27.4); ② врата-руина + 3 гнезда + расширение ValleyQuests (блок «любой порядок»); ③ замок ур.2 (квартал-соседство); ④ комнаты по одной с плейтестами (начать с Подземелья — самый рискованный дизайн); ⑤ пробуждение меха + финал-открытие врат. Проходы к комнатам Врат до своей фазы закрыты (завал/ворота в периметре); застава открыта сразу (база).
