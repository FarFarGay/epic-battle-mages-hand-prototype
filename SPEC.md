...
**Зона сборки** (этап 51). `gather_radius: float = 50.0` (@export) — XZ-радиус от `deploy_anchor`. Кучи дальше невидимы. Хочешь дальше — двигай лагерь. До этого `_find_nearest_pile` иттерировал ВСЕ кучи на карте, гном мог топать на 100м к pile'у на противоположном краю.
...
#### Mine — `scripts/mine.gd`, `scenes/mine.tscn` (2026-05-13, Damageable + StaticBody 2026-05-15)

Объект-ловушка от Mine Scatter. **StaticBody3D** (этап 51) на слое ITEMS (=2) + MeshInstance3D (визуально — тёмный диск с красной emission «индикатор») + BodyShape (CylinderShape3D — для приёма AOE-damage через ITEMS-маску) + Area3D `TriggerArea` (SphereShape3D radius=0.6, proximity «педаль»).

**Damageable** (этап 51 — принцип симметрии): мина регистрируется как Damageable в `_ready`. ARMED-фаза реагирует на любой урон — Slam / Fireball AOE / Firestorm shot / Super payload / BurnPatch tick / соседняя мина (chain) — все используют `MASK_HAND_SLAM` который включает ITEMS, видят мину через её collision_layer. FALLING/ARMING фазы no-op: иначе burst-волна mid-fall'е сама детонировала бы паттерн раскидывания.

**FSM:** `FALLING → ARMING → ARMED`.

| Phase | Поведение |
|---|---|
| `FALLING` | Гравитация (22 м/с²) опускает мину, `Area3D.monitoring = false`. В текущей доставке через Fireball-pattern эта фаза транзитная (мина спавнится с velocity=0 уже на земле → ARMING в следующем кадре). |
| `ARMING` | `arming_delay` сек (по дефолту 0.5с). Защищает от уже стоящих врагов в точке установки. Мина видна, но не мигает. |
| `ARMED` | `Area3D.monitoring = true`. **Emission пульсирует** — синусоида с `armed_blink_rate=8.0` rad/s (~1.27 циклов/сек), `armed_blink_peak=3.0` → emission_energy колеблется от base до base×(1+peak). Сигнал игроку что мина живая. На `body_entered` — `_explode`. |

**`_explode`** → `PhysicsShapeQuery` радиусом `aoe_radius`, damage всем Damageable. Дополнительно **`AoeVisual.spawn_explosion`** в position мины — полный fire-explosion VFX (ядро + огонь + дым). Этим взрыв при детонации отличается от приземления-доставки (там pыль без огня). Эмитит `exploded` + `queue_free`.

**Per-instance material**: `_material` дублируется в `_ready` из `_mesh.material_override` (StandardMaterial3D). Без этого пульсация одной мины меняла бы emission у всех — материал шарится между инстансами сцены по дефолту.
...
- `aoe_damage_mask = ENEMIES | COLD_ENEMY | FRIENDLY_UNIT | ACTORS | CAMP_OBSTACLE | PALISADE_OBSTACLE | ITEMS = 951` — кого бьёт взрыв (включая Tower/палатки/палисад — friendly fire ON, дизайнерское решение 2026-05-13; ITEMS добавлен 2026-05-15 — chain reaction между минами).
