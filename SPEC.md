...
- **ТРЕВОГА (2026-07-02, клавиша V — «население как рычаг в бою»):** `Population.alarm_active` + `set_alarm` (мгновенный `_recompute`, эмитит `EventBus.alarm_changed`). Во время тревоги `_recompute` НЕ комплектует производство вовсе (`staffed={}`) → вся добыча/мана встаёт существующим механизмом простоя; военных не трогает (солдаты воюют). Рабочие-артель бросают приказ и прячутся в УБЕЖИЩЕ (`SoldierGnome._active_tick` ветка тревоги → `_tick_hide_at(_shelter_center())`): замок (группа `castle`), пока замка нет — башня; та же IN_TENT-механика (невидим/неуязвим), `_tick_hide_in_tower` отрефакторен в обёртку над `_tick_hide_at(dest)`. Отбой (V повторно) → обычные ветки выводят наружу. V слушает `Population._unhandled_input`, НО уступает клавишу Camp'у, если тот в сцене (main: V = режим ALARM сборщиков). HUD: красный баннер «🚨 ТРЕВОГА …» под монетами (`_build_alarm_banner`, mouse_filter IGNORE, подписка/отписка симметричны); плашки шахты/института при тревоге пишут «🚨 ТРЕВОГА — простой» вместо «⏸ Нет населения». Мобилизация (рабочие→ополчение) отклонена юзером — не делать.
