...
- ⚠ Осознанная дыра: замок НЕ восстановить (чертёж и фундамент one-shot), поражения-оверлея нет → потеря замка = мягкий софтлок города (штурмы прекращаются — WaveDirector гейтится группой castle). Дизайн-решение (руина+перезакладка / поражение) — PENDING за юзером.
