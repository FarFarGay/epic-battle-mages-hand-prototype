...
**`_spawn_instances`:**
- **Safe-фильтр включается только для `resource_type == WOOD`.** Логика геймдизайнера: лес — «глушь», вокруг поселений вырубленный; деревья ближе к лагерю не растут. Камень/железо/еда фильтр не проходят: каменоломня может стоять под защитой лучников, ферма — вокруг лагеря, склад железа — на POI. Это явный, не-обобщённый фильтр на один тип ресурса.
- Если зона WOOD — `get_first_node_in_group(&"wave_director")`. WaveDirector добавляет себя в эту группу в `_ready`; ResourceZone находит его без явного NodePath. Если WaveDirector не нашёлся (тестовая сцена без режиссёра) — safe-фильтр выключен. Для не-WOOD зон `wave_director` остаётся `null` → safe-чек не работает.
