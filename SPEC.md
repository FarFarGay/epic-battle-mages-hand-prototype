...
Стройка не висит кнопкой на экране — она **вкладка карточки отряда рабочих**. Только у `soldier_type==&"worker"`: кнопка «🔨 Стройка» → `_open_build_menu(btn)` → PopupMenu (`_build_menu`, ленивое создание) с пунктом `BUILD_MENU_BRIDGE` «🌉 Мост через пропасть» → `hand.bridge_aim.start_aim()`. Под новые постройки — добавлять пункты в это меню.
