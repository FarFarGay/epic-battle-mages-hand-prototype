...
**МОДЕЛЬ «КВАРТАЛ» (2026-06-28, коммиты 31fdfc7/a272a8b) — ОТМЕНЯЕТ конвейер: добыча = активное здание + сапорты-баффы по соседству (хрупкая цепь линий убрана).**
- **ШАХТА** (`PAD_MINE`, role mine) на жиле (`OilDeposit`, 1 тип = бронза; гейт `_pad_valid`: на жилу только шахту, шахту только на жилу) сама капает деньги в казну, медленно: `rate = MINE_RATE(1.0) × (1 + N_сапортов × SUPPORT_BONUS(0.6))`. Бронза → `GoldBank.add_coin` (одометр копит вверх). +N всплывашка (реюз `SquadXpPopup`, drift «дымок») и **фейерверк на каждую новую золотую** (`get_coin(GOLD)` до/после) — над шахтой.
- **САПОРТ** = соседнее (4-соседство) здание категории PRODUCTION (плавильня/двор, НЕ другая шахта) ИЛИ SOCIAL (дом). `_count_support_neighbors` (сапорт = +1 независимо от формы, dedup по зданию). Снос сапорта = минус бонус, шахта качает дальше (живуче).
- **ФИДБЭК «квартал собран»** (`_play_quarter_fx`/`_quarter_cells`, коммит этой сессии): при РОСТЕ числа сапортов ВЕСЬ квартал (шахта + все её сапорты, полные футпринты + площадка под ними) разом мигает ярко-жёлтым полупрозрачным столбом по каждой клетке, гаснет тваном за 0.5с. Не пшик на каждое здание — единая вспышка пятна. Триггер на шахте (по `_prev_supports`).
- **PENDING (дыра баланса):** сапорты пока ОДИНАКОВЫ (+скорость), 4 плавильни = 4 разных по бонусу → разнообразие не вознаграждается. План: бонус по РАЗНЫМ типам (distinct-роли, дубликат +0), и/или разные баффы у типов (плавильня=скорость, двор=ценность, дом=…). НЕ решено/не сделано.
- **Формы:** шахта 1 кл.; плавильня (`PAD_SMELTER`) 1 кл. (ровный дым-флавор); чеканный двор (`PAD_MINT`) Г 4 кл. (статичная монета); дом (`PAD_HOUSE`) прямой 3 кл.
...
- **Превью стыковки на гриде** (`hand_place_aim._update_connection_hints`): при наведении силуэта соседние здания подсвечиваются полупрозрачным оверлеем — зелёный=сочетается / красный=нет (по `connects`). `set_connection_hint` у PadBuilding и Castle (оверлей-бокс, НЕ правка emission — иначе сбили бы свечение; гард по состоянию).
...
- **ПОСТРОЙКИ АТАКУЕМЫ (Фаза 2, коммит c2f10a4):** `PadBuilding` → `StaticBody3D` (сам = коллайдер = damageable). Коллайдер по футпринту, слой `CAMP_OBSTACLE` (блокирует скелетов и башню, ловит магию/слэм). HP по роли (стены/ворота 140 / казарма-банк 100 / плавильня-башня 80 / шахта-двор 60) + `take_damage`/`damaged`/`destroyed` + hit-flash; смерть → `ShatterEffect` + снос с грида + `refresh_walls`. Группы: все `skeleton_target`+`damageable`; стены/ворота ещё `melee_only_target` (щит). Бродячие скелеты бьют ближайшую постройку по vision (как башню); волны-штурм у базы — pending. См. [[project_ebm_buildings_combat_aggro]], [[project_ebm_ore_deposits]].
