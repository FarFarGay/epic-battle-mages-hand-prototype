...
- **`CastleFoundation`** (`castle_foundation.gd`, узел на **(66, 84)** — чуть восточнее центра, чтобы ОБЕ восточные жилы попали в площадку замка 13×13; жилы не двигались): плита-постамент + пульсирующее кольцо в тон «синьки». `seat(blueprint)`: доводка-поглощение предмета → FX+шейк → спавн `RoomBuildSite` c `building_id=PUMP` на снап-клетке (артель строит обычным путём; чит free_build — мгновенно) → кольцо гаснет, плита остаётся под замком.
