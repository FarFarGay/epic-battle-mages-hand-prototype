...
**Статус (2026-06-22, АПДЕЙТ 2026-07-07):** в `WaveDirector` НЕ вшит — стоит один тестовый экземпляр `StoneThrowerTest` в `level_rooms.tscn` (гнездо СВ долины, `(85, 3, 53)`). **Room-гейт ДОБАВЛЕН** в базу `SkeletonGiantThrower` (`room_center/room_size/room_leash_margin`; без гейта — радиус пробуждения 60): тестовому задан прямоугольник долины. Дуэль v4 — термо-цикл, молнии-пулемёт, парир, паника, крит окна (коммит `9ad4a9b`). TODO: интеграция в боссовые волны.
...
- `dash_form: Dictionary` (опционально, 2026-07-11) — «ближняя атака» заклинания: заряженный супер-дэш через обводку фигуры на земле (`points`/`mana_cost`/`damage`). Полное описание — §5.29. Нет секции → обычный супер-дэш.
...
- **Tower** спавнится в тоннеле-пещере Room1 ((−30,3,0), с 2026-07-05 — §5.26.1; раньше в поле западнее), **мобильна на WASD** (§5.1, тот же `tower.gd` — едет, дэшит, супер-дэшит). **Hand** + **CameraRig** (follow Tower) — как в `main`.
...
- **Room5** — финальная: пропасть + 3 дерева; сценарий «работа под угрозой» (плашка заперта в роще, волны на шум рубки — §5.26.6), носка/мост — 5.22.5.
...
#### 5.22.5 Рабочие → дерево → склад; мост = ПЛАШКА-предмет (`soldier_worker.tscn`, `wood_source.gd`, `bridge_plank.gd`)
Курьерская петля рабочих на единой модели **«гном → точка → действие»** (группа `gnome_strike_target` + контракт `can_gnome_interact(gnome)` / `gnome_hit(gnome)`, как горшок/рычаг): рубят дерево → несут на склад башни (TowerStore). **Мост больше НЕ стройка** (пивот 2026-07-03: `bridge_site.gd`/`hand_bridge_aim.gd` удалены) — мост это физическая плашка, см. ниже.

- **Рабочий — БУРЫЙ ГНОМ** (`scenes/soldier_worker.tscn` = тело `gnome.tscn` + скрипт `SoldierGnome`, БЕЗ FacingIndicator-«копья»). Стартовая артель (7) живёт в башне с начала; найм рабочих переехал с домика гномов на КАЧАЛКУ-ЗАМОК (клик, 2026-06-25, §5.22.10b). Каталог `SoldierSystem` `&"worker"`: «Артель рабочих», `squad_size=3`, hp 60, attack 4–6 (по дереву/стройке, НЕ по врагу), move_speed 2.4, цвет бурый. Поведение разводится по `is_worker()` (`soldier_type==&"worker"`): не ищет врага, не делает боевой выпад (LUNGE), а **рубит дерево / носит бревно на склад** и **прячется в башню** по команде. Привязь рабочих шире боевой (`WORK_LEASH_RADIUS=16` vs `SQUAD_LEASH_RADIUS=7`) — курсирует дерево↔склад до ~20м.
- **Носка брёвен (видимая курьерская петля).** Флаг `_carrying_log` гейтит, какая strike-цель примет рабочего: пустой → дерево, гружёный → склад башни. `receive_log()` ставит флаг + спавнит коробку-бревно на руках; `deliver_log()` снимает + убирает визуал. `_tick_worker(delta)`: ищет цель через leash, дальше attack_range — идёт **по навмешу** (`_move_toward`), вплотную — бьёт **на месте** (`_strike_at`, без выпада — иначе 22 м/с прошивали мост → рабочие улетали в пропасть).
...
- **ПЛАШКА-МОСТ** (`bridge_plank.gd`, узел `Room5/BridgePlank` в `level_rooms.tscn`; пивот 2026-07-03 — «больше ручного взаимодействия»). RigidBody3D-доска (11×0.35×4.6, две поперечины снизу) лежит у пропасти. **Grabbable** (подсветка-кандидат `set_highlighted` по [[feedback_grabbable_highlight]]) + **`hand_haul` — ВОЛОЧЕНИЕ** (2026-07-03): рука НЕ морозит доску, а тянет пружиной-демпфером за **точку хвата** (проекция позиции руки на короб коллайдера при грабе — взялся за конец → тянешь за конец, доска разворачивается рычагом, дальний конец скребёт землю). Реализация `HandPhysicalActions._apply_haul_force` (в `_physics_process`; экспорты `haul_stiffness=80 / haul_damping=13 / haul_max_accel=110 / haul_angular_damp=4` — подтянуто с 40/7/60/2 по фидбеку юзера «меньше болтает, покрепче», 2026-07-03); обычный grab (freeze+телепорт) не тронут — haul только для `Layers.HAND_HAUL_GROUP`. Release без импульса броска — доска отцепляется с естественным моментом. Дальнейший тюнинг: болтанку гасит в первую очередь `haul_damping` (16-18), жёсткость поводка — `haul_stiffness`. Рука подняла и отпустила **над полосой пропасти** (X в пределах стены±полдоски, Z в полосе барьера ± `snap_margin=3`) → **защёлк**: tween 0.16с на место поперёк пропасти (X = центр стены, Z clamp в полосу), пыль по обоим концам + `camera_shake 0.3`.
- **Слои плашки:** свободная = `ITEMS` (обычный предмет); защёлкнутая = `freeze` + **`MOUNTED_MODULE`** — тот же приём, что модули башни: Tower (маска 575, без бита 64) проезжает СКВОЗЬ доску, а GrabArea руки (маска 502, с битом 64) видит → **можно схватить обратно** → пропасть закрывается (цельная стена + rebake). Snap/unsnap повторяемы.
- **Владение барьером.** Пропасть = сплошной пол + `ChasmVisual`-полоса + `ChasmBarrier` (StaticBody, группы `chasm_barrier`+`navmesh_source`). При первом защёлке плашка **забирает владение**: `_peek_bounds` читает box-геометрию (без разрушения — для проверки «над пропастью ли дроп»), `_commit_capture` нейтрализует оригинал СРАЗУ (вон из групп + layer 0 + disabled + queue_free), дальше `_rebuild_barrier(gap_lo, gap_hi)` пересобирает сегменты: с проёмом шириной доски (лежит) или цельный (снята). Rebake **отложенный** (`create_timer(0.05)` — синхронный bake в том же кадре ещё видит queue_free'нутую геометрию); гномы/скелеты тоже ходят по проёму.
...
> ⚠ СУПЕРСЕДЕД 2026-07-11: палитра «выбрал → купил за монеты» заменена **КОЛОДОЙ КАРТ за ману** (§5.30). Панель BuildPalette и карточки переиспользованы (Tabs = кнопка взятия, Sections = рука); вкладки BUILD_SECTIONS удалены. Ниже — история UI.
...
- **Секции-категории** (`BUILD_SECTIONS`): ⛏ ДОБЫЧА-квартал (шахта/плавильня/двор) · 🛡 ОБОРОНА (стены/ворота — пассивные структуры) · ⚔ ГАРНИЗОН-квартал (казармы/барак) · 🏰 ЗАМОК·СОЦИУМ (качалка/дом/платформа) · 🔮 МАГИЯ-квартал. Секция 🌉 ИНЖЕНЕРИЯ убрана (2026-07-03): мост = плашка-предмет (§5.22.5), не стройка. Кварталы (продюсер+филлеры) вынесены в свои секции; ОБОРОНА = пассив. Под заголовком — карточки в GridContainer.
- **Карточка** (`_make_build_card`, Button с детьми-лейблами `mouse_filter=IGNORE`): иконка+имя, **цена монетами** справа (`_format_cost`: 🥇🥈🥉 из `cost` / 🪵×N из `resources_needed`), **строка эффекта** снизу (`hint` из `RoomBuildings.CATALOG`). Клик → `_on_build_card_pressed` → `_on_build_menu_id` (диспетчер: всё через `place_aim.start_aim`).
- **Состояния:** недоступное здание серое + причина (`🔒 нужно знание гномов` / `🔒 нужна качалка` / `✓ уже построена`); цена краснеет, если казна не тянет (`_refresh_build_affordability`, live на `resources_changed`). Гейты: `_building_unlocked` (станок Room11, §5.22.7) + `_pump_built` (качалка для фигур города). Старые постройки (свободные стена/башня, бур, трубы) из палитры убраны — заменены полимино-моделью (§5.22.9).
...
> ⚠ **СУПЕРСЕДЕД 2026-07-07:** «Саймон»-пазл ниже ВЫПИЛЕН — станок переехал на заставу Долины и переработан в **физическую растопку** (уголь → Искра → пускач), см. §5.27.4. Диодные расширения `sparked/reset/flash_hint/locked` живы (используются `locked`+`sparked`).
...
**ИМПАКТ УСТАНОВКИ — «печать руки» (2026-07-03, `place_impact_fx.gd` — статик-хелпер, preload-константой БЕЗ class_name):**
- **Удар:** здание спавнится на финальном месте, поднимается на 3.5м и падает за 0.13с (ease-in) → сквош корня `×(1.12, 0.68, 1.12)` с упругим восстановлением (TRANS_BACK, 0.24с) → кольцо пыли по краю футпринта (4–10 клубов по радиусу) → рябь по гриду (2 расходящихся `spawn_expanding_ring` грид-голубого со сдвигом 0.09с) → тряска камеры (`EventBus.camera_shake` 0.35, дистанс-фоллофф). Сквош восстанавливает В БАЗОВЫЙ scale (стены растянуты по X — не в ONE). Все твины node-bound. Хитстоп НЕ используется (HitStop — политика оглушения врагов, «рука не замирает» by design). Автолоады в статик-контексте берутся через `/root/EventBus`.
- **Точки вызова:** полимино — `HandPlaceAim._commit` → `PadBuilding.play_place_impact()` (радиус по дальней клетке маски); хауленные — `RoomBuildSite._finish` → `PlaceFx.play(building, footprint/2+0.7)` (одиночный dust-puff оставлен только для площадок без сцены). `refresh_walls` импакт НЕ зовёт: перестройка соседа ≠ новая постройка.
- **Линк-пульс квартала** (`PadBuilding.flash_quarter_links`, зовётся ДО импакта — позиции ещё наземные): установка закрыла грань продюсера → световая сфера летит от сапорта к продюсеру (гаснет кольцом цвета роли), продюсер показывает плашку осей на 2.5с БЕЗ наведения (`flash_indicator`, повтор продлевает — kill прежнего твина). Обе стороны: поставил сапорт → пульс к продюсеру; поставил продюсера на готовые сапорты → пульс от каждого.
- **Трата ощущается:** `EventBus.coins_spent(value)` эмитится из `GoldBank.spend_cost`/`try_spend` (ЕДИНАЯ точка — стройка/найм/спеллы) → HUD панчит счётчик казны (scale 1.1→1) + красная всплывашка «−N🥇🥈🥉» (одометр-раскладка `_fmt_coin_value`; курсы дублируют gold_bank — без импорта, как HEAVY_GROUPS в HitStop), уплывает вниз 0.7с. Подписка/отписка симметричны.
- Проверено рантаймом 2026-07-03: замок поставлен через палитру, импакт без ошибок, «👥 население 6» появилось (замок жив).

**ВРЕМЕННО (тест, вернуть позже):** `gameplay_hud.TEMP_BUILD_ALWAYS_UNLOCKED` (стройка с старта без станка Room11); `room_build_site.free_build` (2026-07-02: бывший const `TEMP_FREE_BUILD` → **static var**, рантайм-тумблер читом «Бесплатная стройка» в Журнале; действует на момент установки площадки).

**САМОСТРОЙКА + ЕДИНЫЙ ПУТЬ ПЛОЩАДКИ (2026-07-03, решение юзера «артель только лечит башню, не строит»):**
- **Доставка ресурсов ВЫРЕЗАНА из стройки:** RoomBuildSite больше не strike-цель гномов (`can_gnome_interact`/`gnome_hit` удалены, из `GNOME_STRIKE_TARGET_GROUP` не входит). Здание возводится САМО: `_process` растит призрак по таймеру `build_time_for(data)` — явный ключ каталога `"build_time"` > полимино `1.6 + 0.8×клетка` > дефолт `4.0с`. Артель гномов = ремонт башни (gnome_hit башни жив), рубка/мост не тронуты. Поля каталога `resource_type`/`resources_needed` — мёртвое легаси (помечено, до чистки).
- **ПОЛИМИНО ЧЕРЕЗ ПЛОЩАДКУ:** `_commit` больше НЕ строит PadBuilding мгновенно — любая постройка (кварталы/стены/ворота/замок/стена-брус) = RoomBuildSite. Призрак полимино — куб на каждую клетку маски (общий материал → flash урона красит весь недострой). `_finish` при `"cells"` собирает `PadBuilding.new()+setup()` кодом, зовёт `flash_quarter_links` + `PadBuilding.refresh_walls` + `play_place_impact` («печать» на ДОСТРОЙКЕ). Ворота сносят стены под клетками СРАЗУ при установке (не ждать достройки). `free_build=true` (дефолт) — мгновенная достройка тем же путём.
- **Резерв грида:** `RoomBuildSite.occupied_cells()` (полимино) + `HandPlaceAim._occupied_cells` сканирует `BUILD_SITE_GROUP` — второй силуэт не наложить на недострой. Недострой НЕ считается гранью квартала/сапортом (не PadBuilding) — грань закрывает только достроенное здание. Скелеты могут сорвать стройку квартала (site в skeleton_target, site_hp дефолт 35).
- **Карточки палитры:** цена = монеты + «⏱Nс» самостройки (`_format_cost` через `RoomBuildSite.build_time_for`); «N🪵» доставки убран.

**РАЗГРУЗОЧНАЯ ПЛАТФОРМА + ТРЮМ БАШНИ (2026-07-03, ритм «выезд → фарм → домой разгружаться»):**
- **Трюм** = склад башни (`tower_store`, кап 30/тип): руда-орбы всасываются башней в зоне добычи, артель сдаёт ношу туда же. Полный тип → орбы НЕ всасываются (лежат) — добыча встаёт, пора домой. HUD: строка «📦 трюм N» в правой панели (сумма материалов; «N ⚠» янтарным при любом полном типе), реактивно на `resources_changed`.
- **Платформа** (`PAD_UNLOAD`, 2×2, роль `unload`, секция 🏰 ЗАМОК·СОЦИУМ, 3🥈): плоская плита с маячками, **БЕЗ коллизии** (`collision_layer=0` в `_ready` — башня заезжает; урон через группы, как RoomBuildSite). Башня в `UNLOAD_RADIUS=6` → раз в `UNLOAD_INTERVAL=0.22с` одна единица трюма → `GoldBank.smelt_yield` → монеты («ссыпается струйкой», салют на золотые штатный).
  > ⚠ СУПЕРСЕДЕД 2026-07-11: сдача-ритуал убран — ресурсы конвертятся в казну САМИ в момент прибытия (`tower_store.deposit`, §5.30); PAD_UNLOAD недоставаем (вне колоды), код жив.
- **ЕДИНАЯ точка конверсии — `GoldBank.smelt_yield`** (переехала из soldier_gnome): плавильная ветка рабочего ВЫРЕЗАНА (`_tick_smelt_at`/`_smelt_yield`/`HARVEST_BRONZE_PER_LOAD` удалены) — плавильня снова ЧИСТЫЙ сапорт шахты (двойная роль ушла). Таблица поправлена по интуиции тиров: дерево→бронза×5, камень→СЕРЕБРО, железо→ЗОЛОТО (было камень→золото).

**ТРИ ВАЛЮТЫ ИГРОКА (2026-07-03, консолидация ресурсов):** игрок видит только МОНЕТЫ (одометр) + МАНУ + НАСЕЛЕНИЕ. Материальные счётчики (золото-ресурс/дерево/камень/железо) вырезаны из правой панели HUD (`RESOURCE_DISPLAY = []` — вернуть счётчик = вернуть запись). Материя осталась ТЕХНИКОЙ под капотом: руда с источников → артель несёт в ПЛАВИЛЬНЮ → сразу монеты в казну (`_tick_smelt_at`; плавильни нет → ноша роняется орбом, орб ждёт склад); дерево → мост/блюпринт-станок в руках напрямую; склад башни (`tower_store`) — невидимый техбуфер (руда до плавильни, ferry дерева к блюпринту), `debug_start_amount` 100→0 (боевой старт пустой). PAGE/FOOD давно мертвы; тип GOLD живёт как НОМИНАЛ монеты (не трогать).
...
#### 5.22.9b WaveDirector в rooms: банды днём + ночной штурм на замок (2026-07-02) (`level_rooms.tscn`, `wave_director.gd`, `journal_panel.gd`)

**Оркестратор волн переиспользован в rooms** (был только в main.tscn). В `level_rooms.tscn` добавлены: `SpawnZones` (2 × `SpawnZone` 30×22 в комнатах z=0 и z=28, `wave_count=999` — warband-origin не расходует budget), `EnemySpawner` (target=Tower, зоны, `map_half_extent=150`), `WaveDirector` (`skeleton_scene`, `giant_scene`, `warband_siege_enabled=true` дефолтом, `background_initial_count=0` — без стартовой россыпи поверх SkeletonFiller'ов, `day_warband_count=1`/`size=12`, `night_assault_size=40` — комнатный масштаб).

**Выключен по умолчанию:** новый export `autostart_on_match: bool = true`, в rooms-ноде `false` → «Начать игру» НЕ стартует кампанию; включение — чит «Старт/рестарт волн», выключение — новый чит «Стоп волны» (`cheat_stop_campaign`: `_phase=IDLE` + чистка телеграфов, живые враги остаются).

**Якорь штурма обобщён (один путь для двух сцен).** Тракт ночного штурма больше не требует Camp: три хелпера `_assault_anchor_center()` / `_assault_zone_center()` / `_assault_zone_radius()` пробуют группу `&"camp"` (main-поведение дословно: центр = башня лагеря, телеграф = build_zone_center/build_radius), затем группу `&"castle"` (rooms: центр/телеграф = позиция замка, радиус = export `castle_zone_radius=14`). Нет обоих → `Vector3.INF` → штурм молча не стартует. **Следствие-гейт: ночные штурмы в rooms начинаются только ПОСЛЕ постройки замка.** Цель штурма по-прежнему ближайшая структура из `skeleton_target` к якорю (в rooms это пады/стены/Tower — сам замок в группу не входит, его атакуемость = отдельное дизайн-решение).

**Читы в rooms:** живые — старт/стоп волн, банда, штурм (с замком), день↔ночь, +100, стресс, гигант; «недоступно» — камповые (+ресурсы, +1000 золота лагеря, копейщики, туман). Новые: «Бесплатная стройка» (тумблер `RoomBuildSite.free_build`), «+10 золотых (казна rooms)» (`GoldBank.add_coin`). Проверено рантаймом 2026-07-02: кампания стартует читом, дневная банда 12 спавнится, штурм без замка отклоняется warning'ом.

**PENDING:** плейтест ночного штурма с построенным замком; зоны спавна двигать в редакторе под финальную геометрию комнат (двери-пазлы могут запирать путь банды к базе); дневная банда опортунистично грызёт припаркованную башню — тюнинг size/агро после плейтеста.
...
- **Метки целей**: `WoodSource` в группе `resource_source` (Layers), блюпринты (`RoomBuildSite`) в `build_site`; на достройке/сломе цель выходит из группы → диспетч видит «готово». (`BridgeSite` удалён 2026-07-03 — мост = плашка, §5.22.5.)

#### 5.23.4 Сбор через склад (gather→store)
Сбор идёт **через склад**: ткни деревья → GATHER наполняет склад башни (TowerStore), оттуда трюм разгружается на платформе в монеты (§5.24). Историческая ветка «BUILD берёт дерево со склада и кладёт доску в BridgeSite» умерла вместе с мостом-стройкой (2026-07-03, мост = плашка-предмет, §5.22.5); постройки полимино строятся САМИ по таймеру (§5.22.9), ресурс-доставка не нужна.
...
>
> Деньги = **ОДНО число** (`GoldBank._value`, бронза-эквивалент). Три номинала 🥉/🥈/🥇 — лишь ОТОБРАЖЕНИЕ (одометр). Курсы **ДЕСЯТИЧНЫЕ (пивот 2026-07-07, «доллары и центы»): 1🥈 = 10🥉, 1🥇 = 10🥈 (🥇 = 100🥉)** — разряды казны и цен сравниваются поразрядно (было 1🥇=25🥈 — разряды не бились, юзер путался). Бронза с добычи сама «копится» в серебро/золото (визуально); **размена как проблемы не существует** — покупка вычитает из общего числа, дисплей пересобирает монеты.
> - **ЦЕНЫ — ОДНИМ НОМИНАЛОМ (2026-07-07, фидбек «60🥉+5🥈 = это же 11🥈»):** все `cost` каталога построек — одно число бронзы, округлённое до чистых разрядов (казарма/верфь/институт 100=«1🥇», дом 80, ворота/барак 60, кристалл/проём 50, шахта/док/мостки 30, плавильня/палисад 10, стена-кусок 5). `GameplayHud._format_cost` суммирует цену в бронза-эквивалент и рисует через `_fmt_coin_value` — ТОТ ЖЕ одометр, что у казны; единый формат на палитру, найм, срезы верфи, магазин заклинаний. Сырую сумму («110 бронзы») в UI не показывать НИКОГДА — юзер мыслит номиналами.
> - ⚠ АПДЕЙТ 2026-07-11: **пады за монеты больше НЕ покупаются** — здания приходят картами колоды за ману (§5.30), их `cost` в каталоге спит (карты руки цену не показывают). Монетные цены живы у мостков, найма, срезов верфи, Врат — формат-одометр тот же.
...
- **«ЗАЩЁЛК» полного квартала (2026-07-02, СДЕЛАН):** ВСЕ грани продюсера закрыты сапортами → комбо-множитель ТЕМПА `FULL_QUARTER_BONUS = 1.5` поверх осей (один const на всех продюсеров). Шахта: `_tick_mine` умножает накопление (`MINE_RATE × speed × защёлк`); институт: `_mana_mult(st)` домножает на выходе (там же вспышка «собран» добавлена симметрично шахте). Хелпер `_full_quarter_mult(st)` читает `fill` из готового `_quarter_status` (без пересчёта). Плашки-индикаторы шахты/института: строка «⚡ Квартал ×1.5 СОБРАН! / — (заполни все грани)», пул строк 4→5, итоговый темп учитывает бонус. Казарма НЕ в защёлке (кап дискретен — обсудить отдельно).
- **PENDING:** разные ОСИ баффа по типам — СДЕЛАНО ранее (e347571); обобщить силуэт на другие категории (оборонный квартал); тюнинг формы/позиции `MINE_PLOT_CELLS`.
...
**НАСЕЛЕНИЕ = АРТЕЛЬ РАБОЧИХ (⭐ пивот 2026-07-12; юзер: «Артель рабочих = население… ничо почти и не поменяется»).** Абстрактный supply-пул (2026-06-30: замок `CASTLE_POP=6` + дома `pop_provided=4`, военный приоритет, `has_castle`) — ВЫПИЛЕН. Население — это ЖИВЫЕ гномы-рабочие артели (кап найма 7, `SoldierSystem`).
- **Смена в здании:** PRODUCTION-здание (`pop_demand>0`: шахта/плавильня/двор/институт/Кафедра/Осколок) забирает РЕАЛЬНОГО свободного гнома — `Population` (пересчёт ~0.25с) назначает ближайшего (`assign_staffing`), тот ИДЁТ к зданию и прячется внутри (IN_TENT, `_tick_hide_at(bpos, STAFF_ENTER_RADIUS=3.2)` — шире башенного 2.2, навмеш-дыра здания держит в ~2м от origin). Здание работает, ТОЛЬКО пока гном ВНУТРИ (`is_staffed` = дошёл; в пути — ещё простой). Источник правды — сам рабочий (`SoldierGnome._staff_building`); из прятки-башни на смену сперва выходит наружу (без glue-телепорта). Снос/взрыв здания или спрос упал (жила иссякла) → гном освобождается в артель. Назначения ЛИПКИЕ (без перетасовки), дефициты закрываются свободными по `pop_priority` (шахта/институт=0 раньше сапортов=1). Перетекание сапорт→продюсер — PENDING.
- **Солдат = потраченный гном:** найм лучника/копейщика РАСХОДУЕТ свободного рабочего — `Population.consume_free_workers(count, near)`: ближайший к казарме свободный гном исчезает (кольцо-визуал; из `SOLDIER_GROUP` сразу, `queue_free` после — мёртвые души). Спавнер зовёт `_consume_population` во ВСЕХ трёх путях (`request_squad_for` / `request_squad` / `_on_purchased`); рабочие сами гномы — не расходуют. Гейт торга: `_my_hire_cap = min(гарнизон, живые + military_room)`, где `military_room = free_slots` = свободные гномы.
- **Пополнение артели:** найм рабочих у замка + домик гномов (`hire_artel`) — как раньше, кап 7. ДОМ население НЕ даёт (карточка: «уют кварталов») — только ось «Объём» шахты и ×темп института; `pop_provided`/`HOUSING_POP` удалены, `pop_for_role(housing)=0`.
- **API `Population` сохранён** (потребители не переписывались): `cap()`=живых рабочих всего, `used()`=на сменах, `free_slots()=military_room()`=свободные, `is_staffed()`. `has_castle()` удалён (HUD гейтится по `cap()>0`).
- **БАРАК = ЁМКОСТЬ казармы** — без изменений: ось «Гарнизон» (+`HIRE_CAP_PER_BARRACK` к капу СВОЕЙ казармы), населения не даёт, категория DEFENSE, визуал стеновой пристройки.
- **HUD:** `👥 Артель: свободно X из Y` под монетами, янтарным при 0 свободных; виден при `cap()>0`. Карточки палитры: `👥−1` у production (реальный гном, голубым; у дома иконки больше нет) + `🛡+N` ёмкость у барака.
- **КАРТОЧКА АРТЕЛИ ДУБЛИРУЕТ СЧЁТЧИК (фидбек по скрину 2026-07-12: «счётчики должны дублировать показатели»):** заголовок worker-карточки (`_refresh_squad_card`) берёт `free_slots()/cap()` из Population, НЕ `alive/members` отряда — оба места на экране всегда показывают одно и то же (гном на смене или потраченный на солдата списывается из обеих цифр синхронно). Сантехника: (1) потраченный гном выписывается ШТАТНО `Squad.remove_member` (тихий `queue_free` destroyed-сигнал не эмитит → карточка не обновилась бы — так и был баг «на солдат не тратятся»); (2) `consume_free_workers` правит снапшот `_alive` сразу (синхронный refresh из remove_member читает свежие цифры); (3) смену чисел БЕЗ squad-события (спавн/смерть/уход-возврат со смены) Population пинает сам — в конце `_recompute` сравнивает `_prev_alive/_prev_free` и эмитит `EventBus.squad_changed` со squad'ом первого рабочего.
- **ТРЕВОГА (V, 2026-07-02, механика жива):** свободные рабочие прячутся в убежище (замок→башня, `_tick_hide_at(_shelter_center())`), здания ПРОСТАИВАЮТ (`is_staffed=false` при `alarm_active`), но гномов СО СМЕН НЕ выгоняем — внутри здания так же безопасно. Плашки: «🚨 ТРЕВОГА — простой» / без гнома — «⏸ Нет гнома на смене — простой». V уступает Camp'у в main. Мобилизация отклонена юзером — не делать.
- Связано: [[project_ebm_population_supply]], [[project_ebm_coin_economy]], [[project_ebm_quarter_variety_todo]]. Магические здания берут гнома идентично добыче (2026-07-01) — см. блок «МАГИЯ» ниже.
...
- **МАГАЗИН ЗАКЛИНАНИЙ** — клик по 📜 Кафедре свитков (`mana_crystal`, `_tick_scroll_click` → `EventBus.spell_shop_requested`) → HUD-окно (программно, `_build_spell_shop`/`_populate_spell_shop`). 3 заклинания: Огненный шар(5🥇)/Огненный шквал(8🥇)/Минное рассеивание(6🥇). **Купить за монеты** (`GoldBank.spend_cost`) → `SpellSystem.unlock(id)` (force-анлок без камп-цены) → «✓ Куплено». Не хватает → серая/красная. v1 единоразово; PENDING: производство заклинаний ПАТРОНАМИ (карточка → «Заряды: N»). `SpellSystem.spell_data(id)` — геттер каталога. **Фикс:** покупка эмитит `spell_unlocked` → HUD ПЕРЕСОБИРАЕТ трей (`_build_action_bar` теперь идемпотентен: чистит слоты + `_populate_action_slots`; подписан на `EventBus.spell_unlocked`) → купленное заклинание появляется слотом и equip'ается клавишей (был баг: трей строился 1 раз в `_ready`, новое заклинание не появлялось → нельзя использовать).
- Связано: [[project_ebm_spearmen_garrison]], [[project_ebm_spell_shop]], [[project_ebm_magic_institute]].

**ГАРПУН (2026-07-07, `hand_spell_harpoon.gd` + `harpoon_bolt.gd`).** Новое заклинание башни, ЕДИНАЯ модель: стрела с ВЕРЁВКОЙ летит ПРЯМО от башни (высота ~1 м над землёй у цели), **мелких врагов пробивает насквозь** (damage 60 → one-shot скелета/лучника, их собственный shatter; `_pierced`-сет — не бить дважды), **горшки/утварь разбивает на пролёте** (контракт `shield_breakable`→`on_spark`, как щит/Искра), **в тяжёлого (super_dash_only/мех) или свободный grabbable-предмет вцепляется и подтягивает к башне** (PULL: RigidBody — velocity к башне + ОБЯЗАТЕЛЬНЫЙ `sleeping=false` (спящее тело присвоением velocity не будится — «цеплялся, но не тянул»), CharacterBody — позиционное волочение по XZ; release 3.5 м; чип-урон тяжёлым 0.35×damage; cap 3 с). Предметы ищутся **sphere-query по слою ITEMS** (по коллайдеру — group-скан по origin мазал мимо края 4-м плашки); уложенный мост не вырывается (MOUNTED_MODULE+freeze вне запроса).
- **Верёвка ФИЗИЧЕСКАЯ**: верлет-цепочка 12 точек (гравитация, демпфирование, 4 итерации, клэмп пола), рендер — сегменты-цилиндры (растяжение ЛОКАЛЬНОЙ оси Y базиса — `.scaled()` мировой и ставил канат вертикально; гвард вырожденного кватерниона UP→−UP). ⚠ строить в `setup()`, не `_ready` (_effects_root приходит после add_child).
- **Фиксированная длина**: в момент зацепа (стена/добыча) `_rope_length` = текущая дистанция (кап `range`) — дальше НЕ резинится (рендер минфит длину), масштаб гарпуна = range в уровнях каталога.
- **Стена = ФИКСАЦИЯ**: гарпун втыкается (SPENT stuck), верёвка живая, и работает ПРИВЯЗЬ `_enforce_tower_tether` — башню осаживает по XZ на длине верёвки (держит против хода/обычного рывка, «зафиксировался у стены»); задел под грэпплинг-подтяжку. **Промах** = стрела клюёт носом и падает, верёвка болтается; натянул — упавший гарпун ВОЛОЧИТСЯ за башней (`_enforce_bolt_drag`, зеркало привязи).
- **Жизнь/смерть гарпуна**: сам НЕ исчезает (safety 30 с) — **отмена повторным ПКМ при выбранном гарпуне** (`cancel()`, без маны/кулдауна; в полёте не отменяется — спам ПКМ не убивает выстрел; другое заклинание выбрано — верёвка не трогается). **РАЗРЫВ верёвки**: по ВЕСУ (RigidBody mass > `ROPE_BREAK_MASS`=12 или МЕХ — apex непротягиваем, канон соблюдён) и по СИЛЕ (супер-дэш на натянутой привязи = осознанный срыв с фиксации; чтение `_dash_is_super`/`_dash_timer` башни). Разрыв = искры в середине верёвки + резкий fade 0.4 с. Доставка добычи — fade 0.35 с; **исчезновение всегда НАТИВНОЕ** (fade прозрачности), не мгновенный пшик.
- Каталог: range 20, cooldown 2.5, mana 25, bolt_speed 40, pull_speed 14; **unlocked_by_default=true ВРЕМЕННО (плейтест)** → потом false, товар уже в магазине Кафедры (6🥇). Проводка стандартная: SpellType.HARPOON=6, узел Harpoon в hand.tscn, ABILITY_META+DEFAULT_ASSIGNMENT в HUD. ЭМЕРДЖЕНТ: гарпун по клетке Room1 тянет её к башне на 14 м/с > break_speed 7 — клетка разбивается об башню, гном спасён дистанционно (симметрия сил, не баг).
- **Крупные враги**: гигант, гигант-метатель И обычный камнеметатель (все в `super_dash_only`: SkeletonStoneThrower extends SkeletonGiantThrower и наследует группу) — цепляются и тянутся («выдернуть снайпера из тыла» работает из коробки); мех — рвёт верёвку весом (канон apex).

**ГОРОД = ТЫЛ БАШНИ: КАФЕДРЫ-ШКОЛЫ, МАНА — ЕДИНСТВЕННАЯ ВАЛЮТА КАСТОВ (2026-07-07).** Корневое решение «стройка инородна»: город производит то, что башня расстреливает; связь = снабжение, НЕ удалённые бафы/наказания.
- ⚠ **ЗАРЯДЫ-БОЕУКЛАДКА: СДЕЛАНЫ И ОТКАЧЕНЫ В ТОТ ЖЕ ДЕНЬ.** Второй ресурс поверх маны юзер зарубил дважды: «снаряды поверх маны, которые тратятся вместе с маной — нахуя» и «плодить сущности не надо». Урок: заряды, платящие вместо маны = вторая мана с другим именем. НЕ ПРЕДЛАГАТЬ ре-интродукцию зарядов; дефицит тяжёлой магии решается ЦЕНАМИ маны и источниками. Механика полностью выпилена из кода (tower/hud/each caster/институт/орбы).
- **Экономика кастов (итог)**: одна мана; тяжёлые спеллы дорогие (шквал 50, мины 40 и т.д.), реген мизерный (0.8/с), институт заливает бак только В ДОКЕ, орбы с мелочи кормят по чуть-чуть (5), **тяжёлые враги роняют МАНА-КРИСТАЛЛ** (`XpOrbSpawner._spawn_mana_crystal`: super_dash_only/мех → XpOrb `mana_amount=40`, крупный ледяно-белый) — затяжной бой сам подкидывает топливо артиллерии.
- **КАФЕДРЫ-ШКОЛЫ (role `spell_lab`)** — «хочешь пушку, построй завод», здание на ШКОЛУ, не на спелл: 🔥 Кафедра огня (fireball+firestorm, 120🥉), ⚙ Инженерная гильдия (harpoon+mines, 120🥉), ❄ Кафедра льда (frost, 90🥉). Секция «🔮 Магия» палитры, гейт институтом (needs_magic). Постройка → `_unlock_lab_spells` (SpellSystem.unlock, идемпотентно) + хинт. **Снос кафедры знание НЕ отбирает** (локальная потеря = производство). **Магазин заклинаний УМЕР**: клик по Кафедре свитков теперь только редиректит хинтом; SPELL_SHOP_ITEMS/окно — мёртвый код (вычистить при случае).
- **МОДУЛИ-ЛОДАУТ, ПИЛОТ РЕАЛИЗОВАН (2026-07-07): Гарпунная турель** (`harpoon_module.gd`, класс HarpoonModule, @tool). Аппарат-ВЕЩЬ (grabbable RigidBody, мини-баллиста со стрелой и бухтой каната): рука несёт, **отпустил в 3.5 м от башни → защёлк на корпус** (freeze + reparent к башне + слой MOUNTED_MODULE — едет на корпусе, GrabArea снимает обратно; реперент deferred — не в теле сигнала). Установлен → **гном-оператор вылезает** (декор-меши, viden только mounted) и группа `harpoon_module_mounted` → **спелл «Гарпун» жив** (гейт в HandSpellHarpoon.can_trigger — без аппарата слот тусклый, попытка каста подсказывает; отмена воткнутого работает всегда); стрела вылетает ИЗ аппарата. Один слот (пилот): второй аппарат рядом с занятым не монтируется.
	- **Источники**: стартовый аппарат лежит В ТОННЕЛЕ Room1 (узел `Room1/HarpoonModuleStarter` + QuestMarker с done_group=mounted + HintModule — урок установки ДО клетки); **Гильдия инженеров КУЁТ новые кликом** (40🥉, `_tick_forge_click`: аппарат выпадает перед зданием в сторону башни, искры+шейк). Из spells гильдии harpoon УБРАН (мины остались) — доступ к гарпуну = аппарат, не «знание». unlocked_by_default=true у harpoon остаётся (слот в трее виден, гейт модулем).
	- Дальше по модели: слоты 2-3, аппараты других школ (шквал-мортирка, морозный излучатель), снятые лежат у верфи = физический лодаут.

**CLASH-ИНКРЕМЕНТ, дубль два (2026-07-07).** ⚠ Первая попытка — КЛИК-УРОВНИ зданий (×2/×4 выработка за монеты) — СДЕЛАНА И ВЫПИЛЕНА в тот же день: юзер поймал дубль («зачем, если за это отвечает полимино?») — квартал УЖЕ есть прокачка продюсера (оси граней + защёлк), клик-уровень был вторым путём к тому же множителю. Урок: из Clash брать не «уровни», а **СБОР**.
- **СТОПКА-СБОР у шахты (реализовано)**: добыча НЕ капает в казну сама — копится **видимой стопкой золотых монет на крыше** (`_stack_bronze`, 1 монета визуала ≈ 10 бронзы, до 8 в столбике; бронзовый эквивалент номинала — двор сохраняет ×10 через ценность). **Кап 60🥉 — полная стопка ОСТАНАВЛИВАЕТ добычу** (Clash: полный коллектор стоит, забери). Сбор: **башня вплотную (4.5 м) или ЛКМ-клик по шахте** → веер из 2-6 монет-орбов летит к башне (форс-магнит `_activate_magnet_to_tower`), каждый на прибытии кладёт монеты в казну с попапом «+N🥉» и салютом на золотую. Плашка шахты: строка «💰 Стопка N🥉 из 60». Итог: город производит ВЕЩИ, которые трогаешь; объезд города = жест сбора урожая.
- Кандидаты дальше: растущая цена повторных зданий (×1.5 за штуку); стопки у других продюсеров, если зайдёт.

**МАНА-РАДИУС ИНСТИТУТА (2026-07-07, фикс «хуярит через всю карту»).** Док-гейт маны с фоллбеком «нет дока → глобальный поток» ВЫПИЛЕН (дыра: без построенного дока институт кормил отовсюду). Одно правило: мана течёт в `INSTITUTE_FEED_RADIUS=18` м от института (XZ), при ПОЛНОМ баке не течёт и луч-пуповина не рисуется. `passive_mana_regen=false` в tower.tscn (пассивный 0.8/с отовсюду — выключен, пивот «в поле мана только с орбов» доведён). Итог: орбы +5, мана-кристаллы с тяжёлых +40, институт 3/с×сапорты×уровень в 18 м. Док = только ремонт.

**СЛОЙ ПРОПАСТИ CHASM_BARRIER (2026-07-07, фикс «мост врезается в каньон»).** Барьеры фейковых пропастей сидели на слоях стен (544) — когда предметы получили стены в маску (575), мост стал биться об невидимую стену пропасти. Барьеры переведены на СВОЙ слой 15 `Layers.CHASM_BARRIER=16384`: **держит** башню (tower.tscn mask 575→16959) и врагов (MASK_SKELETON 2087→18471, все 6 enemy-сцен — отброс не выкидывает «в дыру»), **пропускает** предметы (мост несётся/кладётся поперёк, гарпун летит через каньон; его WALL_MASK=544 = только настоящие стены). ⚠ Навмеш: слой добавлен в `geometry_collision_mask` запекания (4129→20513) — иначе пропасти переставали выгрызаться (поймано по polygons 456→358 в буте; после фикса 456). Побочка: XpOrb (mask 292) больше не упирается в барьер — орбы могут закатиться в зону «дыры», башня их вакуумит.
...
### 5.26 Туториал-анфилада Room1–5: пещера, уроки глаголов, босс, «работа под угрозой» (2026-07-05..06)

**Анфилада переработана в обучающую арку** «одна комната = один глагол»: Room1 — тело башни (движение/дэш/мана/супер-дэш), Room2 — рука-предмет (мост), Room3 — рука+цепь (ретранслятор), Room4 — первый бой и социум, Room5 — командование артелью. Коммиты `187611a`, `7dc36ad`.

#### 5.26.1 Room1 = пещера-туториал
- Запад Room1 (был **полностью открыт** в поле — башня заезжала снаружи) заделан: `WallWestNorth/South` (проём z −3..3) + **тоннель-аппендикс** x −34..−18 (`TunnelWallNorth/South/EndCap`, ширина 6 м). **Старт башни (−30, 3, 0)** — в глубине тоннеля; сценарий «башня выходит из пещеры».
- Скелеты зонированы: `SkeletonFiller` count 72 в восточных ⅔ (center (4,0,0), half 11×12) + новый `SkeletonFillerIntro` 6 шт у устья — первые одиночные цели урока дэша.
- Биты: движение (тоннель) → **спасение гнома из клетки** → дэш (одиночки) → мана (орбы) → супер-дэш при ≥50 маны → красная дверь = выход из пещеры.
- **Клетка с гномом (2026-07-07, `gnome_cage.gd`, класс `GnomeCage`)** — «первое спасение»: grabbable-RigidBody у устья тоннеля ((−14,0.1,−5), узел `Room1/GnomeCage`); рука хватает и МЕТАЕТ, удар ≥ `break_speed=7` м/с (калибровка: «положил» = 3-5, бросок >8 — см. `soft_release_velocity_threshold`) → ShatterEffect по направлению полёта + шейк 0.25 + рабочий в артель `request_squad(ROLE_WORKER,1)` (карточка HUD родится сама через `squad_created`) + 2 реплики гнома через `EventBus.tutorial_hint`. Порог удара сравнивается со скоростью ПРОШЛОГО физкадра (`_prev_speed`) — в `body_entered` контакт уже разрешён и velocity погашена. `HintCage` (зона у устья) подсказывает механику.
- **Артель стартует с НУЛЯ** (`starting_worker_count=0` в сцене): первый рабочий = спасённый из клетки; UI артели появляется в момент спасения (прогрессивное пробуждение). ⚠ Клетку можно проехать — тогда артели нет до стола торга Room4 (Room5-рубка без рабочих = стоп; принято для прототипа, клетка на пути + хинт).
- **Прогрессивное пробуждение HUD**: мана-бар скрыт до первой маны (`_build_tower_stats` прячет, `_refresh_tower_mana` показывает при current>0 навсегда — приём SuperBar «до разблокировки»); карточка артели — с первого гнома (см. выше).
- **Красная дверь (полиш 2026-07-07)**: осколки летят ВПЕРЁД по рывку (башня передаёт `_dash_dir` в `RoomDoor.shatter(hit_dir)` → `burst_dir` ShatterEffect) + лёгкий шейк 0.35.
- **QuestMarker (2026-07-07, `quest_marker.gd`)** — единый язык «золотой ромб = квест-цель»: @tool-Node3D-ребёнок квест-объекта (виден в редакторе), парит/вращается над ним, следует и умирает вместе с родителем; скрыт пока предмет в руке (на release виден снова); `done_group` (те же маркер-группы, что у TutorialHint) — шаг сделан → схлоп-твин. Повешен: клетка Room1, BridgePlank2 Room2 (`bridge_snapped`), RelayCrystal Room3 (`relay_seated`). Кандидаты дальше: элементы Врат акта II, BridgePlankA Разлома.

#### 5.26.2 Система подсказок TutorialHint (`scripts/tutorial_hint.gd`, class `TutorialHint`)
- Area3D-узел, **один скрипт на все триггеры**, `trigger_mode`: **ZONE** (бокс `zone_size`, collision_mask=4 — реагирует только на башню) / **MANA_GAINED** (первая мана > 0) / **MANA_AT_LEAST** (порог `mana_threshold`) / **GROUP_APPEARS** (в `watch_group` появился узел — «шаг пазла сделан, вот следующий») / **GROUP_EMPTIED** (группа была непуста и опустела — «врага не стало»; на пустоту со старта НЕ срабатывает). GROUP_* — поллинг Timer 0.3с (сигналов членства групп у движка нет).
- `_fire()` → `EventBus.tutorial_hint(text, duration)` → GameplayHud показывает `TutorialHintLabel` (узел в `gameplay_hud.tscn`, низ по центру, `mouse_filter=2`); прячет **Timer-нода** (не SceneTreeTimer-лямбда — та падает на freed HUD при рестарте). Новый текст перебивает старый. Одноразовость: `queue_free`; `monitoring` снимается только `set_deferred` (прямое присваивание в body_entered = ошибка «blocked during in/out signal»).
- `suppress_group`: если группа непуста — подсказка молча самоснимается (условие уже выполнено игроком; напр. «нужен мост» глушится `bridge_snapped`).
- **Маркер-группы шагов:** `bridge_snapped` (плашка лежит мостом, ставит/снимает `bridge_plank.gd`), `relay_seated` (ретранслятор в гнезде), `gnome_lever_powered` (`lever.enable()` при `gnome_pullable`+`gnome_needs_power`), `room4_giant` (личная группа TestGiant в сцене; ⚠ в группе `skeleton_giant` сидит и КАМНЕМЕТАТЕЛЬ — она не пустеет, для GROUP_EMPTIED непригодна).
- Хинты в `level_rooms` (узлы под `TutorialHints`): Move/Dash/Mana/Super (Room1), Grab/NeedBridge/Spark (Room2), Relay/ChainReady (Room3), Giant/GiantDead/GnomeLever (Room4), Artel (Room5); GroveGate шлёт свои прямо в шину. Искра в текстах — **клавиша [1]** (трей нумерует по разблокированным, не по input-action `equip_spark`=5).

#### 5.26.3 Room2 = урок руки: пропасть + мосток; генерализация `bridge_plank.gd`
- Пропасть **поперёк** Room2 (z=28, ширина 7): `ChasmVisual2` + `ChasmBarrier2` (группы `chasm_barrier`+`navmesh_source`, узел **повёрнут yaw 90°**, бокс 7×3×38), `BridgePlank2` на северном берегу. `SkeletonFiller2`: 20→8, только северный берег. Выход прежний — диод-дверь: «рука хватает → рука бьёт Искрой».
- **Генерализация плашки** (два бага одним махом): барьер выбирается **ближайший** из группы по центру ШЕЙПА (`_nearest_barrier`; было first-in-group — две пропасти в сцене дрались бы за одну плашку, а origin узла Room5-барьера смещён от полосы); вся математика — в **локальных осях шейпа барьера** (`_wall_xf` + yaw-базис `_yaw_basis`: local X = ось перехода, local Z = длина полосы). Повернул барьер в сцене → плашка, проём и сегменты подстраиваются сами; Room5 (identity-базис) — поведение бит-в-бит прежнее.

#### 5.26.4 Room3 = разрыв цепи: ретранслятор (`relay_socket.gd`, `relay_item.gd`, `scenes/relay_socket.tscn`)
- Цепь растянута по комнате: `BlueDiode` у входа (−12,46) → провод → **гнездо `RelaySocket`** в центре (−3,58) → провод → `RedDiode` у двери → рычаг. **`RelaySocket` extends `RedDiode`** (провод и бусина переиспользованы): переопределён `_on_current_arrived` — гнездо занято → `target_path.activate()` (каскад в RedDiode), пусто → разряд-пшик + `_energized=false` + `BlueDiode.reset()` — **нет софтлока**, синий диод можно бить снова. API `seat/unseat/is_seated/seat_transform`, группы `relay_socket`/`relay_seated`.
- **`RelayItem`** (`RelayCrystal` в сцене, RigidBody3D) — Grabbable-паттерн плашки БЕЗ haul (рука морозит сама): release в `snap_radius=2.5` от свободного гнезда → tween-посадка, слой MOUNTED_MODULE (башня проезжает, GrabArea видит → снимается обратно); grab из гнезда → `unseat`. `on_current_pass()` — тёплая вспышка кристалла при проходе тока. Визуал = усечённый цилиндр как у диодов («родня цепи»).

#### 5.26.5 Room4 = первый босс: слой Boss-боя в `SkeletonGiant`
Всё в `@export_group "Boss-бой"` `skeleton_giant.gd` (работает и для wave-гигантов):
- **Подача:** первый аггр башни → `_roar_intro`: `EventBus.boss_wave_incoming(3.0)` (баннер BossWarningOverlay), `camera_shake 0.7`, пыль+разряд, и **аггр всей комнаты** (`_aggro_room_skeletons`: forced-цель = башня для мелочи в границах room-гейта; тумблер `roar_aggro_room`) — бой плотный сразу, мана-фарм ВНУТРИ боя.
- **Стан-матадор:** заряд ПРОМАЗАЛ (`_charge_hit=false` на конце рывка) → стан `miss_stun_duration=1.5с` (было 3.0 — фидбек «очень длинно»), гигант стоит столбом, синий shared-материал `_shared_stun_material` = «окно, бей»; уворот от снарядов в стане не тикает → Искра-снайпинг гарантирован.
- **Урон:** `take_damage` override с ручкой `active_damage_factor=1.0` — гигант **уязвим всегда** (фидбек: стан = удобное окно, не единственное; <1.0 вернёт «броню активного»). **hp 240→750** (= ровно 3 супер-рывка по 250); `_apply_stat_variance` override — HP босса БЕЗ ±20% вариации базы (ломала «ровно 3 удара»), замах/скорость/кулдаун вариативны.
- **Призыв + фазы:** пороги ⅔ и ⅓ HP → `_summon_pack` (6 скелетов кольцом r=4, `attack_damage 3` — эскалация И мана-топливо супер-рывков) + `_escalate_phase` (`move_speed ×1.3`, `attack_cooldown ×0.75`, дважды за бой — финал самый горячий). На смертельном ударе призыва НЕТ (гвард `_dying`). `summon_scene = skeleton.tscn` задан в базовой `skeleton_giant.tscn`.
- **Шоквейв:** финиш ЛЮБОГО заряда (попал или в стан) → `_charge_shockwave`: expanding ring + пыль + шейк, knockback мелочи r=4.5 impulse 9 (+UP 2). Башню не трогает — она наказана самим контактом заряда.
- **Базовый темп** (tscn): `move_speed 2.0→3.2`, `attack_cooldown 1.6→1.2`.
- Шаги после победы: HintGiantDead (GROUP_EMPTIED `room4_giant` → «Искрой [1] по синему диоду на западе») → ток по цепи Room4/West → рычаг ожил (`gnome_lever_powered`) → HintGnomeLever («пошли артель к рычагу»).

#### 5.26.6 Room5 = «первая работа под угрозой»: мост-стройка (`grove_gate.gd`, ПИВОТ 2026-07-07)
Урок командования артелью + производственная цепочка. **ПИВОТ (юзер, 2026-07-07): готовой плашки в Room5 НЕТ — доска СТРОИТСЯ из брёвен**, «работа под угрозой» получила цель-производство (диагноз «жидко»: игрок был зрителем — один приказ и жди; идеи волн/камнеметателя отклонены как не решающие):
- **ЕДИНАЯ ВАЛЮТА (юзер, 2026-07-07: «мешанина — для моста дерево, для стройки деньги»):** дерево-как-валюта убрано из живого пути. **Бревно продаётся казне на СДАЧЕ у башни** (`SoldierGnome._tick_deposit_to_store`: `_carried_type==WOOD` → `GoldBank.add_coin(BRONZE, WOOD_SALE_BRONZE=4)` + разряд-вспышка, склад не участвует; руда/камень по-прежнему едут в трюм для города). Рубка = ЗАРАБОТОК, гном с бревном = живые деньги в пути. Экономическая арка: стол торга Room4 опустошает казну → Room5 учит зарабатывать.
- **`RoomBuildings.BRIDGE_PLANK`** («Мостки», секция «🌉 ИНЖЕНЕРИЯ» палитры, гейт только знанием — замка НЕ требует): **`cost={BRONZE:30}`** (≈8 сданных брёвен; обычная монетная оплата), `build_time=6`, `forbid_on_chasm=true` — блюпринт **не ставится на полосе пропасти** (`_chasm_blocked`: центр силуэта в локальных осях шейпа `chasm_barrier` → `_blocked`, силуэт красный). Продукт стройки = `scenes/bridge_plank.tscn` (RigidBody-плашка со всей физикой защёлка, группа **`built_plank`** — триггер хинта «доска готова»). Смысловое разделение: **гномы производят вещь — рука её применяет** (стройка не умеет «через дыру»).
- **АРТЕЛЬ: старт 1 → покупка 6 у гномов (юзер, 2026-07-07).** `GnomeSquadSpawner.starting_worker_count=1` (был кап 7); стол торга у Загадочного гнома продаёт РАБОЧИХ, не копейщиков: ветка диалога «Мне бы рабочие руки» (req `signed`) → effect `open_artel_trade` → `TradeUI.open(ROLE_WORKER)`; `TradeUI.HIRE_SIZE={worker:6}` (долив по капу 7). Боевых отрядов в туториале нет — армия приходит из казарм города.
- **`GroveGate`** (узел Room5; дети-Marker3D `FlankNorth/South`): первый удар топора → хинт «шум разбудил» + волна `first_chop_wave=4`; каждое срубленное дерево → волна `tree_felled_wave=6` (фланги поочерёдно, forced-цель=башня, урон 3); **казна ≥ `coins_ready_threshold=30`** (поллинг Timer 0.5с — GoldBank пополнение не сигналит; гейт «после начала рубки», чтобы богач не получил хинт на входе) → хинт «Монет хватает → Стройка → Мостки».
- **`WoodSource` сигналы** `chopped(remaining)` / `depleted` (эмиты в `gnome_hit`/`_break`) — для сценариев комнат.
- ⚠ **Дебаг-капиталы обнулены** (обходили экономическую арку): `TowerStore.debug_start_amount` 30→0, `GoldBank.start_gold` 100→0. Для тестов города — крутить в инспекторе.
- Цепочка хинтов: HintArtel (вход: «мост придётся сколотить → клик по дереву») → ambush → coins_ready → HintPlankReady (GROUP_APPEARS `built_plank`: «схвати рукой и положи поперёк»). Рифма с Room2: там мосток НАШЁЛ, тут ПОСТРОИЛ.
- `SkeletonFiller6`: 26→10, только **дальний берег** (center (43,0,112)) — арьергард ждёт перехода по мосту.
- Зона добычи башни `gather_radius=16` — башня должна стоять возле рощи.
- С читом «Бесплатная стройка» (`RoomBuildSite.free_build`, дефолт ВКЛ) доска достраивается мгновенно, но дерево всё равно списывается.

### 5.27 Акт II: Долина-хаб — замок ур.2, чертёж Врат, три вылазки, мех-финал (ДИЗАЙН 2026-07-07, утверждён; НЕ реализовано)

Долина превращается из «песочницы с чеклистом» в полный акт: **хаб** (город вокруг замка) → **развитие замка до уровня** → **чертёж из 3 элементов** → **три вылазки в соседние комнаты** (любой порядок) → **мех-страж Врат** → победа акта. Развилки утверждены юзером: артефакт = **Врата наружу**, уровень замка = **квартал-по-соседству**, вылазки = **любой порядок**.

#### 5.27.1 Драматургия и фазы
1. **Город** — текущая цепочка ValleyQuests (станок → замок → шахта → дом+казарма → 5 стен → верфь+срез → пережить ночь) остаётся как есть.
2. **Замок ур.2** — замок становится продюсером квартальной модели (как шахта/институт): обстроил его грани постройками → «Замок ур.2». Ноль новых механик — язык кварталов игрок уже выучил. Достижение уровня открывает чертёж.
3. **Чертёж Врат** — в северной стене долины стоят древние **Врата** (руина с 3 пустыми гнёздами): выход из долины дальше по миру, механизм сломан. Чертёж показывает 3 недостающих элемента; в периметре открываются 3 прохода к соседним комнатам, чеклист показывает блок «Элементы: N/3».
4. **Финал** — третий элемент вставлен → механизм оживает → из врат выходит **мех-страж** (EnemyMech, соло-дуэль, СТРОГО 1 — см. канон). Победил → врата открываются, башня уходит в проём = конец акта (задел на акт III). Старый финал «наполнить замок» уходит из win-condition (казна остаётся экономическим движком).

#### 5.27.2 Три вылазки = экзамены по глаголам туториала
Каждая комната — усложнённый экзамен по одному глаголу арки Room1–5; элемент = крупный grabbable-предмет (паттерн `RelayItem`), гнёзда Врат = сокеты (паттерн `RelaySocket`) — вставка рукой, ПЕРЕИСПОЛЬЗУЕМ цепочку Room3, не плодим сущности.
- **Комната А «Разлом» (север-запад, за z=43)** — экзамен руки: широкий разлом/завал, элемент («Ключ-плита») на той стороне, мостки + перенос под давлением врагов.
- **Комната Б «Застава» (север-восток, за z=43)** — экзамен искры+цепи. ДВУХЭТАПНАЯ: мелкая часть (станок-чертёжник, квест базы — см. 5.27.4) открыта с самого начала; глубинная часть — элемент («Звёздный кристалл») за электро-дверью — на фазе чертежа Врат.
- **Комната В «Подземелье гномов» (восток, за x=92)** — экзамен командования: вход «под гнома» (паттерн проёма из §5.22 — башня физически не лезет), артель уходит внутрь и тащит элемент («Шестерня»), башня снаружи **обороняет вход** от волн. Самый новый геймплей акта.

#### 5.27.3 Порядок реализации (малыми шагами)
① база: закладка замка чертежом (СДЕЛАНО, см. 5.27.4); ② врата-руина + 3 гнезда + расширение ValleyQuests (СДЕЛАНО, см. 5.27.6); ③ ~~замок ур.2 + чертёж Врат как гейт проходов~~ **ВЫПИЛЕНО пивотом 2026-07-07** (проходы = завалы, разбор рукой; замок ур.2 — под вопросом); ④ комнаты по одной с плейтестами — Подземелье гномов СДЕЛАНО v1 (см. 5.27.7), остались «Разлом» (СЗ) и глубина заставы («Звёздный кристалл»); ⑤ пробуждение меха + финал-открытие врат (`GateRuin._on_awakened`).

#### 5.27.4 База: закладка замка чертежом (РЕАЛИЗОВАНО 2026-07-07)
Пивот юзера: замок ставится на **ПРИВЯЗАННУЮ точку**, не карточкой палитры; вход в акт — через Гильдию Камня. Цепочка базы:
- **Гильдия Камня** (домики ЮВ): диалог `builders` переписан — **наём артели одним кликом** прямо тут (реюз effect `hire_artel`, БЕЗ Хартии — своя гильдейская артель, точка пополнения рабочих в долине) + квест «чертёж замка печатает станок на заставе». Факт разговора → домик вешает на себя группу `guild_met` (шаг чеклиста).
- **Застава** (НОВАЯ комната СВ за северной стеной: x 58..90, z 18..43 — расширена 2026-07-07; проём = снесённый `WallNorthMid` x 70..77; гарнизон-филлер 8 скелетов): сюда **переехал станок-чертёжник** из центра долины.
- **Пазл станка = ФИЗИЧЕСКАЯ РАСТОПКА (пивот юзера 2026-07-07, «Саймон» из 3 диодов выпилен):** три глагола игрока строгой цепочкой, каждый шаг открывает следующий + плашка ведёт дальше. ① **ТОПКА** — кучка угля у западной стены (`CoalLump`, `coal_lump.gd/.tscn` — Grabbable-ком, паттерн RelayItem без гнезда); рука кидает в приёмник (`Area3D Intake`, маска ITEMS, body_entered по группе `coal_lump`), нужно `coal_needed=2`, пасть топки разгорается ступенчато (эмиссия `Mat_mouth`). ② **КОНТАКТ** — до растопки диод `locked` (Искра не засчитывается); топка гудит → расглушён, бей Искрой [1]. ③ **ПУСКАЧ** — контакт запитан → `Lever.enable()`, дёрнул → `_ignite`: знание + печать чертежа. `blueprint_machine.tscn` пересобран (Core + Furnace{Body,Mouth,Chimney,Intake} + Diode + Lever); демо/последовательность/`flash_hint`-цикл из скрипта удалены. E2E: 4 угля в сцене, диод заглушен → 2 угля → расглушён → искра → пускач ожил → чертёж + знание ✓.
- **Печать чертежа**: `BlueprintMachine` получил `@export print_scene/print_hint` — на `_ignite` (вместе с `unlock_building`) выталкивает вперёд предмет-чертёж. **`CastleBlueprint`** (`castle_blueprint.gd/.tscn`) — grabbable-«синька» (светящаяся плита + мини-башенка), паттерн RelayItem: release в `snap_radius=3` от фундамента → вклад.
- **`CastleFoundation`** (`castle_foundation.gd` + **`scenes/castle_foundation.tscn`** — АПДЕЙТ 2026-07-07: визуал (Plate+Ring) перенесён из кода в УЗЛЫ СЦЕНЫ, фундамент видим во вьюпорте редактора и двигается мышкой; позиция теперь **(56, 84)** — центр долины, юзер расставляет сам): плита-постамент + пульсирующее кольцо в тон «синьки». `seat(blueprint)`: доводка-поглощение предмета → FX+шейк → спавн `RoomBuildSite` c `building_id=PUMP` на снап-клетке (артель строит обычным путём; чит free_build — мгновенно) → **фундамент queue_free** (АПДЕЙТ 2026-07-07: плита ИСЧЕЗАЕТ после закладки, раньше оставалась постаментом).
- **Качалка-замок УБРАНА из палитры** (BUILD_SECTIONS «Замок»); гейты PUMP в hud/hand_place_aim оставлены мёртвыми (id больше не в меню).
- ⚠ АПДЕЙТ 2026-07-11 (§5.30): чертёж больше НЕ вкладывается в фундамент — рука кладёт его на ВЕРХ БАШНИ → `CastleBlueprint.learned` навсегда, карточка «Замок» в панели («вне колоды»); замок ставится рукой ТОЛЬКО на фундамент (силуэт липнет, commit → `foundation.consume()`; `seat`/`_commit_build` удалены). Квест-цепочка станка (растопка → печать) не тронута.
- **ValleyQuests**: первые шаги = Гильдия (`guild_met` ИЛИ знание — каскад не залипает) → чертёж (`building_unlocked`) → фундамент (`_castle_started`: площадка PUMP или готовый замок — free_build схлопывает площадку за кадр, ловим итог) → замок построен; дальше прежняя цепочка (шахта → дом+казарма → …). Хинты: `HintZastava` (зона у проёма), `HintFoundation` (зона у плиты, `suppress_group=castle`).
- E2E headless: ignite → печать → seat → Castle → чеклист-шаг 4 ✓ (временный dev-узел, удалён).

#### 5.27.5 Здания: единая смерть-взрыв + уязвимый замок (2026-07-07)
Юзер: «все здания получают урон и взрываются при уничтожении». Аудит: PadBuilding/RoomBuildSite/PalisadeSegment урон принимали (смерть тихая или без FX), **замок был неуязвим вовсе** («слой обороны позже»).
- **`ShatterEffect.building_explosion(parent, pos, color, radius, fragments)`** — ЕДИНАЯ смерть здания: `AoeVisual.spawn_explosion` + пыль + обломки в цвет постройки + `camera_shake` по размеру. Один язык на все типы.
- `PadBuilding._die`: взрыв масштабируется числом клеток маски (будка ~1.6 → стена-брус ~2.6, кламп 1.6..3.0) + прежний шаттер в цвет роли.
- `RoomBuildSite._fail`: слабый взрыв (r=1.5, 8 обломков) в цвет призрака — раньше недострой исчезал молча.
- `PalisadeSegment`: добавлен **hit-flash** (не было; красный, как у PadBuilding/врагов, не бьётся с hover-подсветкой) + смерть-взрыв в цвет дерева.
- **Замок (`oil_collector.gd`) стал уязвим**: `@export hp=600` (стены 140 — замок танк, но не вечен), `Damageable.register` + `Enemy.TARGET_GROUP` в `_ready` (бродяги и штурм реально ломают), `take_damage` + hit-flash по всем мешам (материалы шарятся — красятся разом), `_die`: выход из ВСЕХ групп до queue_free ([[reference_godot_queue_free_deferred]]) → большой взрыв r=5 + 26 обломков + screen_flash + плашка «⚠ Замок разрушен!». Сигналы `damaged/destroyed`.
- ~~⚠ Осознанная дыра: замок НЕ восстановить (чертёж и фундамент one-shot)~~ ЗАКРЫТО дважды: перезакладка §5.27.10 (фундамент возрождается) + чертёж-навсегда §5.30 (learned, карточка оживает сама). Поражения-оверлея по-прежнему нет.
- E2E headless: замок damageable, hp 600→550 от удара, смерть=группа пуста; палисад и площадка умирают со взрывом без ошибок ✓.
- **ФИКС (фидбек «скелеты не атакуют замок, на зданиях нет телеграфа»):** широкая цель без reach-бонуса недостижима — скелет упирается в коллайдер, `dist ≤ attack_range` до ЦЕНТРА не срабатывает → ни замаха, ни телеграфа, ни урона. ① Замок объявил `get_attack_reach_bonus() = 2.4` (корпус 4.6). ② PadBuilding — **направленный** вариант контракта `get_attack_reach_bonus_from(from_pos)`: бонус = |до origin| − |до ближайшей точки футпринта маски| (XZ в локальных осях; у стены-бруса origin в торцевой якорь-клетке — скаляр дал бы замах по воздуху у тонкой грани). ③ `Enemy.target_reach_bonus` и `AoeDamage.apply_uniform` предпочитают направленный метод, скалярный — фоллбэк (башня/харвестер не тронуты). E2E: скелет с форсед-целью бьёт замок (600→566), reach стены у дальнего торца 5.00 / сбоку 1.00 — точно по геометрии ✓.
- **ФИКС №2 — LOS-САМОЗАСЛОН (фидбек повторно «скелеты не атакуют замок»):** reach починил только forced-путь; vision-скан замок по-прежнему отбрасывал. Причина: замок на слое `PALISADE_OBSTACLE` (544 — блокирует башню физикой), а LOS-луч `_has_line_of_sight_to` светит именно по этой маске → луч к центру замка бился в ЕГО ЖЕ коллайдер = «цель за стеной». Фикс: цель исключается из луча (`q.exclude += target.get_rid()` для CollisionObject3D) — сама цель преградой не считается. E2E: скелет БЕЗ форсед-цели через vision сам атакует замок (600→556) ✓.
- **Телеграф урона зданий (фидбек «нужен телеграф, что получают урон»):** старый hit-flash (0.9 энергии / 0.08с) не читался днём с высоты камеры. Теперь: ① **МИГАНИЕ** — двойной пульс вспышка→притух→вспышка→погас (~0.35с; апгрейд по фидбеку «пусть здание мигает» — одиночное затухание не читалось; один Tween через tween_method на все материалы разом, ретриггер убивает предыдущий `_flash_tween`; PadBuilding/замок/палисад; родное свечение построек — кристалл института, заливка замка — не глушится: отличаем по цвету `_FLASH_COLOR`); ② **крошка-пыль** при ударе (`spawn_dust`, throttle 400мс на здание — осада не заспамит партиклами); ③ **дым-столб при HP<35%** — постоянный статус «зданию плохо», виден издалека: `AoeVisual.make_smoke_emitter()` (continuous-вариант дыма взрыва, общий конфиг `_smoke_ppm/_smoke_quad`), ребёнок здания — умирает вместе с ним. E2E: дым встаёт, блинк без ошибок при ретриггере ✓.
#### 5.27.6 Фаза ②: врата-руина + гнёзда + завалы (РЕАЛИЗОВАНО 2026-07-07)
- **`GateRuin`** (`gate_ruin.gd`, `scenes/gate_ruin.tscn`, в `level_rooms.tscn` на **(50, 0, 43)** — врезан в северную стену долины, проём x 44..56): две колонны + плита с 3 тусклыми рунами + сломанная перемычка. Коллизия — дочерний StaticBody `Slab` со скриптом [Blocker] (слой 544 + hand_immune/slam_damage_immune/navmesh_source — как стены периметра). Группа `gate_ruin`.
- **Гнёзда = 3 инстанса `relay_socket.tscn`** у подножия со стороны долины — РЕЮЗ [RelaySocket] БЕЗ проводов (пустой `wire_from_path` → `_build_wire` тихо не строится, seat/unseat/is_seated работают как чистый сокет). Элементы вылазок (фаза ④, паттерн [RelayItem]) сядут в них рукой, любой порядок, в любое свободное гнездо (`_nearest_socket` по группе `relay_socket`).
- **Счёт — поллинг** (Timer 0.5с, как ValleyQuests; RelaySocket сигналов не эмитит, плодить не стали): рост счёта → вспышка + плашка «Руна Врат вспыхнула: N/3», руны зажигаются слева направо (у каждой СВОЙ материал — иначе зажглись бы разом). 3/3 → `_awake` (одностороннее, вынутый элемент не «усыпляет»), шейк+взрыв+плашка «древний страж просыпается…» — фаза ⑤ повесит на `_on_awakened` спавн меха и открытие створ. API: `elements_count()/elements_total()/is_awake()`.
- **Завал = валуны, разбираемые РУКОЙ (пивот юзера 2026-07-07, «чертёж Врат» как гейт проходов ВЫПИЛЕН):** `RubbleStone` (`rubble_stone.gd`, паттерн CoalLump) — тяжёлый Grabbable-камень (mass 20, ITEMS), **изначально `freeze=true`** (куча стоит монолитом — башня/скелеты упираются); захват рукой морозит/разморозит штатно (`HandPhysical`), растаскал камни — проход свободен. Ноль счётчиков и триггеров, чистая физика. Камни НЕ в navmesh_source и не в масках гномов (TERRAIN-only) — **гномы просачиваются сквозь завал всегда**: завал гейтит только башню (осознанно: артель может работать за завалом до расчистки). `rubble_pile.tscn` = Node3D + 6 валунов разных размеров (scale на RigidBody нельзя — размер через `@export size`, меш+коллизия кодом). Три кучи в сцене: **RubbleNorthWest + RubbleNorthWest2 (28/30.5, 43)** — проём x 26..32 к комнате А «Разлом» (стена `WallNorthWest` разрезана: 18..26 + `WallNorthWestB` 32..44); **RubbleEast (92, 56)** — проход к Подземелью гномов (рядом гнездо камнеметателя (85,53) — дорога под обстрелом, осознанно). Старый `rubble_pile.gd` (StaticBody + open()) удалён.
- **Стены**: `WallNorthLeft` ужат до x 56..70 (врата заняли 44..56). Периметр в остальном не тронут.
- **ValleyQuests**: шаг «Наполняй замок» (match_won) УБРАН из цепочки (§5.27.1 — старый финал уходит из win-condition; сам match_won в WaveDirector не тронут). После «пережить ночь»: шаг «осмотри руину Врат» (башня ≤16м XZ; вставленный элемент — каскад не залипнет) → шаг-блок **«Вставь элементы в гнёзда Врат: N/3»** — первый ДИНАМИЧЕСКИЙ текст: ключ `text_fn` (Callable():String) вместо `text`, `_tick` эмитит `valley_quest_changed` на ЛЮБОЕ изменение строки (кэш `_last_emitted`), не только на переход шага. Хинт-зона `HintGates` у врат.
- E2E headless: 3 фейк-посадки → count 3/3, awake=true; тексты новых шагов рендерятся (dev-узел удалён) ✓.

#### 5.27.7 Комната В «Подземелье гномов» v1 (РЕАЛИЗОВАНО 2026-07-07, пивот: без чертежа)
Пивот юзера: прослойка «замок ур.2 → чертёж Врат → открытие проходов» ВЫПИЛЕНА из критического пути — проходы это завалы (разбор рукой, §5.27.6), комнаты доступны сразу. Судьба замка ур.2 как фазы — ПОД ВОПРОСОМ (не реализовывать до решения).
- **Лабиринт** (`DwarfDungeon` в `level_rooms.tscn`): пристройка x 92..114, z 44..70 за восточным завалом. Вход = бывший шов `WallEastMid` — фактический проём **z 55.25..56.75 (~1.5м) «под гнома»**: башня физически не пролезает (корпус 4.6), рука с дистанции не дотягивается — лабиринт глубиной ~20м+. Змейка из 3 перегородок (`MazeWall1..3`: x=98 z 44..64, x=104 z 50..70, x=110 z 44..64; коридоры 6м): юг → север → юг → камера СВ. Периметр `WallNorth/East/South`. Всё из blocker_wall → navmesh_source, навмеш внутри печётся (пол 300×300 покрывает).
- **`GearElement` — Шестерня** (`gear_element.gd` **extends RelayItem**, `scenes/gear_element.tscn`, лежит на (112, 0.5, 47) в конце змейки): grab/сокет-снап/вспышка тока — целиком от RelayItem; свой визуал (диск+втулка+6 зубьев, один материал — highlight красит разом) и слой «гном-носильщик» (поллинг 0.3с): свободна и башня дальше `tower_drop_radius=7` → ближайший **гном-рабочий** (SOLDIER_GROUP + soldier_type==ROLE_WORKER) в `pickup_radius=1.8` взваливает на голову (freeze + слой MOUNTED_MODULE — рука может выхватить прямо с головы); гном несёт КУДА ИДЁТ САМ — движением рулят обычные команды артели («Идти сюда»/F), автопилота нет; башня ≤7м → скидывает (плашка «хватай рукой и неси в гнездо»). Петля: артель в лабиринт → подбор → отзыв к башне → сброс → рука → гнездо Врат.
- Хинт-зона `HintDungeon` у проёма («Проём под гнома — пошли артель»).
- E2E headless: 18 валунов (3 кучи × 6); стаб-гном worker подобрал шестерню → донёс до башни → сброс; release «рукой» у гнезда → seated, счётчик Врат 1/3 ✓ (dev-узел удалён).

#### 5.27.8 Город = тело башни: док, гейт маны, язык карточек (2026-07-07)
Тезис юзера («геймплей за башню и за город разрозненные — связать воедино»)
+ его же парковая идея «база конфигурирует башню»: город — не вторая игра, а
СИСТЕМА ЖИЗНЕОБЕСПЕЧЕНИЯ башни. Каждое здание — орган: институт=мана-жила,
казарма=экипаж, верфь=обвесы, шахта=кошелёк, док=причал.
- **Док города** (бывш. «Разгрузочная платформа», PAD_UNLOAD → «⚓ Док города»):
  парковка (XZ ≤ UNLOAD_RADIUS=6) теперь даёт три сервиса: трюм в казну (как
  раньше) + **тихий ремонт корпуса** (`Tower.repair()`, DOCK_REPAIR_RATE=12 hp/с,
  без FX рабочего) + **пуповина маны** (см. ниже). Плашка «⚓ Башня в доке…» на
  входе (переход `_docked_prev`). Хелперы `is_tower_docked()` /
  static `is_tower_docked_any(tree)` / `has_dock_pads(tree)`.
  ⚠ АПДЕЙТ 2026-07-11: PAD_UNLOAD недоставаем (вне колоды, §5.30) — трюм-сдача
  заменена авто-конверсией; ремонт-у-причала вместе с плитой недоступен, перенос
  сервисов на верфь — PENDING.
- **МАНА-ПИВОТ:** `_tick_institute` гейтится доком — институт заряжает башню
  ТОЛЬКО пока она у причала; в поле мана добывается ТОЛЬКО орбами с убитых
  (день-вылазка на батарейке, вернулся — подключился). Фоллбек: в сцене нет
  ни одной док-плиты → старый глобальный поток (чит-пути не софтлочим).
- **Язык карточек**: hint'ы каталога переписаны с «экономики» на «что это даёт
  башне/руке» (замок «сердце города башни», шахта «кормит казну башни», дом
  «+4 гнома: руки для шахт и войск», казарма «„В башню“ — сажает на борт»,
  институт «мана-жила башни», верфь «мастерская башни: обвесы-срезы»).
- E2E: парковка → hp 900→918 за 1.5с; увезли плиту → `is_tower_docked_any`=false ✓.
- PENDING (следующие шаги связки): видимые лучи «город кормит башню» в доке;
  кузня = прокачка руки; скупщик.

#### 5.27.9 Долина ×2, часы дня, акцент заката (2026-07-07, фидбек «места маловато / теншен рваный»)
- **Долина увеличена почти вдвое**: восток x 92→118, юг z 126→152 → прямоугольник **x 18..118, z 43..152 (100×110)**, центр (68, 97.5). Обновлены: `ValleyQuests.valley_center/size`, room-гейт камнеметателя, `SpawnZoneValleyEast` → (112,108) 9×40. Лабиринт Подземелья, RubbleEast и HintDungeon переехали +26 по x (вход теперь x=118; гнездо камнеметателя (85,53) — внутри долины, сторожит дорогу на восток). Стены: WallNorthRight 77..118, Room8/Room6 East на x=118 (Room6 удлинён до z=152), WallSouth 55..118 на z=152, **WallSouthWest x=55 z 126..152** (без неё в юго-западе была дыра). **Ground (300×300) переехал в (40,40)** — у восточного края запас 50м, «стен на краю мира» больше нет; трава (`coverage_target_path`) и навмеш едут за полом.
- **Часы дня на HUD** (`DayClock`, узел в gameplay_hud.tscn, центр-верх): «☀ до заката N:NN» (янтарный) / «🌙 до рассвета N:NN» (голубой); данные — `WaveDirector.get_day_night_remaining()` (поллинг в throttled-блоке `_process`), скрыты вне кампании. Ночь = метроном акта, дедлайн всегда на виду. **Закат отсчитывается ОТ СТАРТА** (решение юзера: логичнее; про ночь надо ГОВОРИТЬ, а не двигать её).
- **Плашки заката** (`WaveDirector._tick_day_night`, `_sunset_warn_stage` сбрасывается на рассвете — акцент КАЖДЫЙ день): за 60с «☀ Солнце садится…», за 15с «🌙 ЗАКАТ! Займи оборону», на смене фаз «🌙 Ночь. Держись до рассвета!» / «☀ Рассвет!».
- Врата-руина ужаты 9м → 5м (перемычка 9.3 → 5.6): 9-метровые колонны при стенах 2.5м читались как «стена висит в воздухе».

#### 5.27.10 Комната А «Разлом» v1, перезакладка замка, наполнение (2026-07-07, «набросали — улучшаем потом»)
- **Комната А «Разлом»** (`RiftRoom` в level_rooms, СЗ за z=43: x 20..44, z 18..43, вход = проём x 26..32 за завалами): разлом поперёк комнаты (фейковая пропасть по канону — `ChasmVisualA` тёмный бокс 24×6 на z≈31 + `ChasmBarrierA` невидимая стена 544, yaw 90° как Room2, шейп 6×26) делит комнату на южный «берег» игрока и северный с добычей. **`KeystoneElement` — Ключ-плита** (`keystone_element.gd` extends RelayItem, только визуал: каменная плита 1.2×0.3×0.8 + рунная полоса; grab/сокет-снап от базы) на (32, 22) за разломом, охрана 6 скелетов-филлеров + горшок. Готовый `BridgePlankA` (bridge_plank, штатный снап к барьеру) лежит на южном берегу. Экзамен руки: мосток поперёк → башня переезжает → рука берёт плиту → в гнездо Врат. Хинт `HintRift` (suppress_group=bridge_snapped). Существуют 2 из 3 элементов (Шестерня+Ключ-плита); третий — «Звёздный кристалл» заставы, PENDING.
- **ПЕРЕЗАКЛАДКА ЗАМКА (решение юзера: потеря замка ≠ софтлок, «ставь новый»):** `Castle._die` спавнит `castle_foundation.tscn` на своём месте + перевзводит пускач станка (`BlueprintMachine.reprint_arm`: `Lever.reset()` — новый метод, тумблер обратно в OFF; пазл растопки повторно проходить НЕ надо). Повторный дёрг → `_reprint` с гейтами: замок жив → «чертёж не нужен»; чертёж уже валяется (группа `castle_blueprint`) → «найди его»; иначе печать нового. Группа `blueprint_machine`. День/ночь цикл от замка НЕ зависит (и не зависел); ночной штурм гейтится живым замком, как раньше.
  ⚠ АПДЕЙТ 2026-07-11 (§5.30): вызов `reprint_arm` из `Castle._die` УБРАН — чертёж выучен башней навсегда, новый предмет не нужен; фундамент по-прежнему возрождается, карточка «Замок» оживает сама. `reprint_arm`/`_reprint` в станке остались (мёртвый резерв).
- **Скелеты в вылазках**: `DungeonGuards` — 7 филлеров в лабиринте Подземелья; `RiftGuards` — 6 за разломом.
- **Юг долины наполнен** (после ×2 был голым): жилы `OreVeinSouth` (62,138) / `OreVeinSouthEast` (96,132), деревья ResWoodSouth ×2, горшки PotSouth 1-3 (один золотой) + PotRift в комнате А, филлер `SkeletonFillerSouth` 15 шт (75,140).
- **Лучи «город кормит башню»**: пока институт реально льёт ману (гном есть + башня в доке) — раз в 1.1с синий импульс `PlaceFx.link_pulse` от института к башне. Пуповина видна, вхолостую не светит.
- E2E: комната А на месте (плита/мосток/барьер), Ключ-плита села в гнездо (счёт Врат 1/3), станок имеет reprint_arm, юг: 3 горшка ✓.

#### 5.27.11 ФИНАЛ АКТА: Звёздный кристалл, мех-страж, открытие Врат, победа (2026-07-07)
Полный цикл акта II замкнут — все фазы §5.27.3 реализованы (в наброске, тюнинг по плейтесту).
- **Глубина заставы (`Zastava/StarVault`, x 62..86 z 2..18):** стена заставы z=18 разрезана (проём x 70..78), в нём **электро-дверь** (`MetalDoorStar`, инстанс-паттерн MetalDoor3: съезжает под пол + rebake навмеша). Цепь искры — реюз Room3: `BlueDiodeStar` (68,23) → провод → `RedDiodeStar` (71,20.5) → `LeverStar` (78,20.5) → дверь. Внутри **`StarCrystal`** (`star_crystal.gd` extends RelayItem: ядро-сфера + 6 шипов, яркая эмиссия) на (74,8) + 4 скелета охраны. Хинт `HintStarDoor`. ВСЕ 3 элемента Врат теперь существуют.
- **Мех-страж** (`GateRuin`): `_on_awakened` → `EventBus.boss_wave_incoming(mech_delay=3)` (BossWarningOverlay) → спавн `mech_scene` (enemy_mech.tscn, `@export` в gate_ruin.tscn) на 6м южнее врат; СТРОГО ОДИН (канон соло-apex). Подписка на его `destroyed` → `_open_gates`.
- **Открытие Врат:** плита-створ (`slab_path`) уезжает под землю (tween 2.2с, глубина 7) → СИНХРОННО в конце: снятие с navmesh_source + `nav.rebake()` + queue_free (паттерн MetalDoor — физика и навмеш согласованы). `is_open()`.
- **ПОБЕДА АКТА:** `_check_victory` в поллинге — башня в полосе врат (|x−50| ≤ 4.5, z ≤ 42.5) при открытых створах → `EventBus.match_won.emit()` → WinOverlay. **Старая победа «замок наполнен» ВЫКЛЮЧЕНА** (`Castle.add_oil` больше не эмитит match_won; `filled` остаётся). match_goal.gd — только в недостижимом main.
- **ValleyQuests** финал-шаги: «⚔ Срази СТРАЖА ВРАТ» (done: `is_open`) → «Веди башню в проём» (done: match_won, `_won` вернулся). Полная цепочка: гильдия → чертёж → замок → экономика → ночь → врата → 3 элемента → страж → выход.
- E2E полного финала: дверь съехала (y −3.4), кристалл на месте, 3 посадки → awake → мех вышел через 3с → kill → врата открыты, слэб исчез → башня в проёме → WinOverlay ✓
- ⚠ СУПЕРСЕДЕД 2026-07-08: механика «3 элемента в гнёзда» заменена ПЛАТНЫМ проходом (§5.28) — гнёзда выпилены, элементы стали артефактами-находками с доставкой в город.

### 5.28 Сюжет «Верхний Предел» — один заход, платный выход (2026-07-08, утверждён юзером)

Кристаллизация лупа: игрок — капитан Ладьи, выходит из гномьих пещер в
**Верхний Предел** (неспокойное графство Верхолазов), проверяет шахтёрские
города, нанимает команду, строит гномий форпост и **копит на плату за проход**
через древние Врата — домой, к подземной столице. Одна валюта до самого конца:
каждая трата на замок отдаляет возвращение.

- **Врата = ценник** (`GateRuin.exit_price_bronze`, деф. 900🥉): ЛКМ-клик по
  плите (гейты как `_clicked_on_self`), руны = одометр накопления по третям
  казны/цены. Баланс: доход ~2800-3300🥉 за 3 суток (2 развитых квартала),
  полный замок ~2100🥉 → выход на 3-и сутки. Сутки = 5 мин (день 180 + ночь 120).
- **ОПЛАТА БУДИТ СТРАЖА**: плата принята → предупреждение → мех-страж +
  `WaveDirector.launch_final_siege()` — штурм-банда с КАЖДОЙ живой зоны
  одновременно (`final_siege_band_size`, деф. 30), у каждой своя дуга.
  «Последняя ночь» наступает когда игрок заплатил; убежать тихо нельзя.
  Мех пал → створ → башня в проёме → победа (как раньше).
- **Артефакты вылазок** (`ArtifactElement` extends RelayItem, сокет-снап
  выключен, доставка к зданию по роли каталога + поллинг): пещера гномов
  (бывш. Подземелье) → 📜 **Рецепт** (gear_element.gd) → институт → Кафедра
  огня открыта в палитре (гейт `PlayerProfile.fire_recipe_found`; кафедра
  теперь даёт ТОЛЬКО fireball); храм на каньоне (бывш. Разлом) →
  ⚡ **Аккумулятор** (keystone_element.gd) → институт ур.2: Искра→МОЛНИЯ
  (spark level 2, `SpellSystem.grant_level` — бесплатный API артефактов) +
  Огненный шквал; застава → 💎 **Клад** (star_crystal.gd) → док → +300🥉.
- **Храм-пазл гарпуна**: каменная плита-мост (bridge_plank:
  `plank_mass=11` > лифт-фильтра руки 10, < разрыва верёвки 12;
  `auto_snap=true` — легла сама, когда гарпун дотащил до полосы) на СЕВЕРНОМ
  берегу. HarpoonBolt игнорит chasm_barrier в wall-луче (стрела летит НАД
  фейковой пропастью) и отпускает замороженную в полёте жертву.
- **Верх башни = инвентарь**: MountSlot (верхушка) паркует grabbable-груз
  группы `tower_cargo` (артефакты) — freeze+пин, снять рукой, один груз;
  у здания-приёмника артефакт всасывается прямо с борта.
- **Мастерская навесок** (бывш. Инженерная гильдия) — производство навесок
  (гарпун-турель 40🥉 кликом), как и было.
- Тексты: плашка въезда «⛰ ВЕРХНИЙ ПРЕДЕЛ», чеклист «Накопи плату:
  казна/цена» (динамический), хинты Врат/храма переписаны.

#### 5.28.1 Дуэль с мехом-стражем: заработанные окна + арка фаз (2026-07-09)

> 📄 Полный дизайн-док боя (профиль, арсенал, фазы, окна, шпаргалка
> инструментов, история изменений и открытые вопросы): **docs/mech_duel.md**.

Фидбек «бой с мехом — драфт». Диагноз: нет арки (ровный от старта до смерти),
нет заработанных окон (губка), арсенал игрока не спрашивается. Пересобрано по
формуле камнеметателя v4 («окно уязвимости надо ЗАРАБОТАТЬ»), каждое окно
спрашивает СВОЙ инструмент:
- **ПЕРЕГРЕВ (парри-рикошет):** отражённая парированием ракета/фаербол попала
  в меха (`note_reflected_damage` — уже звался телеметрией) → реактор открыт
  `overheat_duration=3с`: мех стоит, всё текущее гасится (залп/веер/евейд/
  телеграф), корпус раскалён (per-instance материал, язык «раскалён/потух»);
  урон в окне × `overheat_crit_mult=1.5` (override take_damage). Повторный
  перегрев не раньше `overheat_immunity=8с` после выхода (не станлочится).
- **ГАРПУН-ЯКОРЬ (v2 по фидбеку «дать время на расстрел»):** мех непротягиваем
  (канон), но верёвка ДЕРЖИТ его `tether_time=4с` (каталог спелла harpoon;
  новый State.TETHER болта). На привязи мех — ГАРАНТИРОВАННОЕ ОКНО РАССТРЕЛА:
  борется с верёвкой (дёрганье `tether_struggle_speed=3.5`), НЕ атакует и не
  уклоняется. Вырывание неизбежно: таймер вышел → мех рвёт канат РЫВКОМ
  (`_break_free`, плашка «вырвался»). Источник истины времени — мех (болт
  читает его `_tether_t`; в перегреве таймер привязи замирает — окна не
  рассинхронятся). Отмена ПКМ работает (can_cancel).
  **Якорь = класс «дуэлянты»** (duck-typing `set_tethered` — симметрия
  взаимодействий): КАМНЕМЕТАТЕЛЬ тоже якорится (борьба 2.5 м/с, очередь
  камней прерывается, перезарядка замирает, вырывание — реюз dodge-дэша
  ×1.5). Гигант БЕЗ set_tethered → протягивается верёвкой (PULL) как раньше.
  Попутно объяснился старый глюк «выбрал искру — гарпун слетел»: это был
  3с-кап волочения (max_pull_time), совпадавший по времени со сменой спелла.
- **ФАЗЫ по HP** (`_tick_phase`, пороги 0.66/0.33 от стартового hp):
  ф.1 «джеб-ритм» — только AIMED + простые залпы (без капкана и Шквала);
  ф.2 «перегруппировка» — + капкан-поле (kite-комбо) и Шквал;
  ф.3 «ЯРОСТЬ» — стрейф ×1.35, бит ×0.8, обычные залпы ДВОЙНЫЕ (супер не
  трогается) + **анти-черепаха**: башня вне kite_max_range дольше
  `turtle_timeout=6с` → мех лупит по ближайшей постройке города (≤45м,
  skeleton_target без гномов) с плашкой «вернись в бой». Переход фазы =
  вспышка-кольцо + плашка.
- E2E headless: рикошет → overheat 3.0/immune 11.0, крит в окне ровно ×1.5,
  болт в меха → привязь 2.5 (в перегреве таймер привязи замирает — окна не
  съедают друг друга), hp 20% → фаза 3 ✓. Баланс окон/фаз — по плейтесту..

### 5.29 ⭐ Дэш-форма заклинаний: обводка фигуры заряжает супердэш (2026-07-11, юзер: «одно из самых важных изменений»)

**Модель:** у заклинания ДВЕ атаки — дальняя (ПКМ-каст из башни, как было) и
ближняя «дэш-форма» (заряженный супер-рывок). Коммит `5f555fb`; апдейт
2026-07-12 `0a9238a` — фигура опциональна, синий визуал, линия дальности,
рывок-до-точки, золотой пульс-маркер заряда.

**Флоу (Tower.SDash FSM: IDLE → HOLDING → TRACING → AIMING):**
1. Тап Space = обычный рывок (не изменился). Зажатие ≥`super_dash_hold_threshold`
   (0.3с) → `_try_enter_sdash_trace()`: у экипированного заклинания
   (`HandSpell.equipped_spell_id()`, разблокировано) есть `dash_form` → TRACING;
   **нет формы (Искра, гарпун и др.) → ОБЫЧНЫЙ супер-дэш как раньше** (сразу
   AIMING без обводки, fallback @export 250 dmg / 50 маны, синий маркер, без
   импакт-эффекта). Гейт всегда один — мана на цену выбранного пути.
2. **TRACING (обводка):** слоумо 0.15 + затемнение; на земле фигура точек
   (`dash_form.points` — смещения от башни в camera-yaw пространстве, читается
   на экране одинаково при любой орбите; y=0.05 наземный). Курсор руки касается
   точек ПОСЛЕДОВАТЕЛЬНО (XZ-снап `super_dash_trace_snap`=1.6м); «не та» точка
   инертна, провалов нет. Отпустил Space — отмена, мана не тронута.
   **Фигура ОПЦИОНАЛЬНА (2026-07-12):** ПКМ прямо во время обводки = обычный
   супер-дэш в точку прицела с ЛЮБЫМ заклинанием (форма сбрасывается →
   fallback-цена/урон, без импакт-эффекта) — «мили-удар» доступен всегда,
   обводка лишь заряжает его заклинанием.
3. **Телеграф обводки:** маркер касания под курсором с первого кадра; НИТЬ по
   земле — живой сегмент от последней взятой точки к курсору, взятые отрезки
   застывают статикой (`AoeVisual.spawn_ground_line`/`update_ground_line` —
   переиспользуемая плоская линия, язык ground_ring); свечение под блум
   (эмиссия 5-8) + пульс реального времени (слоумо не глушит); взятые точки
   горят нарастающе (4.5+2.0×i) — визуал набора заряда.
   **Цвет (2026-07-12): весь визуал дэша ЕДИНЫЙ СИНИЙ** (`super_dash_marker_color`),
   НЕ `icon_color` заклинания — красный (фаербол) в прицеле читался как
   «опасность/ошибка» и фрустрировал. Импакт-эффект заклинания свои цвета
   сохраняет.
4. **Линия дальности + честный прицел (2026-07-12, TRACING и AIMING):** от
   подножия башни к ТОЧКЕ ПРИЗЕМЛЕНИЯ тянется линия + кольцо-маркер
   (`_update_sdash_range_visuals`, ленивый спавн, r=`super_dash_marker_radius`
   =1.4). Рывок летит К КУРСОРУ и ОСТАНАВЛИВАЕТСЯ в нём (`_dash_timer =
   dist/speed`); предел = `super_dash_speed`×`super_dash_duration` (~11.5м) —
   курсор дальше → круг клампится на максимуме и фиксируется, линия дальше не
   растёт. Коммит-цель = точка приземления («куда целюсь, туда лечу»);
   дистанция-гейт ДО списания маны (цель у подножия — ману не жжём).
5. Фигура обведена → «сверкнула» → AIMING: маркер приземления становится
   **ЗОЛОТЫМ** (`super_dash_charged_color`) и **ПУЛЬСИРУЕТ** (эмиссия +
   масштаб ±15%, реальное время) = «заряд готов»; без фигуры — статичный
   синий. ПКМ = коммит: списание `dash_form.mana_cost`, таран =
   `dash_form.damage`.
6. **Импакт:** естественный конец супер-рывка → `EventBus.super_dash_impact
   (position, spell_id)` → `HandSpell._on_super_dash_impact` диспатчит на
   модуль-владелец: **fireball** → `super_dash_burst` (взрыв-визуал + BurnPatch
   по level-данным; урон тарана уже нанесён контактами); **firestorm** →
   `super_dash_volley` (полный залп серии вокруг точки прибытия, без
   маны/кулдауна — цена уплачена дэш-формой). Рывок, прерванный отбросом меха,
   импакт НЕ эмитит (`_sdash_impact_pending` гасится).

**Акценты (фидбек «не хватает импакта», словарь существующих FX):** вход в
обводку = cast_charging-панч башни + волна цветом заклинания; точка взята =
кольцо + искры; фигура готова = вспышка экрана + шейк 0.25 + панч; запуск (ПКМ)
= вспышка + шейк 0.5 + панч + стартовая волна; путь = кольца-след каждые 0.05с;
прибытие = `HitStop.slowmo_beat(0.25/0.12)` + шейк 0.55 + пыль + кольцо, ПОТОМ
импакт-эффект заклинания (дубль-шейк из fireball-burst убран).

**Данные (`SPELL_CATALOG[id].dash_form`, опционально):** `points:
Array[Vector2]` (x=вправо, y=вглубь экрана отрицательным), `mana_cost`,
`damage`. Фаербол: 2 точки-«росчерк» перед башней, 50/250. Шквал: пентаграмма
5 точек (порядок обхода звездой — нить рисует пируэт), 75/200 + залп.

**Не тронуто:** старый экранный QTE «Великий удар» (`hand_super.gd`, заперт) —
его язык нити/точек мигрировал в обводку; хардкод `super_dash_*` @export'ов
Tower остался fallback'ом. Чит «Все заклинания» (журнал → Читы) открывает весь
каталог КРОМЕ `super`; все чит-кнопки получили ✓-вспышку подтверждения
(`_flash_cheat_button`).

**PENDING (дизайн):** дэш-формы для мороза/гарпуна/мин; эволюция формы по
уровням заклинания; баланс мана-к-урону заряженного vs обычного супердэша.

### 5.30 ⭐ Колода карт стройки: выдача зданий за ману (2026-07-11, коммит 55d051b)

Пивот юзера («упростить стройку до того, что мы хотели»): МОДЕЛЬ города не
тронута (грид/кварталы/продюсеры+филлеры/население, §5.22.9+), заменена МОДЕЛЬ
ВЫДАЧИ — вместо «выбрал в палитре → купил за монеты» здания приходят КАРТАМИ из
колоды за МАНУ башни. Развод валют: **мана = рост города, монеты = рост башни**
(найм, срезы верфи, кафедры, Врата).

- **Колода**: фикс-набор на заезд с дублями (`DECK_RECIPE` в `gameplay_hud.gd`,
  ~45 карт, пары [menu_id, count]); перетасовка в `_deck_init` (лениво при
  первом открытии панели). Разгрузочной платформы (PAD_UNLOAD) в колоде НЕТ —
  см. «ресурсы прибывают сами» ниже.
- **Взятие**: кнопка «🎴 Взять карту — 20 🔮 (рука N/5, в колоде M)» (бывший ряд
  вкладок Tabs) — `Tower.try_consume_mana(CARD_DRAW_MANA=20)` → верхняя карта
  колоды в РУКУ (вслепую: «что попалось, то и попалось»). Кап руки
  `CARD_HAND_LIMIT=5` — полная рука давит «ставь или сбрасывай». Кнопки
  live-гейтятся по мане (`_refresh_deck_buttons` из `_refresh_tower_mana`).
  Чит free_build обнуляет цены карт (draw и discard).
- **Рука**: карточки прежнего вида (превью формы/имя/эффект; БЕЗ монетной цены —
  уже оплачено маной), гейты живы (нет качалки/института/рецепта → серая с
  причиной, карта лежит и ждёт). Клик → `HandPlaceAim.start_aim(id,
  prepaid=true)`: монетная цена НЕ проверяется/НЕ списывается, sticky выключен
  (одна карта = одно здание, aim закрывается после установки). Карта уходит из
  руки ТОЛЬКО по факту установки (новый сигнал `HandPlaceAim.placed` →
  `_on_card_placed` по `_pending_hand_idx`); Esc-отмена оставляет карту в руке.
- **Сброс**: ✖ на карте руки, `CARD_DISCARD_MANA=5` — карта возвращается В
  КОЛОДУ в случайное место НИЖЕ залоченного верха (НЕ сгорает: уникальные
  институт/лабы потерять нельзя, сброс = мягкий реролл).
- **Стартовая тройка** (`DECK_OPENING`): первые взятия ВСЕГДА шахта → институт
  магии → верфь-док (экземпляры вынимаются из перетасованной колоды и кладутся
  наверх, не сверх рецепта). Эти ТРИ ЭКЗЕМПЛЯРА залочены от сброса
  (пер-экземплярный `_card_hand_locked`; юзер явно сузил с «типа» до «первых
  трёх» — обычные копии шахты из случайной части сбрасываемы).
- **⭐ ПРАВИЛО ключевых зданий (юзер):** всегда достижимы минимум 1 шахта +
  1 институт + 1 верфь + 1 замок. Здание уничтожено БОЕМ (`PadBuilding._die` →
  новый `EventBus.pad_building_destroyed(building_id)`), и этого типа не
  осталось ни на поле (включая недострой в BUILD_SITE_GROUP), ни картой в
  руке/колоде → карта ложится НАВЕРХ колоды: выпадет СЛЕДУЮЩИМ взятием,
  залочена от сброса; если лежала глубоко в колоде — поднимается наверх без
  дубля. Механизм — `_deck_locked_top`: единый счётчик залоченного верха
  (стартовая тройка + гарантийные возвраты); сбросы и возвраты от сноса
  вставляются строго НИЖЕ него.
- **ПКМ-снос** (в режиме стройки) → карта здания возвращается в колоду ВМЕСТО
  монетного рефанда (`HandPlaceAim.demolished` → `_on_building_demolished`;
  `_refund_building` монеты больше не возвращает). Гибель НЕключевого здания от
  врагов карту НЕ возвращает (осознанно; ключевые — по правилу выше).
- **Замок ВНЕ колоды**: чертёж (`CastleBlueprint`) больше НЕ вкладывается в
  фундамент — рука кладёт его на ВЕРХ БАШНИ (XZ ≤ 2.5 до `tower_top_slot`, тот
  же жест, что груз MountSlot) → **`CastleBlueprint.learned` (static, навсегда)**,
  предмет растворяется с искрами. Карточка «Качалка-замок» — в секции «вне
  колоды» панели (рядом с мостками; до чертежа серая «🔒 нужен чертёж замка»).
  Ставится ПО-ПРЕЖНЕМУ только на фундамент (юзер явно оставил привязку): силуэт
  ЛИПНЕТ к незанятому `CastleFoundation` (`FOUNDATION_SNAP_RADIUS=6` в
  hand_place_aim), вне радиуса — красный запрет; commit → `foundation.consume()`
  (FX + плита исчезает; старые `seat`/`_commit_build` удалены). Замок разрушен →
  фундамент возрождается на руинах (Castle._die, §5.27.10), НО перепечатка
  чертежа станком УБРАНА (`reprint_arm`-вызов выпилен — learned навсегда):
  карточка оживает сама (гейт `_pump_exists_or_building`). Софтлок «замок не
  восстановить» (§5.27.4) ЗАКРЫТ.
- **Ресурсы прибывают САМИ** (ритуал сдачи убран, юзер: «мы больше не сдаём»):
  `tower_store.deposit` конвертит всё, КРОМЕ дерева, в монеты прямо в момент
  прибытия (орб-магнит/ноша гнома) — `GoldBank.smelt_yield` (та же математика,
  что была у разгрузки) + попап «+N🥉» бронза-эквивалентом у башни. Дерево —
  стройматериал (мостки), остаётся буфером с капом; брёвна и так продаются
  гномом напрямую (§5.26.6). Парковка для сдачи не нужна; код unload-плиты жив,
  но здание недоставаемо — ⚠ вместе с ним недоступен ремонт-у-причала
  (§5.27.8); перенос док-сервисов на верфь — PENDING за юзером.
- **UI**: панель `BuildPalette` переиспользована (Title «🎴 СТРОЙКА — КОЛОДА»,
  Tabs-ряд = кнопка взятия, Sections = рука + «··· вне колоды»: мостки, замок);
  вкладки BUILD_SECTIONS удалены. `passive_mana_regen` башни ВКЛЮЧЁН в
  level_rooms (мана на карты нужна до института).
- **PENDING**: тюнинг цен (20/5) и состава DECK_RECIPE по плейтесту; судьба
  среза «Грузовой ярус» (кап трюма почти не нужен без сдачи).

### 5.31 Лучник = носимое оружие башни: VS-автострельба с крыши + XP за убийства (2026-07-12)

Продолжение линии «носимых оружий» (гарпунная турель §5.28): **любого живого
лучника можно взять рукой и поставить на крышу башни** — он автострелятет сам
(VS-режим). Слот на крыше ОДИН и **заменяет экипаж-3 «В башню»** (решение юзера).

- **Захват живого юнита — `scripts/unit_grab_handle.gd` (`UnitGrabHandle`)**:
  невидимая «ручка»-RigidBody3D при юните (mask=0, freeze, живёт в сцене и
  следует за юнитом). Слой ДИНАМИЧЕСКИЙ: на земле FRIENDLY_UNIT (курсор не
  прыгает по гномам), НА КРЫШЕ — MOUNTED_MODULE + сфера 0.28→0.7 (фикс первого
  плейтеста: курсор-raycast руки (маска 67) FRIENDLY_UNIT не видит → рука не
  взбиралась на крышу и GrabArea не дотягивалась — снять было нельзя; с
  MOUNTED_MODULE снятие = в точности механика съёма гарпуна). Даёт руке
  штатный Grabbable-контракт —
  `hand_physical.gd` НЕ правился ни на строку. Подсветка транслируется юниту
  (`ArcherSoldier.set_highlighted` — эмиссия на материалах бокс-модели).
  Контракт юнита: `begin_hand_carry / carry_follow / end_hand_carry /
  try_mount_on_tower / is_carry_available` (duck-typing; переиспользуем для
  будущих носимых юнитов). Спрятанный в башню лучник не хватается (layer=0).
- **Монтаж**: отпустил в радиусе 3.5 от башни (XZ, как у гарпуна) →
  `try_mount_on_tower`: слот занят другим лучником → hint и обычный дроп.
  Установленный НЕ reparent'ится — пин каждый физкадр к
  `tower.global_position + TOWER_MOUNT_OFFSET` (0.55, 3.0, 0) — крыша как у
  MOUNT_LOCAL гарпуна, чуть в сторону от оси (турель/груз по центру). Вне
  SKELETON_TARGET_GROUP (как бывший экипаж; melee до крыши не достаёт). Смерть
  башни → слезает и живёт штатно. Снятие — тем же хватом руки.
- **VS-автострельба — `_weapon_tick` в `archer_soldier.gd`**: дословно боевой
  блок гарнизона (§ гарнизон стен) — 360° ближайший враг в attack_range
  (`_grn_nearest_enemy`) → ориентация конуса → штатный
  `_try_fire_at_resolved_target` (точность/стрела/cd — общий путь).
- **XP за убийства (per-instance, «храни ветеранов» буквально)**: легаси-числа
  отрядного опыта (10 XP/kill, кривая [50,120,250,500,1000] из мёртвого
  `Camp.squad_xp_*`) перенесены НА ЛУЧНИКА. Kill-credit: `Arrow.shooter_ref`
  (WeakRef) + проверка `is_queued_for_deletion` сразу после `try_damage`
  (смерть врага синхронна). Уровень: урон ×(1+0.15·lvl), кулдаун ×1/(1+0.12·lvl)
  (юзер: «простая ось темп+урон», кривая на плейтест). XP работает везде —
  поле/стена/крыша; значки = золотые кубики столбиком над головой, левелап =
  pulse-sparks + hint. Смерть = потеря опыта (как `_shots_fired`).
- **Арбалетные окна (`tower_upgrades.gd`)**: экипаж = лучник на крыше
  (`_roof_archer()` по группе `tower_archer_mounted`), а не спрятанные «В башню».
  Один лучник оживляет ВСЕ окна (`active = _windows`); болты залпа приписаны
  ему (`shooter_ref`) — его XP растёт и от окон. `debug_fake_crew` жив.
- **Бонус контракта**: ношение ручки = «несём модуль» для `Hand` → курсор сам
  взбирается на башню (ACTORS в raycast-маске), как с турелью.
- **PENDING**: команда «В башню» на карточке отряда осталась как транспорт
  (экипажем больше не считается) — судьба кнопки за юзером; баланс кривой.
...
| `dash` | Space | Tower — тап: рывок (`dash_speed`×`dash_duration`, таранит скелетов — `dash_damage`, §5.16.2). Зажать ≥0.3с: супер-дэш — у заклинания с `dash_form` сначала обводка фигуры на земле (§5.29), без формы — сразу прицел; ПКМ = коммит (работает и ПОСРЕДИ обводки = обычный супер-дэш, фигура опциональна); линия башня→круг приземления, рывок летит до точки (кламп ~11.5м) |
...
| `tutorial_hint` | `(text: String, duration: float)` | `TutorialHint._fire` (§5.26.2), `GroveGate` (§5.26.6). GameplayHud показывает плашку `TutorialHintLabel` внизу экрана на duration сек (новый текст перебивает) |
| `pad_building_destroyed` | `(building_id: StringName)` | `PadBuilding._die` (гибель БОЕМ; ПКМ-снос идёт локальным `HandPlaceAim.demolished`). GameplayHud: правило ключевых зданий колоды (§5.30) |
...
**Актуальные (сюжет «Верхний Предел» 2026-07-08, §5.28 — ЦИКЛ ПЕРЕСОБРАН, идёт плейтест юзера):**

- **⭐ СТАТУС: один заход целиком** (§5.28): пещера → Верхний Предел → форпост → 3 суток добычи/ночей → вылазки за артефактами → плата за Врата → мех + осада со всех сторон → выход-победа. Тюнинг по замечаниям юзера с плейтеста.
- **Цена выхода** `GateRuin.exit_price_bronze=900` — плейсхолдер по прикидке (доход ~2800-3300/3 суток, замок ~2100); подтвердить плейтестом.
- **Дыра «рука через пропасть»**: рука-курсор не привязана к башне — лёгкий артефакт (аккумулятор, mass 3) теоретически можно унести рукой через ущелье БЕЗ моста (дыра существовала и в v1 с Ключ-плитой). Плита-мост от неё защищена массой (11 > лифт-фильтр 10). Если плейтест покажет обход — решение за юзером (привязь руки / тяжёлый артефакт + гном-носильщик).
- **Второй шахтёрский город**: по сюжету их два; сейчас найм = Гильдия Камня (ЮВ) + замок/казармы. Кандидат — ре-тема заставы (станок уже там). PENDING обсуждения.
- ~~**Замок потерян = мягкий софтлок.**~~ РЕШЕНО юзером + сделано (§5.27.10): перезакладка — фундамент возвращается, станок перевзводится, поражения нет.
- **Усиление заставы — одобрено, не сделано:** волны на шум работы у станка (паттерн GroveGate), «печать под осадой» (~15с обороны станка после пускача; станку нужен HP — контракт зданий готов). Электро-дверь глубины СДЕЛАНА (§5.27.11).
- **Замок ур.2** — юзер: НЕ делать, хватает текущего.
- **Кузня/прокачка руки** — юзер: пока бесполезно, не делать. **Директор темпа/волны** — настраивать ПОСЛЕ костяка (костяк готов — ждём замечаний). **Расходники** — вопрос открыт (мины-связка/зелья маны — обсудить после плейтеста).
- **Подземелье v1 — кандидаты на развитие (плейтест покажет):** опасность внутри лабиринта (скелеты/волны на вынос шестерни — сейчас пусто), гейт для гномов у завала (сейчас просачиваются сквозь камни всегда — завал держит только башню), fog/темнота в лабиринте.
- **Камнеметатель у дороги на заставу:** гнездо (85,53) рядом с проёмом — первый поход за чертежом может оказаться жёстким; если душит, подрезать агро-прямоугольник по стене заставы. Теперь он же сторожит будущий восточный проход (RubbleEast, x=92 z 54..58).

Не реализовано в старших итерациях (на будущее):

- ~~**Склад + депозит гномов.**~~ СДЕЛАНО (§5.23: склад-депозит + кап ресурсов).
- **Функционал портала** (§5.17.4, Этап 2): портал → найм гномов ×3 за золото (казармы давно наняли, §5.24+). Сейчас здание только ставится.
