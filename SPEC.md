...
Магическое заклинание-ловушка. Tower запускает `SuperCarrier` в небо (reuse супер-носителя с tuned-down параметрами — меньшая высота burst'а, агрессивнее homing), тот лопается над целью и от burst-точки рассыпаются мины. Мины приземляются, 0.5с arming-delay, ждут жертв. Friendly fire ON.
...
4. Telegraph: `AoeVisual.spawn_ground_ring` в зоне scatter'а, `warning_duration ≈ 1.2с` (carrier_flight + arming_delay = ~2с до полной готовности; кольцо угасает примерно к моменту первой детонации).
5. `_spawn_carrier(target_pos)` — стандартный super-carrier pipeline.

**`_on_carrier_burst(burst_position, ground_target)`:**
- `AoeVisual.spawn_explosion(burst_position, carrier_burst_visual_radius)` — воздушная вспышка для feedback'а.
- Для каждой мины: `angle = random(TAU)`, `r = sqrt(rand) × _series_scatter_radius` (uniform-в-круге), `landing_xz = ground_target + (cos×r, 0, sin×r)`.
- Спавн мины ПРЯМО НАД landing_xz на высоте burst.y, **initial_velocity = Vector3.ZERO**. Mine.gravity (14 м/с²) сама опускает её на землю. Простая геометрия — точное приземление куда задумано, без расчёта baллистики.
- Параметры мины (`damage`, `aoe_radius`) override'ятся из `_series_*` ПЕРЕД `setup()`.

**Балансовая ниша:** между Fireball (точечный мгновенный) и Firestorm (бурст-серия моментальная). Mine Scatter — **отложенный** урон: мины ждут. Игрок тратит ману на «застолбить точку», урон применяется когда подойдут враги. Не годится для прорывов в моменте, годится для подготовки kill-зоны на будущей волне.
