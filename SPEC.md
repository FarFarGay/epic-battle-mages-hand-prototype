...
#### HandSpellMineScatter — `scripts/hand_spell_mine_scatter.gd` (2026-05-13)

Магическое заклинание-ловушка. Tower кидает carrier-«сумку» по высокой дуге, тот лопается **в апексе** (не над целью, а на пол-пути), и осколки-мины разлетаются с инерцией carrier'а + outward-разлётом + индивидуальными арками. Ощущение «вытряхнул жменю», не «удар сверху».

**Биндинг:** `equip_mine_scatter` (клавиша 5), `SpellType.MINE_SCATTER` в `HandSpell.SpellType` enum.

**Принципиально отличается от Super:** у Super carrier boost-up → homing-to-target → точечный удар сверху. Здесь — ballistic high-arc, burst в апексе, осколки получают часть инерции carrier'а и разлетаются ШИРЕ цели. Это **другой carrier** (`MineCarrier`, не `SuperCarrier`) и другая геометрия разлёта.

**`_cast`:**
1. Spell-gate (`SpellSystem.is_unlocked(&"mine_scatter")`).
2. Резолвит `mine_count / scatter_radius / mine_damage / mine_aoe_radius / cooldown / mana_cost` из `get_current_level_data(&"mine_scatter")` с fallback на @export'ы. Параметры **фиксируются** в `_series_*` (как Firestorm — series consistency).
3. Atomic mana через `Tower.try_consume_mana(p_mana_cost)`.
4. Telegraph: `AoeVisual.spawn_ground_ring` в зоне `_series_scatter_radius`, `warning_duration ≈ 1.2с` (carrier_flight + arming_delay = ~2с до полной готовности).
5. `_spawn_carrier(target_pos)`. Создаёт `MineCarrier` в `_effects_root` (current_scene), `setup(launch_pos, target_pos, carrier_launch_speed)`. Carrier сам считает high-arc баллистику и эмитит `burst(position, velocity)` в апексе.

**`_on_carrier_burst(burst_position, carrier_velocity)`:**
- `AoeVisual.spawn_explosion(burst_position, carrier_burst_visual_radius)` — воздушная вспышка в апексе.
- `inertia = carrier_velocity × inertia_factor (0.4)` — горизонтальная компонента carrier'а в момент апекса (vy ≈ 0) идёт всем минам как «forward drift». Это сохраняет направленность разлёта вперёд.
- Для каждой мины:
  - `outward_angle = random(TAU)`, `outward_speed = random(2.5, 5.5) м/с` — случайный разлёт в плоскости XZ.
  - `up_boost = random(1.5, 3.5) м/с` — лёгкий «вспух», чтобы осколки не падали моментально под carrier-vy < 0.
  - `mine_velocity = inertia + outward_xz + Vector3.UP × up_boost`.
- Спавн `Mine` ПРЯМО В burst-точке (не над приземлением) с этой initial_velocity. Mine.gravity сама опустит на землю по своей арке — каждая мина ложится в уникальную точку без хардкода координат приземления.
...
#### MineCarrier — `scripts/mine_carrier.gd`, `scenes/mine_carrier.tscn` (2026-05-13)

Доставочный снаряд для Mine Scatter. Не путать с `SuperCarrier` — это самостоятельная нода, специально под «бросок жмени», не «удар сверху».

**`setup(source, target)`:**
- **Фиксированный угол старта** `launch_angle_deg = 60°` (читаемый «лоб», узнаваемая дуга). Скорость вычисляется обратной баллистикой так, чтобы попасть из `source` в `target`:

  `v² = g·d² / (2·cos²(α)·(d·tan(α) − dy))`

  где `d` — горизонтальная дистанция, `dy = target.y − source.y`. Знаменатель > 0 ⇔ target ниже трактории под углом α. Если знаменатель ≤ 0 (target выше «потолка» арки) или вырожденный случай (d≈0) — фоллбэк на `fallback_speed × (cos·dir + sin·UP)` при том же угле.

- В отличие от Arrow.gd (low-arc, фиксированная скорость, угол выводится) — здесь **угол первичен, скорость подстраивается под расстояние**. Дуга визуально одинаковая на 5м и на 30м, отличается только скорость carrier'а.

**`_physics_process`:**
- Стандартная Newton-интеграция: `_velocity.y -= gravity × delta; position += velocity × delta`.
- `_orient_along_velocity()` каждый кадр — мещ смотрит «носом» вдоль скорости (визуально читается как дуга).
- **Apex detection:** `_prev_vy > 0 and _velocity.y ≤ 0` — vy переходит из + в −. В этой точке `_do_burst()`. `min_flight_time = 0.2с` защищает от false-positive в первый кадр.
- `lifetime = 3.0с` — failsafe: если апекс не сработал (фоллбэк-полёт без баллистики), всё равно burst на 3-й секунде.

**Визуал:** capsule-mesh (0.18 × 0.55) с медно-оранжевой emission + **GPUParticles3D `Trail`** (40 частиц/цикл 0.35с, QuadMesh с billboard'ом, оранжевый gradient затухающий к чёрному). Похоже на ракету.

**Сигнал:** `burst(world_position: Vector3, carrier_velocity: Vector3)`. Слушает `HandSpellMineScatter._on_carrier_burst` — использует velocity для inertia осколков.

#### Mine — `scripts/mine.gd`, `scenes/mine.tscn` (2026-05-13)

Объект-ловушка от Mine Scatter. Node3D + MeshInstance3D (визуально — тёмный диск с красной emission «индикатор») + Area3D `TriggerArea` (SphereShape3D radius=0.6, «педаль»).

**FSM:** `FALLING → ARMING → ARMED`.

| Phase | Поведение |
|---|---|
| `FALLING` | Гравитация (14 м/с²) опускает мину, `Area3D.monitoring = false` — никаких ложных детонаций на пролёт. **Trail emitting = true** — частицы летят за миной как у ракеты. На y ≤ `ground_y (0.05)` → ARMING. |
| `ARMING` | `arming_delay` сек (по дефолту 0.5с). Защищает от само-цепной реакции с соседями и от уже стоящих врагов в точке падения. **Trail emitting = false** — мина приземлилась, ракета «остыла», оставшиеся частицы фейдятся через свой lifetime. |
| `ARMED` | `Area3D.monitoring = true`. На `body_entered` — `_explode`. Без таймаута, мина лежит сколько надо. |

**`_explode`:** `PhysicsShapeQueryParameters3D` со SphereShape3D радиусом `aoe_radius`, маской `aoe_damage_mask`. Для всех попавших в `Damageable.is_damageable` → `Damageable.try_damage(node, damage)`. Эмитит `exploded(world_position)` для слушателей звука/частиц + `queue_free`.

**Маски:**
- `trigger_mask = ENEMIES | COLD_ENEMY | FRIENDLY_UNIT = 400` — кто активирует мину (только живые сущности; постройки на мину не наступают).
- `aoe_damage_mask = ENEMIES | COLD_ENEMY | FRIENDLY_UNIT | ACTORS | CAMP_OBSTACLE | PALISADE_OBSTACLE = 949` — кого бьёт взрыв (включая Tower/палатки/палисад — friendly fire ON, дизайнерское решение 2026-05-13).
