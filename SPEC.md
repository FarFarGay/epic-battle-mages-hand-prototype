...
ближняя «дэш-форма» (заряженный супер-рывок). Коммит `5f555fb`.
...
4. Фигура обведена → «сверкнула» → AIMING: прежний прицел (маркер цветом
   заклинания, `icon_color`), ПКМ = коммит: списание `dash_form.mana_cost`,
   рывок `super_dash_speed`×`super_dash_duration`, таран = `dash_form.damage`.
5. **Импакт:** естественный конец супер-рывка → `EventBus.super_dash_impact
...
| `dash` | Space | Tower — тап: рывок (`dash_speed`×`dash_duration`, таранит скелетов — `dash_damage`, §5.16.2). Зажать ≥0.3с: супер-дэш — у заклинания с `dash_form` сначала обводка фигуры на земле (§5.29), без формы — сразу прицел; ПКМ = коммит |
