...
**Дебаг-старт (тюнинг найма):** `GoldBank.start_bronze=200` (единая валюта, → `🥈20`); `trade_ui.HIRE_PRICE={archer_squad:10}` (лучники дёшевы под тест; прочие — `base_price=200`). Цены найма теперь в бронза-эквиваленте (одна валюта).
...
> - `GoldBank`: `_value` + `_unit(type)`=1/10/250 + `_cost_value(dict)`. Публичный API сохранён: `add_coin/get_coin` (get_coin = одометр-разряд), `can_afford/spend_cost` (составная цена → суммарное значение), `get_gold/try_spend/add_gold` (trade_ui: «золото» = весь кошелёк). HUD поллит `get_coin` → показывает одометр. Дебаг-старт `start_bronze=200` (→ `🥈20`).
...
**КОНВЕЙЕР ДОБЫЧИ (жила→шахта→линия→плавильня→двор→банк) — РЕАЛИЗОВАНО (2026-06-26, коммит d6bd8b9), всё reuse.** Альтернативная (к гном-добыче выше) АВТОМАТИЧЕСКАЯ цепочка на полимино-сетке. Стадии — буферы на `MINE_CAP=20`, каждая ТЯНЕТ из предыдущей по цепочке смежных линий (`find_via_lines(role)`, BFS), `FLOW_RATE=4/сек`. Видна «движуха»: каждая стадия наполняется, даже если дальше не достроено.
- **Жила** = `OilDeposit` (реюз, 1 клетка). **1 ТИП** — `coin_type()` всегда BRONZE (`tier`-поле легаси, не влияет). Гейт `_pad_valid`: на клетку жилы — только шахта; шахту — только на жилу.
- **Шахта** (`PAD_MINE`, role mine): копает из жилы под собой в буфер `_metal` (бронза) до cap; индикатор-столбик `_metal_fill`.
- **Линия** (`PAD_LINE`, role line): Z-тетромино, просто ЖЁЛТАЯ стена с зубцами (конвейер).
- **Плавильня** (`PAD_SMELTER`, role smelter): единое жёлтое здание со шпилем; свой буфер металла (раскалённый столб у зева) + дым (`_smelt_cd`) пока переплавляет.
- **Чеканный двор** (`PAD_MINT`, role mint): растущая стопка монет на крыше (буфер). Стадия конвейера; ручного клика-чеканки НЕТ (валюта единая, см. блок выше).
- **Гномий банк** (`PAD_BANK`, role bank): помпезная крепость; монеты в казну (`GoldBank.add_coin`) идут ТОЛЬКО через него. Фидбэк: всплывашки «+N» (реюз `SquadXpPopup` + opt-in `drift` «дымок») + **фейерверк (`GPUParticles3D`, радуга + трейлы) на КАЖДУЮ новую золотую монету** (банк сверяет `get_coin(GOLD)` до/после).
- Интерфейс стадии: `_out_amount/_out_tier/_out_remove/_in_room/_in_add`, `get_role()`. По цепи течёт бронза (одометр катит в серебро/золото в казне). См. [[project_ebm_ore_deposits]].
