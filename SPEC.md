...
- **Этап 3 (утверждён, НЕ реализован): МОДУЛИ-ЛОДАУТ** — кафедра производит модуль-АППАРАТ (grabbable), рука ставит на слот корпуса (2-3 слота: лодаут физический, без UI), спелл активен пока модуль стоит; гном-оператор садится за аппарат (театр); снос кафедры не трогает установленное. Пилот — гарпунная турель.

**УРОВНИ ЗДАНИЙ — Clash-инкремент (2026-07-07, юзер: «инкрементальной хуйни как в Clash of Clans»).** Продюсеры (шахта, институт) апгрейдятся **ЛКМ-кликом по зданию** (тот же клик-паттерн, что найм казармы): `PadBuilding._level` 1→3, выработка ×1/×2/×4 (`LEVEL_OUTPUT_MULT`, множится ПОВЕРХ осей квартала и защёлка), цена растёт втрое на шаг при доходе вдвое (`upgrade_cost_bronze` = base×2 → base×6 — инкремент-кривая «первая шахта подарок, прокачанная — цель»). Визуал: N-1 золотых светящихся колец на мачте (`_add_level_accents`, generic поверх любого визуала). Hover-плашка получила строку «⭐ Уровень ×N (клик: ур.N+1 за M🥉)» и уровень в заголовке (рядов 5→6). Не хватает монет — хинт с ценой; макс — хинт «максимальный уровень». Кандидаты на распространение: казарма (кап гарнизона), растущая цена повторных зданий (×1.5 за штуку), рука-дойка (кликер-актив; юзер выбирал Clash-прочтение — отложено).
