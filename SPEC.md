...
**Дебаг-старт (тюнинг найма):** `GoldBank.start_gold=100`; `trade_ui.HIRE_PRICE={archer_squad:10}` (лучники дёшевы под тест; прочие — `base_price=200`).
...
- **Жила** = `OilDeposit` (реюз, 1 клетка) + `tier` (BRONZE/SILVER/GOLD, @tool тинт диска). Гейт `_pad_valid`: на клетку жилы — только шахта; шахту — только на жилу.
- **Шахта** (`PAD_MINE`, role mine): копает из жилы под собой в буфер `_metal` (тип = тир) до cap; индикатор-столбик `_metal_fill` цвета тира.
- **Линия** (`PAD_LINE`, role line): Z-тетромино, просто ЖЁЛТАЯ стена с зубцами (конвейер).
- **Плавильня** (`PAD_SMELTER`, role smelter): единое жёлтое здание со шпилем; свой буфер металла (раскалённый столб у зева) + дым (`_smelt_cd`) пока переплавляет. Сама монеты НЕ чеканит.
- **Чеканный двор** (`PAD_MINT`, role mint): растущая стопка монет на крыше (буфер); ручная чеканка 100→1 ПЕРЕНЕСЕНА сюда с плавильни (клик по двору).
- **Гномий банк** (`PAD_BANK`, role bank): помпезная крепость; монеты в казну (`GoldBank.add_coin`) идут ТОЛЬКО через него. Фидбэк: всплывашки «+N» с иконкой номинала (реюз `SquadXpPopup` + opt-in `drift` «дымок») + фейерверк (`GPUParticles3D`, радуга по hue + трейлы) каждые 100 монет.
- Интерфейс стадии: `_out_amount/_out_tier/_out_remove/_in_room/_in_add`, `get_role()`. Тир тянется по всей цепи от жилы. См. [[project_ebm_ore_deposits]].
