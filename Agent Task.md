...
Phase A системы боевых групп запушена (`3694749` + `ebad70b`): новые
Resource'ы `UnitEntry` / `CombatGroup`, расширенный `WaveStage.groups`
(legacy `skeletons_per_wave` сохранён для backward compat), ветка в
`WaveDirector` (`_spawn_groups_wave` / `_spawn_legacy_poi_wave`), демо-cheat
«Многофронтовая волна». Тестирование пока только через cheat — реального
контента в .tres ещё нет.

Дальнейшая работа:

- **Содержательный `wave_schedule_multifront.tres`.** Сейчас единственное
  расписание `wave_schedule_default.tres` использует legacy single-front
  модель. Создать новое расписание (или обновить default) со стадиями,
  использующими `groups`. Дизайн-задача: первая стадия — 1-2 фронта,
  средняя — 3 фронта с разными composition, поздняя — 4 фронта +
  смешанные типы (когда появятся новые юниты). Хорошо бы 2-3 разных
  .tres под разные POI («лёгкий лес», «тяжёлая равнина», …).
