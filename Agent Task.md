...
## TODO для следующей сессии — модуль армии

Phase 1+2+3 (частично) запушены. Что осталось:

- **Distribution внутри отряда.** Сейчас `target_for_member` распределяет юнитов по кольцу radius=1.6м вокруг центра (для HOLD/ESCORT). На больших отрядах (10+) кольцо тесное. Нужны: динамический радиус от squad_size, или 2 концентрических кольца. (DEFEND-state кольцо не использует — wander-патруль per-soldier.)
- **Ограничения по spawn-логике.** Можно призвать всех gatherer'ов в копейщиков, оставить лагерь без сборщиков. Это by design? Или нужен резерв (мин. N gatherer'ов на палатку)?
- **Доступ к призыву** — сейчас «Армия» вкладка журнала всегда доступна (когда лагерь развёрнут). В будущем — может гейтить через здание-казарму (`Camp.try_build` уже есть для построек).
- **Второй тип юнита** (опц.). Сейчас копейщик единственный мобильный класс. Если придёт время — лучник-кочевник с дальним боем (вернуть `_fire_at`-flow через subclass).
