...
## Сессия 2026-07-02 — Ревизия «проект разросся» №2 + безопасный слой чистки

### Контекст / запрос
Геймдизайнер: «посмотри код, структуру, баги, костыли — надо почистить немного». 5 параллельных Explore-агентов (враги/волны, свои юниты, стройка/экономика, camp-легаси, HUD/рука/спеллы), каждое агентское утверждение перепроверено grep'ом/чтением до правок.

### Главный вывод аудита
**В шиппинг-сцене level_rooms.tscn исполняется меньше половины кодовой базы.** main.tscn из кода недостижим (нет ни одного change_scene; start_menu → reload_current_scene). Только в main.tscn живут: Camp (~4.5k строк), ВЕСЬ WaveDirector-пласт (режиссёр волн/осада/боссы/банды, ~2.5k с обвязкой), harvester, fog_of_war, quest_actor, match_goal, обе старые модели стройки (BuildGrid + HandBuildAim, ~2.4k). Недостижимый контент в rooms: заклинания frost/super/meteor, ВСЕ спелл-апгрейды (try_upgrade требует Camp), заряд/ульты отрядов (маркер спавнит только Camp; заряд копейщика ненаполним — add_charge на килле удалён), DEFENDING_CAMP, alarm лучника. Легаси припарковано АККУРАТНО: ноль краш-дерефов, всё под null-гейтами.

### Что закрыли (безопасный слой)
- **[BUG] gameplay_hud**: spell_shop_requested/spell_unlocked подключались в _ready, но отсутствовали в _disconnect_eventbus (нарушение собственного инварианта файла, свежий спелл-шоп-код) — добавлены.
- **[BUG] pad_building._build()**: free детей убивал _conn_overlay без сброса _conn_state → хинт стыковки молча пропадал до конца сессии (гард по state съедал пересоздание). Сброс добавлен.
- **[BUG] gnome_squad_spawner**: срез «последние add» в request_squad_for/request_squad при недоспавне (instantiate==null) захватывал СТАРЫХ бойцов как «новых» → переназначение постов. Срез от prior-size.
- **[DEAD] Enemy.set_targets / Enemy.is_frozen** — 0 вызовов (grep), выпилены + доки.
- **[DEAD] SpellSystem.spell_data** — дубль get_spell_data (тела идентичны), выпилен, HUD переведён.
- **[DEAD] scenes/enemy_arrow.tscn** — сирота (лучник с 2026-05-22 стреляет aoe_arrow.tscn), удалена. Заметка прошлой сессии «enemy_arrow используется arrow.gd» была про докстринг-упоминание, не usage.
- **[DOC] skeleton_archer** докстринг врал «инстанцируется как Arrow» — код кастует as AoeArrow (наследник GiantStone!); дизайнер по доке поставил бы Arrow-сцену → молчащий лучник. Исправлен + SPEC (§5.5.3.1 ретирован, зависимости).
- Компайл-чек: headless-бут level_rooms — чисто.

### НЕ тронуто сознательно (ждёт решения дизайнера)
- **Голд-гол-панель HUD (~250 строк мертва)** — завязана на открытую развилку «победа: нефть vs монеты/врата». Канон «монеты» = панель включается обратно одной строкой.
- **TEMP_FREE_BUILD=true** (room_build_site) — вся доставочная экономика стройки выключена костылём; **start_gold=100 (DEBUG)** шипнут в gold_bank. Дизайн-тумблеры, не чистка.
- **Ампутация main.tscn-пласта (~10-12k строк)** — см. project_ebm_legacy_cleanup: 5 guard-точек сращения перечислены агентом camp-аудита.
- **[BUG-latent] Skeleton._far_step зовёт super._ai_step** → SkeletonGiant в FAR-LOD теряет тик заряда/доджа. Редкий, фикс = поведение врагов, отложен.
- **[BUG?] ворота на 1 клетку стены** сносят ВСЮ стену с полным рефандом (_remove_walls_under) — дизайн-вопрос «что должно оставаться».
- Рефакторы по желанию: dodge-механика скопирована в 3 врагах; гарнизон лучника/копейщика — 2 параллельные реализации (_grn_* vs _post_*, поведение уже разъехалось); PadBuilding — роль размазана по 8 match'ам вместо колонок каталога; hand_spell_* — 5 почти идентичных модулей под HandSpellBase.

### Метод / грабли
- Один агент преувеличил: «collector-FSM гнома целиком мёртв» — headless-бут показал [Gnome] IN_TENT → SEARCHING у воркеров в rooms. Статическая достижимость ≠ рантайм: gnome.gd резать только после трассировки.
- Мёртвые ключи каталога проверять И по строковым id в .tscn (building_id) — в этот раз чисто, но ловушка реальна.
- Из ~40 находок агентов до правок дожили все проверенные; отфильтрованы 2 FP (JournalPanel «не отписывает spell_unlocked» — отписывает; «функции >100 строк в HUD» — max 98 и та мёртвая).
...
### Контекст / запрос
Геймдизайнер: «зона спавна очень большая, больше плейна; группа иногда спавнится за пределами плейна или слишком далеко от лагеря». При диагностике обнаружил три проблемы: (1) Ground несимметричный — BoxMesh 400×400 со `scale.z=0.439` → реальные размеры 400×175.7м, ассимметрия по Z. (2) SpawnZone1 расположена сильно сбоку (`SpawnZones` контейнер @ Z=-133, локальный scale (6.53, 1, 1.66)) и **перекрывала лагерь** — точка спавна uniform внутри 392×100м могла оказаться вплотную к палаткам. (3) `wave_safe_radius=32` декларировался в коде, но к POI-волнам не применялся — только к фоновому спавну. Документация на параметре врала.

Решение по запросу: упростить геометрию. Квадратная карта 300×300, центральная точка — лагерь+POI+ресурсы+башня. Одна SpawnZone размером с карту. Safe-фильтр применяется к POI-волнам, чтобы зона-размером-с-карту не давала спавн вплотную к палаткам.

### Главные изменения

- **`main.tscn` геометрия:**
  - Ground: `BoxShape3D`/`BoxMesh` 400→300, убран кривой `scale.z=0.439` → симметричный квадрат 300×1×300, origin (0, -0.5, 0).
  - Tower: (159.15, 3, 0) → (0, 3, 0).
  - Hand: (164.49, 2.5, 0) → (5.35, 2.5, 0) — сохранён старый offset от Tower.
  - OctagonTurret1: (167.61, 0.35, 4) → (8.47, 0.35, 4) — тот же offset.
  - ResourceZones (4 шт.) — каждая сдвинута на (-159.15, 0, 0) относительно старого Tower-якоря: получились (-29.7, 13.4) / (2.98, 11.5) / (5.02, -16.7) / (-27.7, -15.4).
  - Poi_Heart: (-119.65, 0.3, -0.36) → (0, 0.3, 0). Poi_ESE удалён целиком (один POI достаточно для тестов).
  - SpawnZones-root: убран translate (был Z=-133.22) → identity.
  - SpawnZone1: убран scale (6.53, 1, 1.66), translate (-0.88, 0, 127.81) → identity. `size = (300, 300)`. Зона теперь ровно с картой.
  - `EnemySpawner.map_half_extent`: 195 → 150 (под 300×300).

- **`wander_map_half_extent` в Skeleton и Gnome:** 195 → 145 (150 − 5м буфер). Скелеты/гномы wander-ят строго в пределах нового плейна.

- **`WaveDirector._pick_safe_point_in_zone(zone)` — новый helper.** Up to `wave_position_attempts` (30) попыток выбрать uniform точку в zone, отбрасывая те, что в `_safe_score(p) < 0` (т.е. внутри `wave_safe_radius` от любого Camp или `safe_radius` от любого POI). Если за 30 попыток safe-точка не найдена — берётся «лучшая» (с максимальным excess'ом), как в `_pick_safe_pos`. Это страховка: даже если zone перекрывает лагерь полностью — спавн не окажется вплотную к палаткам. Заменяет прямой `_spawner.random_point_in_zone(zone)` в `_spawn_legacy_poi_wave` и `_spawn_single_group`.

### Главное архитектурно

- **Один POI и одна SpawnZone — конфигурация для отладки.** Дизайнер просил «сделать проще, потом будем думать что добавлять». На карте 300×300 с лагерем в центре + safe_radius=32 — кольцо спавна 32..150м от центра. Скелетам идти ~30-100м до палаток, плюс-минус секунд 15-30 в зависимости от позиции. Если потребуется больше POI / зон — все механизмы (CombatGroup, multifront, per-zone budget) уже готовы.
- **Safe-фильтр в POI-волнах = страховка, не редизайн.** Структурно правильное решение — зоны привязаны к POI и стоят с разных сторон в разумной близости (см. отложенный вариант B диагностики). Но текущий запрос «упростить» — поэтому фильтр работает на одной большой зоне. Когда дизайнер добавит зоны вокруг POI меньшего размера, фильтр продолжит работать (он не зависит от количества/размера зон) и просто реже будет включаться.
- **`map_half_extent` остаётся квадратным.** Сейчас карта симметричная, отдельные X/Z не нужны. Если в будущем плейн станет прямоугольным — придётся раздельные `map_half_x`/`map_half_z`. Запоминаю как точку расширения.
- **POI safe_radius=12 (QuestActor default), wave_safe_radius=32 (WaveDirector).** Точка должна быть вне max(12, 32) = 32м от лагеря/POI, оба в (0,0). Эффективное запрещённое кольцо — 32м.

### Что НЕ сделано / отложено
- **Контентное `wave_schedule_multifront.tres`** — TODO для следующей сессии (есть в плане). Сейчас расписание `wave_schedule_default.tres` — legacy single-front, 3 стадии.
- **Per-group `target_priority`** (ANCHOR / SPECIFIC_TENT / FURTHEST_TENT) — отложено.
- **Зоны привязанные к POI** (вариант B диагностики) — пока не нужен с одной центральной локацией.
- **Cheat «Многофронтовая волна»** — на текущем layout с одной зоной даст один фронт (только одна группа). Это естественное следствие 1 зоны, не баг. Когда добавится 2-3 зоны вокруг POI — чит сам собой станет многофронтовым.
- **GrassField** — coverage_target_path читает Ground/GroundMesh world AABB, должен подхватить новый симметричный плейн автоматически. Если в редакторе будет visual mismatch — сразу видно по чанкам, тогда правим. Не проверял в running.

### Метод
- Парс-чек `--check-only` чистый. Editor-pass не требовался — новых `class_name` не добавлял.
- Главное по диагностике: считать **итоговые world-extents** SpawnZone с учётом ВСЕХ родительских transform'ов. Локальный scale (6.53, 1, 1.66) × size (60, 60) × дочерний от контейнера с translate Z=-133 — давал world rectangle X∈[-197, 195], Z∈[-55, 44]. Это **не очевидно из инспектора одной ноды**.

## Сессия 2026-05-11 (3) — Костёр горит от лагеря, не от квеста

### Контекст / запрос
Геймдизайнер: «костёр не горит когда ставишь лагерь, давай сделаем чтобы загорался если поставил лагерь». До правки `QuestActor._refresh_visual` зажигал костёр через `QuestProgress.is_active(quest_order)` — а у `Poi_Heart.quest_order=1` после удаления Poi_ESE статус оставался locked (предыдущий квест quest_order=0 теперь не существует), и POI никогда не становился active.

### Главные изменения

- **`QuestActor._is_camp_deployed_here` — новый флаг**, драйвер «горения». true если на этом POI развёрнут лагерь (anchor в `safe_radius`). Завязки на quest-state нет: костёр горит просто от факта лагеря, независимо от того открыт ли тут квест.
- **Подписка на `EventBus.camp_deployed` / `camp_packed`** в `_ready`. На `camp_deployed(anchor)`: `is_within_safe_radius(anchor)` → флаг true, refresh. На `camp_packed`: флаг false если был true, refresh.
- **`_refresh_visual()`** — приоритеты:
  - `completed` → отгоревший (как было; задание сделано = след зелёного огня даже при лагере)
  - `_is_camp_deployed_here` → горящий
  - иначе → потухший (locked)

### Главное архитектурно
- **Один Camp в проекте → `camp_packed` без аргументов однозначен.** Если когда-нибудь появятся несколько лагерей одновременно — придётся добавить anchor в сигнал packed. Сейчас не нужно.
- **Семантика «костёр = очаг лагеря»**, а не «маркер квеста». Раньше дизайн был «костёр горит когда у тебя есть открытое задание сюда», теперь — «костёр горит когда у его костра стоит ТВОЙ лагерь». Это ближе к «места силы» из спека: лагерь освящает костёр.
- **Completed имеет приоритет** над лагерем. Развернуть лагерь на пройденном POI — он останется зелёным «отгоревшим». Это сохраняет визуальную маркировку прогресса.

### Что отложено
- Если у POI несколько источников «горения» (например, лагерь + ритуал + квест) — пока единый источник (лагерь). При добавлении других — нужна композиция флагов.
- `QuestProgress.is_active` остался в коде — используется журналом для рендера карточек заданий. С костром больше не связано.

## Сессия 2026-05-11 (4) — wave_schedule_default переведён на CombatGroup'ы

### Контекст / запрос
После закрытия первой проблемы дня (карта + safe-фильтр) — следующий пункт из плана: содержательное расписание с groups вместо legacy. Запрос: «делаем контентные волны».

### Что сделано
`resources/wave_schedule_default.tres` переписан с legacy `skeletons_per_wave` на новую модель `groups: Array[CombatGroup]`. Три стадии:

- **Stage 0 «разведка»** (duration=60с, interval=60с): 1 группа из 5 скелетов, spread=1.0. Один кластер раз в минуту — тихий начальный темп.
- **Stage 1 «давление»** (duration=90с, interval=45с): 2 группы — основная (5 скелетов, spread=1.0) и облегчённая (3 скелета, spread=1.3 — чуть разреженнее). Сумма 8/волна.
- **Stage 2 «осада»** (бесконечная, interval=30с): 3 группы — 5/4/3 скелетов с spread'ами 1.0/1.2/1.5. Самая «облачная» группа из 3-х с spread=1.5 = ~6м радиус — приходит вразлёт.

Все группы используют `spawn_zone_index = -1` (random live zone). На текущем main.tscn с одной SpawnZone все группы резолвятся в одну zone, но `_pick_safe_point_in_zone` (rejection sampling 30 попыток) даёт **разные safe-точки внутри зоны** для каждой группы → визуально многофронт даже с одной зоной. Когда дизайнер добавит зоны вокруг POI с конкретными `spawn_zone_index`, расписание даст реальный multifront без правок.

`UnitEntry`-ресурсы переиспользуются между стадиями: `Entry_skel_5/4/3` — три экземпляра, обслуживают 6 групп. CombatGroup'ы не шарятся между стадиями (по дизайну: тюнинг spread/zone_index per-stage).

### Полировка лога
`WaveDirector` на stage_advance логировал `skeletons_per_wave` — поле игнорируется в groups-driven стадиях, лог писал misleading «5 скел/волна». Заменил на ветку: `has_groups()` → «N групп / M юнитов», иначе legacy «N скел/волна».

### Архитектурно
- **Текущий default — groups-driven, не legacy.** Дизайнер открывает один файл и видит сразу как настроить multi-кластер композицию. Legacy-ветка `_spawn_legacy_poi_wave` остаётся в коде для обратной совместимости (старые .tres продолжат работать), но default уже не использует.
- **Безопасный multifront на одной зоне.** Раньше «multifront» требовал несколько SpawnZone. Сейчас safe-фильтр rejection sampling даёт нескольким группам разные точки в одной зоне → визуально похоже на многофронт. Граница: на маленькой zone группы будут пересекаться или клиппиться к одной стороне; на большой (как сейчас 300×300) — разлёт ощутимый.
- **`cluster_spread` теперь по-настоящему используется.** До этого все группы шли spread=1.0. Stage 2 — три кластера с spread'ами 1.0/1.2/1.5 — даёт визуальное разнообразие «плотная / средняя / разреженная пачка». Это designable, не magic constant в коде.

## TODO для следующей сессии — боевые группы и волны

Phase A + контент закрыты. Следующие пункты:

- **Per-group target_priority.** Сейчас все группы метятся в
  `nearest_part_to(origin)`. В CombatGroup есть только заглушка enum
  `TargetPriority { NEAREST_TENT }`. Добавить варианты:
  - `ANCHOR` — целить центр развёрнутого лагеря (если есть `deploy_anchor`).
  - `SPECIFIC_TENT` — указать конкретную палатку по индексу (для скриптовых
    атак типа «снайперский удар по самой северной палатке»).
  - `FURTHEST_TENT` — игнорить ближайшую, идти к дальней (обходной манёвр).
  Реализуется в `WaveDirector._spawn_single_group` после резолва zone'ы.

- **Per-group behavior_hint** (под второй тип юнита). Когда появится
  ranged-юнит (skeleton-archer) или support (mage с щитом), композиция
  «8 melee + 3 ranged» работает эмерджентно — каждый юнит сам решает
  по своему AI. Но для дизайнерского контроля может потребоваться
  «эта группа держится в тылу, эта рвётся вперёд» — через behavior_hint
  enum в CombatGroup или в UnitEntry. Не делать преждевременно — сначала
  посмотреть, нужно ли это после появления второго типа.

- **Второй тип юнита** (по плану из предыдущих заметок). Skeleton-archer
  как первый кандидат — ломает «лучники у палаток в безопасности». Теперь
  добавляется как `UnitEntry.scene = skeleton_archer.tscn` в композиции
  существующих CombatGroup'ов, **без правок системы волн**. Нужно:
  собственный AI (ranged-tick), своя сцена, тюнинг параметров (range,
  cooldown, damage), визуал-отличия от обычного скелета (цвет / форма).

- **Тюнинг новой модели после плейтеста.** После реального .tres-контента
  — смотреть как ощущаются волны. Возможно потребуется:
  - Дополнительный параметр `WaveStage.wave_jitter: float` (рандом
    в `wave_interval` чтобы темп не был метрономом).
  - `CombatGroup.delay_offset: float` (одна группа спавнится сразу,
    другая через 2с — «приманка → засада»).
  - Возможность подмешивать legacy single-front в стадии с groups.

## TODO модуля армии (отложенные)

- **Доступ к призыву** — сейчас «Армия» вкладка журнала всегда доступна
  (когда лагерь развёрнут). В будущем — может гейтить через здание-казарму
  (`Camp.try_build` уже есть для построек). Решение по дизайну: убираем ли
  «армию из коробки» или оставляем как часть базового набора.
- **Второй тип юнита** для армии игрока (опц.) — лучник-кочевник с
  дальним боем (вернуть `_fire_at`-flow через subclass). Параллельно
  с врагом-лучником, симметрично.

## Сессия 2026-05-10 (6) — Армия: HOLD-приоритет, dismiss, адаптивная скорость, scroll

### Контекст / запрос
Геймдизайнер: «делаем отряды дальше, пункты 1, 2, 3, 6, 7». Закрываем 5 из 9 пунктов TODO модуля армии: live-debug «Идти сюда» (1), HOLD = жёсткий приоритет (2), dismiss-кнопка с proximity-гейтом (3), адаптивная скорость в эскорте (6), ScrollContainer для карточек (7).

### Главные изменения

- **`Squad` — `_strict_move` флаг + `remove_member` API.**
  - `command_hold(pos)` ставит `_strict_move = true`; `command_escort()` — false.
  - `is_strict_move()` геттер. Используется юнитами в `_active_tick`.
  - Флаг squad'а не сбрасывается на «прибытие» — вместо этого каждый юнит проверяет per-self: если он в `HOLDING_POSITION + strict + ещё не в слоте` — игнорирует бой и идёт. Дойдя — сам выпадает в обычную ветку (per-soldier `dist > arrival` → false). Это избавляет от per-soldier «arrived»-флага и от сложной координации между членами отряда.
  - `remove_member(soldier)` — публичный аналог приватного `_on_member_destroyed`. Нужен dismiss'у: `queue_free()` без `_die()` не фронтит `destroyed`-сигнал, авто-чистка squad'а не сработала бы.

- **`SoldierGnome._active_tick` — strict-march ветка + helper `_move_toward`.**
  - В начале тика, ДО обычного `_find_target`: если squad в HOLD-strict и `dist > SQUAD_TARGET_ARRIVAL` — двигаемся через `_move_toward`, return. Бой полностью игнорируется (не сканируем `_find_target`, не стреляем).
  - `_move_toward(to_goal_xz, dist)` — копия паттерна `DefenderGnome._tick_following_caravan`: lerp от `move_speed` до `caravan_sprint_speed` по доле `caravan_full_sprint_distance`. Все три параметра — унаследованные exports из `Gnome`. Применяется и в strict-march, и в обычном «движемся к слоту» (в т.ч. эскорт). Защитник был один такой быстрый — теперь и солдат догоняет.

- **`Camp` — dismiss-API (новый + 1 export).**
  - `@export var dismiss_radius: float = 12.0` — чуть больше `deploy_radius=8` чтобы slot защитника (~12м снаружи) тоже укладывался.
  - `can_dismiss_squad(squad)` — guard: `_squads.has + gnome_scene != null + count_alive > 0 + ВСЕ члены в `dismiss_radius²` от `_deploy_anchor` (xz-distance, без sqrt).
  - `dismiss_squad(squad)` — итерация по `members.duplicate()`: для каждого живого солдата spawn нового gatherer'а через существующий `_spawn_one_gnome(gnome_scene, tent, "gatherer")`, выставление позиции = soldier.global_position, в `State.DEPLOYED` дополнительно `gatherer.enter_deployed()` (иначе сидел бы IN_TENT). Затем `_gnomes.erase(soldier) + squad.remove_member(soldier) + soldier.queue_free()`. На `members.is_empty()` squad сам эмитит `disbanded`, дальше Camp слушает и эмитит `EventBus.squad_disbanded` — карточка HUD'а сама уйдёт.
  - `_pick_tent_for_new_gatherer()` — palace для нового gatherer'а: первая палатка с вакансией; fallback — первая живая.

- **`gameplay_hud` — ScrollContainer + dismiss-кнопка + dynamic refresh + debug-print.**
  - `_ensure_squad_panel`: теперь создаёт `ScrollContainer` (anchored TOP_RIGHT, до `-20px` от низа экрана, `horizontal_scroll_mode = DISABLED`) + внутрь VBox для карточек. На 6+ отрядах больше не выпадает за экран.
  - `_build_squad_card`: вторая `HBoxContainer` строка с кнопкой «Распустить». Disabled по `Camp.can_dismiss_squad`. Tooltip объясняет «Размобилизация только в лагере — верните юнитов» / «Конвертировать обратно в gatherer'ов на их позиции».
  - `_apply_dismiss_state(btn, squad)` — общий helper, используется и из event-driven `_refresh_squad_card`, и из периодического `_update_squad_cards_dynamic` (раз в `UPDATE_INTERVAL = 0.25с`). Юниты ходят без сигналов о позиции — нужно поллить.
  - `_on_squad_aim_pressed` — добавлены детальные debug-print'ы под `LogConfig.master_enabled`: «кнопка нажата», «squad не найден», «hand не резолвится», «hand.squad_aim == null». По логу мгновенно видно где обрывается цепочка. Это закрывает пункт «проверить live» в плане «дать игроку видимый ход отладки».

### Архитектурно
- **Strict-march как squad-state, проверяемый per-soldier на тике.** Альтернатива — на squad держать список «дошедших» — требует синхронизации между членами и обработки edge-кейсов (член умер не дойдя; новый член присоединился; отряд переместили mid-march). Текущий подход stateless: каждый юнит на своём тике сам решает «я дома или иду» — по геометрии. Дешевле и реактивнее, не нужна координация.
- **Dismiss = ленивый `queue_free` + явный `remove_member`.** Не используется `_die()` (он делает damage-эффекты + shatter) — солдат просто исчезает, на его месте появляется gatherer. Заметнее визуально было бы puff-эффект, но оно требует ассетов; на прототипе достаточно мгновенной замены. Если придёт время — добавить short particle-вспышку в `dismiss_squad` через `ShatterEffect.spawn` с белым цветом.
- **`dismiss_radius=12`, не `deploy_radius=8`.** Защитник слот = `deploy_radius+~4м снаружи (patrol_radius=12)`; squad на эскорте может стоять примерно там же. С 8м dismiss защитников не пускал бы. С 12м — ок, плюс небольшой запас если отряд жмётся к границе.
- **Адаптивная скорость = переиспользование уже-существующих exports `caravan_sprint_speed=9.0` / `caravan_full_sprint_distance=5.0` из `Gnome`.** Не вводим новые параметры. Если на балансе окажется что солдаты бегают слишком быстро — добавить override в SoldierGnome.tscn (per-сцена). Сейчас одна цифра на всех гномо-классов.
- **ScrollContainer.SCROLL_MODE_DISABLED для горизонтали.** Карточка `size_flags_horizontal = SIZE_EXPAND_FILL` подгоняется под ширину viewport'а scroll'а — не вылезает по горизонтали, scroll-bar не появляется. Только вертикальный.

### Что НЕ сделано / отложено
- **Live-проверка «Идти сюда»** в running игре — добавлен расширенный debug-output, но запустить надо вручную. Если в логе видно `[HUD:SquadAim] кнопка нажата` → `[Hand:SquadAim] aim старт` → `commit @ (...)` — фикс полностью работает. Если обрыв в каком-то месте — print точно укажет.
- **Visual feedback на dismiss** (puff-эффект на месте солдата) — отложено до доступности эффекта.
- **Distribution на больших отрядах (пункт 5)** — не трогал. Кольцо radius=1.6м всё так же. Если призывать второй тип юнита (пункт 4 — копейщик), кольцо актуально пересмотреть вместе.
- **Spawn-лимиты (пункт 8) и здание-казарма (пункт 9)** — game-design решения, отложены.

### Метод
- Парс-чек `--check-only` чистый. После правок только в существующих файлах (без новых `class_name`) editor-pass не требуется — обычный `--check-only` достаточно.
- Адаптивная скорость скопирована один-в-один из `DefenderGnome._tick_following_caravan:642` — единый паттерн «sprint когда отстал, walk когда в строю» для всех классов гномов на эскорте.
