...
Phase 1+2 базовой армии запушены (см. сессии 2026-05-10 (4) и (5) ниже): призыв отрядов через журнал, squad-сущности, 2 команды («Идти сюда» / «За башней»), HUD-карточки справа. Что нужно доработать:

- **Проверить «Идти сюда» в реальной игре.** Фикс с lazy-lookup `_camp` запушен в `b64d7c3`, но на момент завершения сессии не проверено живьём. Если debug-print'ы `[Hand:SquadAim] aim старт` / `commit` появляются в Godot output — фикс сработал. Если нет — копать дальше:
  - HUD-кнопка не вызывает `toggle_aim_for` (resolve_hand нашёл не тот узел?)
  - SQUAD_AIM-mode активируется, но ПКМ ловится не там (overlay перехватывает?)
  - Squad команда применяется, но `SoldierGnome._active_tick` всё равно идёт к башне (проверить `_squad.target_for_member` — `state == HOLDING_POSITION`)
- **Команда «Идти сюда» = жёсткий приоритет.** Геймдизайнер настаивал «точное указание места это четкое указание которое все прерывает». Сейчас юниты при увиденном враге останавливаются и стреляют (attack-and-move). Возможно надо: при HOLD-команде юниты идут до точки **игнорируя бой**, а только по прибытии стреляют. Или промежуточное: «гоу-mode» с быстрым движением + «hold-mode» с боем. Решить с дизайнером.
- **Phase 3: dismiss-кнопка** на карточке отряда (или в журнале). Солдат → gatherer обратно. По дизайнерскому решению размобилизация только в лагере — нужно гейтить «соldier рядом с палаткой» или выводить кнопку по proximity.
- **Phase 3: второй тип юнита** — копейщик (melee, ближний бой, толще лучника). Catalog-entry в `SoldierSystem.SOLDIER_CATALOG`, отдельная сцена `soldier_pikeman.tscn`, в `_active_tick` ветка для контактного damage'а вместо стрелы (либо subclass `SoldierGnome` с override'ом fire-логики).
- **Distribution внутри отряда.** Сейчас `target_for_member` распределяет юнитов по кольцу radius=1.6м вокруг центра. Это компактно. На больших отрядах (10+) кольцо тесное. Нужны: проверка что 5 юнитов нормально стоят, динамический радиус кольца от squad_size, или 2 кольца концентрических.
- **Эскорт vs движение башни.** В ESCORTING_TOWER юниты идут к `tower_position` каждый кадр. Пока башня едет — они догоняют (если move_speed достаточен). Возможно надо: в эскорт-режиме адаптивная скорость (далеко = быстрее, рядом = walk), как у DefenderGnome'а.
- **Карточки и пустые отряды.** Disbanded отряд исчезает с HUD'а — норм. Но что если у игрока 5+ отрядов одного типа? Сейчас панель `_squad_panel` фиксированная высота 600px, на 6+ карточек может выйти за экран. Нужен ScrollContainer.
- **Ограничения по spawn-логике.** Можно призвать всех gatherer'ов в солдат, оставить лагерь без сборщиков. Это by design? Или нужен резерв (мин. N gatherer'ов на палатку)?
- **Доступ к призыву** — сейчас «Армия» вкладка журнала всегда доступна. В будущем — может гейтить через здание-казарму (`Camp.try_build` уже есть для построек).
